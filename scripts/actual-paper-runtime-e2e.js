...
if (require.main === module) {
  run(parseArgs(process.argv.slice(2))).then((evidence) => {
    process.stdout.write(`${JSON.stringify({ result: evidence.result, evidence_type: evidence.evidence_type, source_commit: evidence.source_commit, artifact_hash: evidence.artifact_hash })}\n`);
  }).catch((error) => {
    process.stderr.write(`actual PAPER runtime E2E failed: ${error instanceof Error ? error.message : String(error)}\n`);
    process.exitCode = 1;
  });
}
