...
const SAFETY_INPUT_FIELDS = ["liveCapabilityDetected", "privateApiCapabilityDetected", "liveAbsent", "privateApiAbsent", "credentialAbsent", "openP0"];
  ...
  for (const root of SCAN_ROOTS) {
    for (const file of sourceFiles(path.join(ROOT, root))) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (waived) continue;
        for (const match of line.matchAll(FIELD_PATTERN)) findings.push({ file: path.relative(ROOT, file), line: i + 1, field: match[1], value: match[2] });
      }
    }
  }
  return findings;
...
if (require.main === module) {
  // Temporary diagnostic: preserve exact advisory identities before the unchanged security gate
  // evaluates them. Remove once the dependency remediation is committed.
  require("./audit-diagnostic.js");
  const findings = scan();
  if (findings.length === 0) console.log("[safety-inputs] no unwaived safety-gate literals found");
  else {
