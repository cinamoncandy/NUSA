...
function patchImageSizeIcns(text) {
  const before = "const imageHeader = readImageHeader(input, imageOffset);\n        const imageSize = getImageSize(imageHeader[0]);";
  const after = "const imageHeader = readImageHeader(input, imageOffset);\n        if (imageHeader[1] < 8) throw new TypeError('Invalid ICNS entry size');\n        const imageSize = getImageSize(imageHeader[0]);";
  return patchExact(text, before, after, 2, "image-size/icns");
}
