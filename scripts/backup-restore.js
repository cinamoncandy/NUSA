...
const AUTHORIZATION_OBJECT = '[A-Za-z_$][\\w$]*(?:(?:\\s*\\.\\s*[A-Za-z_$][\\w$]*)|(?:\\s*\\[\\s*["\\'][^"\\']+["\\']\\s*\\]))*';
const AUTHORIZATION_TARGET = `(?:${AUTHORIZATION_OBJECT}\\s*(?:\\.\\s*authorization|\\[\\s*["\\']authorization["\\']\\s*\\])|["\\']?authorization["\\']?)`;
...
const AUTHORIZATION_SETTER = new RegExp(`\\.\\s*(?:set|append|setHeader|appendHeader|setRequestHeader)\\s*\\(\\s*["']authorization["']\\s*,\\s*${AUTHORIZATION_VALUE}`, "i");
...
function hasAuthorizationCredential(text) {
  ...
  for (const expression of [AUTHORIZATION_ASSIGNMENT, AUTHORIZATION_PAIR, AUTHORIZATION_SETTER]) {
