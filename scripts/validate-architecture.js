...
function analyzeRepository(root = process.cwd()) {
  ...
  for (const edge of edges) {
    ...
    if (isForbiddenMobilePresentationReference(edge)) {
      findings.push(finding("MOBILE_PRESENTATION_SHORTCUT", edge, "Mobile presentation may use mobile-local modules and static type-only shared contracts, but must not reference implementation owners directly."));
    }
  }
...
function isForbiddenMobilePresentationReference(edge) {
  ...
  if (edge.target.startsWith("packages/contracts/") && edge.kind === "type") return false;
