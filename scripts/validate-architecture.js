...
function analyzeRepository(root = process.cwd()) {
  ...
  for (const edge of edges) {
    ...
    if (isForbiddenMobilePresentationReference(edge)) {
      findings.push(finding("MOBILE_PRESENTATION_SHORTCUT", edge, "Mobile presentation may use mobile-local modules and static type-only shared contracts, but must not depend directly on implementations."));
    }
  }
