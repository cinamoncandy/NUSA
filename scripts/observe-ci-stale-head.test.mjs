...
describe("CI stale-head observer", () => {
  ...
  it("fails closed when PR provenance is absent or ambiguous", () => {
    assert.deepEqual(
      buildObservation({ run: run({ pull_requests: [] }), currentHeadSha: HEAD_A, observedAt: 1_000 }),
      { status: "UNKNOWN", reason: "PULL_REQUEST_PROVENANCE_UNAVAILABLE" },
    );
    assert.deepEqual(
      buildObservation({ run: run({ pull_requests: [{ number: 1, head: { sha: HEAD_A } }, { number: 2, head: { sha: HEAD_A } }] }), currentHeadSha: HEAD_A, observedAt: 1_000 }),
      { status: "UNKNOWN", reason: "PULL_REQUEST_PROVENANCE_UNAVAILABLE" },
    );
  });
  ...
  it("fingerprints immutable source fields deterministically", () => {
    ...
    assert.notEqual(
      first.observation.sourceFingerprint,
      buildObservation({ run: run(), currentHeadSha: HEAD_B, observedAt: 1_000 }).observation.sourceFingerprint,
    );
  });
});
