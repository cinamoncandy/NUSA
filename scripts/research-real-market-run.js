...
const COST_STRESS_SCENARIOS = [
  { id: "BASE", label: "configured baseline", feeRate: BACKTEST_CONFIG.feeRate, spreadBps: BACKTEST_CONFIG.executionCosts.spreadBps, slippageBps: BACKTEST_CONFIG.executionCosts.slippageBps },
  { id: "MODERATE", label: "moderate cost inflation", feeRate: BACKTEST_CONFIG.feeRate * 1.5, spreadBps: BACKTEST_CONFIG.executionCosts.spreadBps * 2, slippageBps: BACKTEST_CONFIG.executionCosts.slippageBps * 2 },
  { id: "SEVERE", label: "severe cost inflation", feeRate: BACKTEST_CONFIG.feeRate * 2, spreadBps: BACKTEST_CONFIG.executionCosts.spreadBps * 4, slippageBps: BACKTEST_CONFIG.executionCosts.slippageBps * 6 }
];
...
function buildRsiRobustnessGrid() {
  const entries = RSI_PARAMETER_NEIGHBORHOOD.map((parameters) => ({ key: candidateIdFor(RSI_FAMILY_ID, parameters), parameters, neighbors: [] }));
  const by = (period, oversold) => entries.find((entry) => entry.parameters.period === period && entry.parameters.oversold === oversold);
  const periods = [7, 14, 21]; const bands = [25, 30, 35];
  for (let pi = 0; pi < periods.length; pi += 1) for (let bi = 0; bi < bands.length; bi += 1) {
    const entry = by(periods[pi], bands[bi]);
    for (const [dp, db] of [[-1, 0], [1, 0], [0, -1], [0, 1]]) { const neighbor = by(periods[pi + dp], bands[bi + db]); if (neighbor) entry.neighbors.push(neighbor.key); }
    entry.neighbors.sort(); Object.freeze(entry.neighbors); Object.freeze(entry);
  }
  return Object.freeze(entries);
}

function buildDonchianRobustnessGrid() {
  const entries = DONCHIAN_PARAMETER_NEIGHBORHOOD.map((parameters) => ({ key: candidateIdFor(DONCHIAN_FAMILY_ID, parameters), parameters, neighbors: [] }));
  for (let index = 0; index < entries.length; index += 1) {
    ...
    entries[index].neighbors.sort(); Object.freeze(entries[index].neighbors); Object.freeze(entries[index]);
  }
  return Object.freeze(entries);
}

const PBO_EVIDENCE_UNAVAILABLE_CODES = Object.freeze(["ZERO_RETURN_VARIANCE", "INSUFFICIENT_CANDIDATES", "INSUFFICIENT_OOS_EQUITY_POINTS", "INSUFFICIENT_OOS_RETURN_POINTS", "NO_SYMMETRIC_CSCV_PARTITION"]);
function isResearchRunPboEvidenceUnavailable(error) { return typeof error?.code === "string" && PBO_EVIDENCE_UNAVAILABLE_CODES.includes(error.code); }

function buildParameterRobustnessRequest({ candles, manifest, strategyFamily = SMA_FAMILY_ID }) {
  if (!Array.isArray(candles) || candles.length === 0) throw new Error("real parameter robustness requires canonical candles");
  if (manifest == null || typeof manifest.market !== "string" || typeof manifest.datasetId !== "string" || typeof manifest.contentSha256 !== "string") throw new Error("real parameter robustness requires a canonical dataset manifest");
  const common = { schemaVersion: 1, id: `real-run:${manifest.datasetId}:${strategyFamily}:parameter-robustness`, market: manifest.market, candles, minimumTrades: 0, execution: { initialCash: BACKTEST_CONFIG.initialCash, orderQuantity: BACKTEST_CONFIG.orderQuantity, executionCosts: { spreadBps: BACKTEST_CONFIG.executionCosts.spreadBps }, latencyCandles: 0, riskPolicy: {} }, evaluation: { mode: "BOTH", oosWindows: { trainingCandles: WALK_FORWARD_CONFIG.trainSize, testCandles: WALK_FORWARD_CONFIG.testSize, stepCandles: WALK_FORWARD_CONFIG.testSize } }, costConditions: COST_STRESS_SCENARIOS.map(({ id, feeRate, slippageBps }) => ({ name: id, feeRate, slippageBps })) };
  if (strategyFamily === SMA_FAMILY_ID) return { ...common, id: `real-run:${manifest.datasetId}:parameter-robustness`, referenceParameters: [{ source: "PRODUCTION_DEFAULT", shortWindow: 5, longWindow: 20 }, { source: "MANUAL_RESEARCH_REFERENCE", shortWindow: 2, longWindow: 8 }], neighborhood: { shortOffsets: [-2, -1, 0, 1, 2], longOffsets: [-5, -2, 0, 2, 5] } };
  if (strategyFamily === RSI_FAMILY_ID) return { ...common, strategyFamily, candidateGrid: buildRsiRobustnessGrid(), referenceParameters: [{ source: "PRODUCTION_DEFAULT", candidateKey: "rsi-14-30-70", parameters: { period: 14, oversold: 30, overbought: 70 } }, { source: "MANUAL_RESEARCH_REFERENCE", candidateKey: "rsi-7-25-75", parameters: { period: 7, oversold: 25, overbought: 75 } }] };
  if (strategyFamily === DONCHIAN_FAMILY_ID) return { ...common, strategyFamily, candidateGrid: buildDonchianRobustnessGrid(), referenceParameters: [{ source: "PRODUCTION_DEFAULT", candidateKey: "donchian-20", parameters: { channelPeriod: 20 } }, { source: "MANUAL_RESEARCH_REFERENCE", candidateKey: "donchian-55", parameters: { channelPeriod: 55 } }] };
...
async function main() {
  ...
  const deflatedSharpe = buildResearchRunDsrEvidence(leagueCandidates); let probabilityBacktestOverfitting; let pboUnavailableReason; try { probabilityBacktestOverfitting = buildResearchRunPboEvidence(leagueCandidates); } catch (error) { if (!isResearchRunPboEvidenceUnavailable(error)) throw error; pboUnavailableReason = error.code; }
  const league = buildResearchRunLeague(leagueCandidates.map((candidate) => ({ ...candidate, deflatedSharpe: deflatedSharpe.evidenceByCandidate.get(candidate.id), trialLedgerSummary: deflatedSharpe.trialLedgerSummary })), { generatedAt, ...(probabilityBacktestOverfitting == null ? {} : { probabilityBacktestOverfitting }), robustnessEvidence, hypothesis }); const factoryQualification = qualifyResearchFactoryRun(league);
  const oos = result.walkForwardResult.combinedOutOfSampleMetrics;
  console.log(JSON.stringify({ NOTICE: "REAL_MARKET_DATA_RESEARCH_TIER_ONLY -- not operational Paper evidence, does not authorize release", strategyFamily: definition.familyId, researchMarketSet: { version: RESEARCH_MARKET_SET_VERSION, selectionPolicy: "PREDECLARED_PUBLIC_HISTORY_AVAILABILITY_ONLY_NO_PERFORMANCE_SELECTION", markets: RESEARCH_MARKETS }, hypothesis: league.hypothesis ?? hypothesis, dataset: { datasetId: manifest.datasetId, market: manifest.market, interval: manifest.interval, candleCount: manifest.candleCount, startOpenTime: new Date(manifest.startOpenTime).toISOString(), endCloseTime: new Date(manifest.endCloseTime).toISOString(), contentSha256: manifest.contentSha256, sourceRequest: manifest.sourceRequest, completedBy: new Date(dataAsOf).toISOString(), freshness: { status: "FRESH", expectedLatestCloseTime: new Date(freshness.expectedLatestCloseTime).toISOString(), actualLatestCloseTime: new Date(freshness.actualLatestCloseTime).toISOString(), lagDays: freshness.lagDays } }, evidenceDatasets: marketDatasets.map((entry) => ({ datasetId: entry.manifest.datasetId, market: entry.manifest.market, interval: entry.manifest.interval, candleCount: entry.manifest.candleCount, startOpenTime: new Date(entry.manifest.startOpenTime).toISOString(), endCloseTime: new Date(entry.manifest.endCloseTime).toISOString(), contentSha256: entry.manifest.contentSha256, sourceRequest: entry.manifest.sourceRequest, freshnessLagDays: entry.freshness.lagDays })), windowCount: result.walkForwardResult.windows.length, parameterNeighborhood: { candidateSelectionCounts: result.walkForwardResult.candidateSelectionCounts, selectionChurn: result.walkForwardResult.stabilityDiagnostics.selectionChurn, selectionChurnRatio: result.walkForwardResult.stabilityDiagnostics.selectionChurnRatio, candidates: result.walkForwardResult.stabilityDiagnostics.candidates }, costStress: costStressEvidence, parameterRobustness: parameterRobustnessEvidence, outOfSample: { totalOosPoints: oos.totalOosPoints, totalOosClosedTrades: oos.totalOosClosedTrades, winRate: oos.winRate, totalReturn: oos.totalReturn, maximumDrawdown: oos.maximumDrawdown, profitFactor: oos.profitFactor ?? null, turnover: oos.turnover, totalTradingCost: oos.totalTradingCost, profitableWindowRatio: oos.profitableWindowRatio, benchmarkOutperformanceWindowRatio: oos.benchmarkOutperformanceWindowRatio }, searchOverfitting: { status: probabilityBacktestOverfitting == null ? "INSUFFICIENT" : "AVAILABLE", unavailableReason: pboUnavailableReason ?? null, strategyCount: probabilityBacktestOverfitting?.strategyCount ?? null, observationCount: probabilityBacktestOverfitting?.observationCount ?? null, partitions: probabilityBacktestOverfitting?.partitions ?? null, splitCount: probabilityBacktestOverfitting?.splitCount ?? null, probabilityBacktestOverfitting: probabilityBacktestOverfitting?.probabilityBacktestOverfitting ?? null, medianLogit: probabilityBacktestOverfitting?.medianLogit ?? null }, researchFactoryQualification: factoryQualification, researchRunProvenance: league.provenance, league: { evidenceMode: league.evidenceMode, evidenceReports: league.evidenceReport, reasons: league.reasons, probabilityBacktestOverfitting: league.standing.probabilityBacktestOverfitting ?? null, trialLedger: deflatedSharpe.trialLedgerSummary, allocationUnavailableReason: league.allocationUnavailableReason ?? null, standing: league.standing.entries.map((entry) => ({ id: entry.id, familyId: entry.familyId, rank: entry.rank ?? null, eligible: entry.eligible, leagueScore: entry.leagueScore ?? null, evidenceBreadth: entry.evidenceBreadth, deflatedSharpeProbability: entry.components.riskAdjusted ?? null, deflatedSharpeUnavailableReason: deflatedSharpe.unavailableReasons.get(entry.id) ?? null, regimeRobustness: entry.components.regimeRobustness ?? null, regimeRobustnessClass: entry.components.regimeRobustnessClass ?? null, reasons: entry.reasons })), researchAllocation: league.allocation == null ? null : league.allocation.entries.map((entry) => ({ id: entry.id, familyId: entry.familyId, researchWeight: entry.researchWeight })) }, warnings: result.warnings }, null, 2));
}

if (require.main === module) { main().catch((error) => { console.error("research real-market run failed:", error instanceof Error ? error.message : String(error)); process.exitCode = 1; }); }
module.exports = { RESEARCH_MARKET_SET_VERSION, RESEARCH_MARKETS, SMA_PARAMETER_NEIGHBORHOOD, RSI_PARAMETER_NEIGHBORHOOD, DONCHIAN_PARAMETER_NEIGHBORHOOD, researchStrategyFamily, fetchResearchCandles, researchCandleCount, buildParameterRobustnessRequest, buildResearchRunTimeline, isResearchRunPboEvidenceUnavailable };
