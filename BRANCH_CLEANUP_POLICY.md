...
### 방치된 972개 중 가족 단위 정리 (33개 검토 → 9개 삭제 가능)

이 저장소는 같은 작업을 새 이름으로 재시도합니다 — `-final`, `-final-v2`,
`-current-main-v3`, `-frozen`, `-clean`. 그래서 방치 브랜치가 하나의 작업을 중심으로
가족을 이룹니다. `node scripts/branch-cleanup-report.js --families`가 이를 묶고,
가족마다 "오래된 형제의 작업이 최신 형제 안에 이미 있는가"를 판정합니다.

| 판정 | 수량 | 처리 |
|------|------|------|
| CONTAINED — patch-id까지 최신에 포함 | 4개 | 삭제 |
| REBASE_COPY — 커밋 제목이 전부 최신에 있음 (베이스만 다름) | 5개 | 확인 후 삭제 |
| DISTINCT — 최신에 없는 커밋 보유 | 24개 | 사람이 판단 |

**"가족당 최신 하나만 남긴다"는 규칙은 여기서 틀립니다.** 그 규칙이 지웠을 33개 중
증명된 중복은 4개뿐이고, 24개는 최신 형제에 없는 커밋을 갖고 있습니다. 그 중에는
커밋 1개 차이도 있는데, 확인해 보면 같은 버그를 **다르게 고친 재작성**인 경우가
있었습니다 (`fix/cloud-risk-gating`은 결정 필터링, `-v2`는 게이트웨이 배선).
