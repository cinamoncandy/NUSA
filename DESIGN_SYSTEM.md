...
## Trading UX semantic contract

The trading surface additionally follows these rules:

- Execution mode is global state, not page-local decoration. PAPER, LIVE, and NO AUTHORITY must never become visually ambiguous.
- Information hierarchy is account/execution state → risk state → actionable decision evidence → positions/orders → performance/history.
- Trading-critical actions require visible validation and acknowledgement using runtime truth; presentation code must never invent execution, risk, signal, confidence, PnL, or freshness values.
- Stale, disconnected, rejected, disabled, loading, empty, and success states use canonical semantic tokens/components rather than one-off colors.
- Responsive layouts may hide or collapse secondary columns, but the critical decision fields must remain available and the underlying data ownership must not change.
- Touch targets for primary trading actions should remain usable at narrow widths, and order confirmation should remain explicit about PAPER execution where applicable.
