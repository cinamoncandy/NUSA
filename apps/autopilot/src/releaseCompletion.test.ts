...
describe("evaluateReleaseCompletion", () => {
  ...
  it("fails closed when the outer canonical Release workflow did not succeed", () => {
    assert.equal(evaluate({ ...complete(), outerWorkflowConclusion: "failure" }), "NOT_RELEASED");
    assert.equal(evaluate({ ...complete(), outerWorkflowConclusion: null }), "NOT_RELEASED");
  });
