...
export function validateEvolutionOpportunity(value: unknown): EvolutionOpportunity {
  ...
  if (opportunity.canonicalOwner !== undefined && (typeof opportunity.canonicalOwner !== "string" || !OWNER.test(opportunity.canonicalOwner))) throw new Error("EVOLVE_OPPORTUNITY_CANONICAL_OWNER_INVALID");
  if (opportunity.conflictKeys !== undefined) {
    ...
    for (const key of opportunity.conflictKeys) {
      if (typeof key !== "string" || !CONFLICT_KEY.test(key) || seen.has(key)) throw new Error("EVOLVE_OPPORTUNITY_CONFLICT_KEYS_INVALID");
      seen.add(key);
    }
  }
