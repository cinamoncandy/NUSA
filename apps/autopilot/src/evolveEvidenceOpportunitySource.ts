...
function validateObservation(value: unknown): asserts value is WorkflowFailureEvidence {
  if (!isRecord(value)) throw new Error("EVOLVE_WORKFLOW_EVIDENCE_INVALID");
  if (typeof value.workflowName !== "string" || !NAME.test(value.workflowName)) throw new Error("EVOLVE_WORKFLOW_EVIDENCE_NAME_INVALID");
  if (typeof value.runId !== "number" || !Number.isSafeInteger(value.runId) || value.runId <= 0) throw new Error("EVOLVE_WORKFLOW_EVIDENCE_RUN_ID_INVALID");
  if (typeof value.headSha !== "string" || !SHA.test(value.headSha)) throw new Error("EVOLVE_WORKFLOW_EVIDENCE_SHA_INVALID");
  if (typeof value.conclusion !== "string" || !CONCLUSIONS.has(value.conclusion as WorkflowFailureConclusion)) throw new Error("EVOLVE_WORKFLOW_EVIDENCE_CONCLUSION_INVALID");
  if (typeof value.completedAt !== "string" || !Number.isFinite(Date.parse(value.completedAt))) throw new Error("EVOLVE_WORKFLOW_EVIDENCE_COMPLETED_AT_INVALID");
}
...
export function deriveWorkflowFailureOpportunities(input: WorkflowOpportunitySourceInput): readonly EvolutionOpportunity[] {
  if (!isRecord(input)) throw new Error("EVOLVE_WORKFLOW_SOURCE_INPUT_INVALID");
  if (!Array.isArray(input.observations)) throw new Error("EVOLVE_WORKFLOW_SOURCE_OBSERVATIONS_INVALID");
  if (typeof input.observedAt !== "string") throw new Error("EVOLVE_WORKFLOW_SOURCE_OBSERVED_AT_INVALID");
