...
describe("persistent control-plane HOLD", () => {
  ...
  it("dispatched execution completes once and permanently suppresses the same dedupe identity", async () => {
    ...
    assert.equal((await coordinator.fetch(request("/handoff", { dedupeKey: identity.dedupeKey, executionId: identity.executionId, now: 105 }))).status, 200);
  ...
  it("completion fails closed before dispatch or for stale identity", async () => {
    ...
    assert.equal((await coordinator.fetch(request("/handoff", { dedupeKey: identity.dedupeKey, executionId: identity.executionId, now: 115 }))).status, 200);
