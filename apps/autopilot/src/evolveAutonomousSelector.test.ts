...
test("never selects beyond scheduler remaining capacity", () => {
  const input = baseInput();
  const result = selectNonConflictingEvolutionOpportunities({
    ...input,
    schedulePolicy: { ...input.schedulePolicy, maxConcurrent: 3 },
...
test("rejects malformed or duplicate active conflict keys", () => {
  const input = baseInput();
  const common = {
    ...input,
    schedulePolicy: { ...input.schedulePolicy, maxConcurrent: 3 },
  ...
  assert.throws(() => selectNonConflictingEvolutionOpportunities({ ...common, activeConflictKeys: ["module:busy", "module:busy"] }), /EVOLVE_SELECTION_ACTIVE_CONFLICT_KEYS_INVALID/);
  assert.throws(() => selectNonConflictingEvolutionOpportunities({ ...common, activeConflictKeys: ["bad key"] }), /EVOLVE_SELECTION_ACTIVE_CONFLICT_KEYS_INVALID/);
});
