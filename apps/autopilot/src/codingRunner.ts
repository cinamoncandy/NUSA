      ...
      if (waitUntil !== null && current < waitUntil) {
        ...
        if (githubToken) {
          try {
            ...
            const lastFailure = error instanceof Error ? error.message : "GITHUB_MODELS_CODING_FALLBACK_FAILED";
            return {
      ...
      if (rateLimitReason) {
        ...
        if (githubToken) {
          try {
            ...
            const lastFailure = fallbackError instanceof Error ? fallbackError.message : "GITHUB_MODELS_CODING_FALLBACK_FAILED";
            return {
              status: "BLOCKED_RATE_LIMIT",
