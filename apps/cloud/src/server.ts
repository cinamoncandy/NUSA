...
import { handleMobileBootstrapHttp, handleMobileBootstrapIssueHttp, handleMobileMeHttp, handleMobileSessionRefreshHttp, handleMobileSessionRevokeHttp } from "./mobileSessionHttp";
...
export function startCloudDashboardServer(options: CloudDashboardServerOptions): CloudDashboardServerHandle {
  ...
  const server: Server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      if (mobileSessionService != null && req.url === "/v1/mobile/bootstrap") { respond("mobile_bootstrap", handleMobileBootstrapHttp(dashboardRequest, { sessionService: mobileSessionService })); return; }
      if (mobileSessionService != null && req.url === "/api/operator/mobile-bootstrap") { respond("mobile_bootstrap_issue", handleMobileBootstrapIssueHttp(dashboardRequest, { sessionService: mobileSessionService, legacyTokenVerifier: options.tokenVerifier, userAccessRepository })); return; }
      if (mobileSessionService != null && req.url === "/v1/mobile/session/refresh") { respond("mobile_session_refresh", handleMobileSessionRefreshHttp(dashboardRequest, { sessionService: mobileSessionService })); return; }
      if (mobileSessionService != null && req.url === "/v1/mobile/session/revoke") { respond("mobile_session_revoke", handleMobileSessionRevokeHttp(dashboardRequest, { sessionService: mobileSessionService })); return; }
      if (mobileSessionService != null && req.url === "/v1/mobile/me") { respond("mobile_me", handleMobileMeHttp(dashboardRequest, { sessionService: mobileSessionService })); return; }
