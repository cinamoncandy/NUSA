...
export interface CloudReadinessSnapshot {
  readonly ok: boolean;
  readonly checks: Readonly<{ database: boolean; migrations: boolean; dashboardPersistence: boolean; runtimeRecovery: boolean }>;
}
...
async function readJsonBody(req: IncomingMessage, maximumBytes = 16 * 1024): Promise<unknown> {
  ...
  for await (const chunk of req) {
    ...
    if (bytes > maximumBytes) throw new HttpBodyError(413, "REQUEST_TOO_LARGE");
    chunks.push(value);
  }
  if (bytes === 0) throw new HttpBodyError(400, "INVALID_JSON");
  try { return JSON.parse(Buffer.concat(chunks).toString("utf8")); }
  catch { throw new HttpBodyError(400, "INVALID_JSON"); }
}

/**
 * Localhost-by-default Cloud transport. Read projections use `dashboard:read`; the single
 * `/api/paper-orders` mutation route requires explicit `paper:trade` and is hard-bound to PAPER.
 * No LIVE execution, transfer, withdrawal, production mutation, or permissive fallback exists here.
 */
    ...
    void (async () => {
      ...
      try {
        if (req.url === "/health") {
          ...
          write(res, dashboardJsonResponse(200, { ok: true, observedAt: new Date().toISOString() })); return;
        }
        ...
        if (req.url === "/ready") {
          ...
          try {
            ...
            const readiness = unavailable(); operationalLog("ERROR", "cloud.readiness", requestId, { ok: readiness.ok, checks: readiness.checks }); write(res, dashboardJsonResponse(503, readiness));
          }
          return;
        }
        if (req.url === "/api/paper-orders") {
          ...
          if ((req.method ?? "GET").toUpperCase() === "POST") {
            try { body = await readJsonBody(req); }
              ...
              if (error instanceof HttpBodyError) { write(res, dashboardJsonResponse(error.status, { error: error.message })); return; }
              write(res, dashboardJsonResponse(400, { error: "INVALID_JSON" })); return;
            }
          }
        ...
        if (req.url === "/api/paper-operations") {
          write(res, handlePersonalPaperOperationsHttp(dashboardRequest, { tokenVerifier: options.tokenVerifier, loadSnapshot: options.loadPaperOperations ?? (() => { throw new Error("PAPER operations snapshot not configured"); }) }));
          return;
        }
