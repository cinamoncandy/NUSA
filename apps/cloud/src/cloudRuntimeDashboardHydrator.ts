 ...
 *
 * Intelligence and portfolio composition run through their canonical v10 entrypoints.
...
export class CloudRuntimeDashboardHydrator {
  ...
  public hydrate(provider: CloudDashboardStateProvider, observations: readonly IntelligenceObservation[] = []): void {
    try {
      ...
      const intelligenceInput: readonly IntelligenceObservation[] = validObservations.length > 0
        ? validObservations
        : [{
      ...
      const intelligence = runIntelligenceEngineV10({ now, observations: intelligenceInput }).fused;
