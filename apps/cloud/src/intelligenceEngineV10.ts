...
export interface IntelligenceEngineV10Input {
  ...
  /** Optional because the canonical Cloud ticker path currently has fusion evidence before regime features exist. */
...
export function runIntelligenceEngineV10(input: IntelligenceEngineV10Input): IntelligenceEngineV10Output {
  ...
  if (input.regimeFeatures != null && input.regimeFeatures.observedAt > input.now) throw new Error("regime features cannot come from the future");
