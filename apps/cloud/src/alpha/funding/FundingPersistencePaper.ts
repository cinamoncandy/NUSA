...
export const replayFundingPersistencePaperLedger = (ledger: FundingPersistencePaperLedger): { readonly orders: readonly FundingPersistencePaperOrderSnapshot[]; readonly replayHash: string } => {
  ...
  for (const event of ledger.events) {
    ...
    if (event.type === "ORDER_CREATED") {
      ...
      orders.set(event.orderId, Object.freeze({ orderId: event.orderId, state: "NEW", requestedQuantity: event.quantity ?? 0, filledQuantity: 0, averageFillPrice: null, updatedAt: event.occurredAt }));
      continue;
    }
    ...
    if (event.type === "ORDER_PARTIALLY_FILLED" || event.type === "ORDER_FILLED") {
      ...
      const cost = (averageFillPrice ?? 0) * filledQuantity + price * quantity;
      filledQuantity = round(filledQuantity + quantity);
      averageFillPrice = round(cost / filledQuantity);
    ...
    orders.set(event.orderId, Object.freeze({ ...current, state: nextState[event.type], filledQuantity, averageFillPrice, updatedAt: event.occurredAt }));
  }
...
export const appendFundingPersistencePaperEvent = (ledger: FundingPersistencePaperLedger, input: AppendFundingPersistencePaperEventInput): FundingPersistencePaperLedger => {
  ...
  const payload = { eventId: input.eventId.trim(), sequence: ledger.events.length + 1, orderId: input.orderId.trim(), type: input.type, occurredAt: input.occurredAt, quantity: quantity === null ? null : round(quantity), price: price === null ? null : round(price), fee: round(fee), slippageCost: round(slippageCost), realizedPnl: round(realizedPnl), reason: input.reason.trim(), previousHash: ledger.headHash };
  const event = Object.freeze({ ...payload, contentHash: hash(payload) });
  ...
  const result = Object.freeze({ ...resultPayload, contentHash: hash(resultPayload) });
...
export const createFundingPersistencePaperDailyReport = (ledger: FundingPersistencePaperLedger, reportDate: string): FundingPersistencePaperDailyReport => {
  ...
  const payload = { reportDate, market: ledger.market, createdOrders: count("ORDER_CREATED"), filledOrders: count("ORDER_FILLED"), rejectedOrders: count("ORDER_REJECTED"), cancelledOrders: count("ORDER_CANCELLED"), closedPositions: count("POSITION_CLOSED"), grossRealizedPnl: round(gross), fees: round(fees), slippageCost: round(slippage), netRealizedPnl: round(gross - fees - slippage), tradedQuantity: round(events.reduce((sum, event) => sum + (event.quantity ?? 0), 0)), ledgerHeadHash: ledger.headHash };
  return Object.freeze({ ...payload, contentHash: hash(payload) });
};
