...
export interface AiRoutingInput {
  readonly taskType: AiCapability;
  readonly riskLevel: AiRiskLevel;
  readonly estimatedComplexity: AiComplexity;
  readonly contextSize: number;
  readonly requiredCapabilities: readonly AiCapability[];
  readonly latencySensitivity: AiLatencySensitivity;
  readonly costSensitivity: AiCostSensitivity;
  readonly previousFailures: readonly AiModelTier[];
  readonly confidence: number;
}

export interface AiRoutingDecision {
  readonly selectedModel: AiModelTier | "HUMAN";
  readonly selectedProvider: string | null;
  readonly confidence: number;
  readonly reasonCode: AiRouteReason;
  readonly fallbackModel: AiModelTier | "HUMAN";
...
function allCapabilities(entry: AiModelRegistryEntry, required: readonly AiCapability[]): boolean {
...
function decision(model: AiModelRegistryEntry | null, confidence: number, reasonCode: AiRouteReason, fallback: AiModelTier | "HUMAN", escalationRequired: boolean): AiRoutingDecision {
  return Object.freeze({
    selectedModel: model?.modelId ?? "HUMAN",
    selectedProvider: model?.providerId ?? null,
    confidence,
    reasonCode,
    fallbackModel: fallback,
    timeoutMs: model == null ? 0 : RANK[model.modelId] >= 3 ? 30_000 : RANK[model.modelId] >= 2 ? 15_000 : 5_000,
    maxRetries: model == null ? 0 : 1,
    escalationRequired,
...
export function validateAiModelRegistry(entries: readonly AiModelRegistryEntry[]): readonly AiModelRegistryEntry[] {
  ...
  for (const entry of entries) {
    ...
    const identity = `${entry.modelId}:${entry.providerId}`;
    if (identities.has(identity)) throw new Error("duplicate AI model registry entry");
...
export function routeAiModel(input: AiRoutingInput, registry: readonly AiModelRegistryEntry[]): AiRoutingDecision {
  ...
  const candidates = validateAiModelRegistry(registry)
    .filter((entry) => entry.enabled && entry.stability === "STABLE" && entry.availability === "AVAILABLE")
    .filter((entry) => RANK[entry.modelId] >= minimumRank && entry.maxContextBytes >= input.contextSize && allCapabilities(entry, required))
    .filter((entry) => !input.previousFailures.includes(entry.modelId))
    .sort((a, b) => {
      const rank = RANK[a.modelId] - RANK[b.modelId];
      if (rank !== 0) return rank;
      const cost = COST[a.costClass] - COST[b.costClass];
      if (cost !== 0) return cost;
      const latency = LATENCY[a.latencyClass] - LATENCY[b.latencyClass];
      if (latency !== 0) return latency;
      return a.providerId.localeCompare(b.providerId);
    });

  const selected = candidates[0] ?? null;
  if (selected == null) return decision(null, 0, "NO_SAFE_ROUTE", "HUMAN", true);
  const fallback = candidates.find((candidate) => candidate.modelId !== selected.modelId)?.modelId ?? "HUMAN";
  const reason: AiRouteReason = input.confidence < 0.5 ? "LOW_CONFIDENCE_ESCALATION"
    : RISK_FLOOR[input.riskLevel] > COMPLEXITY_FLOOR[input.estimatedComplexity] ? "RISK_ESCALATION"
      : minimumRank > 0 ? "COMPLEXITY_ESCALATION" : "CAPABILITY_MATCH";
