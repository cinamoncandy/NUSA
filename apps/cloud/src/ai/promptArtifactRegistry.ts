...
const roleInstructions: Readonly<Record<AiAgentRole, string>> = Object.freeze({
  ...
  STRATEGY_PROPOSER: "Using only the supplied verified evidence payloads and prior bounded analysis, propose at most a PAPER/Research candidate or no action. Cite supplied evidence IDs and state assumptions and uncertainty. rawProbability must be a number within [0,1] estimating exactly this event: for the same market as the cited verified UPBIT_PUBLIC_TICKER anchor, the first verified public ticker observed by NUSA at or after five minutes from this proposer prediction being recorded has a price strictly greater than that anchor price. This rawProbability is an uncalibrated model estimate only; it is not a verified success probability, performance guarantee, execution signal, risk input, or authority. Emit only the required structured JSON. Never place or request orders, transfers, credentials, production mutation, LIVE execution, risk changes, risk bypass, or safety-control release.",
