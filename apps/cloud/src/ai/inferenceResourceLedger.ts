...
export class InferenceResourceLedger implements AiInferenceResourceController {
  ...
  public reserveAttempt(request: ModelRequest, at: number): boolean {
    ...
    if (prior != null) {
      ...
      // Admission is deliberately single-use. Exact replay remains non-destructive, but returning
      // true here would grant a second provider side effect for the same request/attempt identity.
      return false;
    }
  ...
  public snapshot(at: number): AiInferenceResourceSnapshot {
    ...
    const completed = attemptsEvidence.filter((item) => item.result === "COMPLETED");
    const fullyAccounted = completed.filter((item) => item.actualInputTokens != null && item.actualOutputTokens != null && item.actualTotalTokens != null);
    const withAnyUsage = completed.filter((item) => item.actualInputTokens != null || item.actualOutputTokens != null || item.actualTotalTokens != null);
    const usageAccountingStatus: AiInferenceUsageAccountingStatus = completed.length === 0 || withAnyUsage.length === 0
      ? "UNAVAILABLE"
      : fullyAccounted.length === completed.length
        ? "VERIFIED"
        : "PARTIAL";
    const aggregatesTrusted = usageAccountingStatus === "VERIFIED";
    return Object.freeze({
      schemaVersion: 1,
      ...
      actualInputTokens: aggregatesTrusted ? this.actualInputTokensCount : null,
      actualOutputTokens: aggregatesTrusted ? this.actualOutputTokensCount : null,
      actualTotalTokens: aggregatesTrusted ? this.actualTotalTokensCount : null,
      elapsedMs,
