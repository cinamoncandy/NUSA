...
export class MultiAgentOrchestrator {
  ...
  public async run(input: AiOrchestrationInput): Promise<AiOrchestrationResult> {
    ...
    for (const role of roles) {
      ...
      const request: ModelRequest = Object.freeze({ requestId: `${input.orchestrationRunId}:request:${role}`, role, providerId: this.provider.providerId, modelVersionId, promptArtifactId: artifact.promptArtifactId, promptArtifactVersion: artifact.version, promptArtifactDigest: artifact.digest, instructions: artifact.instructions, contextHash: bundle.context.contextHash, inputHash: aiSha256(modelInput), input: modelInput, maxOutputBytes: 32_768, maxTokens: 4_096, timeoutMs: 60_000, attempt: 0 });
