...
function replayProjection(episode: ReturnType<GovernedOutcomeAttributionEngine["analyze"]>): AiAttributionReplayProjection {
  return Object.freeze({
    strength: episode.strength,
    primaryCause: episode.primaryCause,
    candidateCauses: Object.freeze(episode.candidates.map((item) => item.cause)),
...
function replayResult(args: {
  ...
  return Object.freeze({
    ...
    status: args.status,
    changedDimension: args.changedDimension,
    baseline: args.baseline,
    ablation: args.ablation,
    resourceBefore: args.resourceBefore,
    resourceAfter: args.resourceAfter,
    reasonCodes: Object.freeze([...new Set(args.reasonCodes)].sort()),
    liveAuthority: "NONE",
...
/**
 * One baseline + one exactly-one-dimension ablation under an externally owned WO-AI-006
 * resource controller. No nested inference budget is created here. Replay output is
 * diagnostic-only and cannot become realized-market learning credit.
 */
export class GovernedAttributionReplayEvaluator {
  ...
  public async run(
    ...
    const originalHistory = historicalIdentity(request);
    ...
    try {
      ...
      return replayResult({
        ...
        ablation: null,
        resourceBefore,
        resourceAfter: resources.snapshot(this.now()),
        reasonCodes: ["INVALID_ABLATION_DIMENSION_COUNT"]
      });
    }
    if (resourceBefore.health !== "HEALTHY") {
      return replayResult({
        ...
        reasonCodes: ["RESOURCE_NOT_HEALTHY_BEFORE_REPLAY"]
      });
    }

    const baselineRequest = Object.freeze({ ...request, episodeId: `${experimentId}:baseline`, signals: request.signals });
    ...
    try {
      ...
      return replayResult({
        experimentId,
        status: after.health === "HEALTHY" ? "FAILED" : "RESOURCE_BLOCKED",
        changedDimension,
        baseline: null,
        ablation: null,
        resourceBefore,
        resourceAfter: after,
        reasonCodes: [after.health === "HEALTHY" ? "BASELINE_REPLAY_FAILED" : "BASELINE_REPLAY_RESOURCE_BLOCKED"]
      });
    }
    if (historicalIdentity(request) !== originalHistory || aiSha256(baselineSignals) !== aiSha256(request.signals)) {
      return replayResult({
        experimentId,
        status: "UNVERIFIED",
        changedDimension,
        ...
        resourceBefore,
        resourceAfter: resources.snapshot(this.now()),
        reasonCodes: ["BASELINE_REPLAY_MUTATED_IMMUTABLE_HISTORY"]
      });
    }
    const baselineEpisode = this.engine.analyze(Object.freeze({ ...baselineRequest, signals: baselineSignals }));
    ...
    if (afterBaseline.health !== "HEALTHY") {
      return replayResult({
        ...
        status: "RESOURCE_BLOCKED",
        changedDimension,
        baseline: replayProjection(baselineEpisode),
        ablation: null,
        resourceBefore,
        resourceAfter: afterBaseline,
        reasonCodes: ["RESOURCE_EXHAUSTED_BEFORE_ABLATION"]
      });
    }

    const ablationRequest = Object.freeze({ ...request, episodeId: `${experimentId}:ablation`, signals: ablatedSignals });
    ...
    try {
      ...
      return replayResult({
        ...
        resourceBefore,
        resourceAfter: after,
        reasonCodes: [after.health === "HEALTHY" ? "ABLATION_REPLAY_FAILED" : "ABLATION_REPLAY_RESOURCE_BLOCKED"]
      });
    }
    if (historicalIdentity(request) !== originalHistory || aiSha256(replayedAblatedSignals) !== aiSha256(ablatedSignals)) {
      return replayResult({
        experimentId,
        status: "UNVERIFIED",
        changedDimension,
        ...
        resourceBefore,
        resourceAfter: resources.snapshot(this.now()),
        reasonCodes: ["ABLATION_REPLAY_MUTATED_IMMUTABLE_HISTORY"]
      });
    }
    const ablationEpisode = this.engine.analyze(Object.freeze({ ...ablationRequest, signals: replayedAblatedSignals }));
    ...
    if (resourceAfter.health !== "HEALTHY") {
      return replayResult({
        ...
        changedDimension,
        baseline: replayProjection(baselineEpisode),
        ablation: replayProjection(ablationEpisode),
        resourceBefore,
        resourceAfter,
        reasonCodes: ["RESOURCE_NOT_HEALTHY_AFTER_REPLAY"]
      });
    }
    return replayResult({
      experimentId,
      ...
      resourceBefore,
      resourceAfter,
      reasonCodes: ["ONE_DIMENSION_ABLATION_COMPLETED"]
    });
  }
}
...
function optionalQualityDelta(before: number | null, after: number | null): number | null {
  return before == null || after == null ? null : after - before;
}

function compareOptionalQuality(name: string, before: number | null, after: number | null, reasons: Set<string>): { regression: boolean; improvement: boolean } {
  if (before != null && after == null) {
    reasons.add(`${name}_MISSING_AFTER`);
  ...
  if (after < before) {
    reasons.add(`${name}_REGRESSION`);
    return { regression: true, improvement: false };
  }
  return { regression: false, improvement: after > before };
}

/** Quality gains never compensate for safety/resource regressions. */
export function evaluateAiAttributionNetBenefit(policyValue: AiAttributionNetBenefitPolicy, input: AiAttributionNetBenefitInput): AiAttributionNetBenefitResult {
  ...
  const known = compareOptionalQuality("KNOWN_CAUSE_ACCURACY", input.beforeQuality.knownCauseAccuracy, input.afterQuality.knownCauseAccuracy, reasons);
  const ambiguity = compareOptionalQuality("AMBIGUITY_ABSTENTION_ACCURACY", input.beforeQuality.ambiguityAbstentionAccuracy, input.afterQuality.ambiguityAbstentionAccuracy, reasons);
  ...
  if (falseAttributionRateDelta > 0) {
    reasons.add("FALSE_ATTRIBUTION_RATE_REGRESSION");
    qualityRegression = true;
  } else if (falseAttributionRateDelta < 0) {
  ...
  if (providerCallDelta > policy.maxProviderCallIncrease) reasons.add("PROVIDER_CALL_REGRESSION");
  if (attemptDelta > policy.maxAttemptIncrease) reasons.add("ATTEMPT_REGRESSION");
  if (tokenDelta > policy.maxTokenIncrease) reasons.add("TOKEN_REGRESSION");
  if (latencyDeltaMs > policy.maxLatencyIncreaseMs) reasons.add("LATENCY_REGRESSION");
  if (failureRateDelta > policy.maxFailureRateIncrease) reasons.add("FAILURE_RATE_REGRESSION");
  if (safetyRegressionCount > 0) reasons.add("SAFETY_REGRESSION");
  if (policy.requireQualityImprovement && !qualityImprovement) reasons.add("NO_MEASURED_QUALITY_IMPROVEMENT");
  if (!qualityRegression && reasons.size === 0) reasons.add("NET_BENEFIT_VERIFIED");
  const failureReasons = [...reasons].filter((code) => code !== "NET_BENEFIT_VERIFIED");

  return Object.freeze({
    schemaVersion: 1,
    status: qualityRegression || failureReasons.length > 0 ? "FAIL" : "PASS",
    knownCauseAccuracyDelta: optionalQualityDelta(input.beforeQuality.knownCauseAccuracy, input.afterQuality.knownCauseAccuracy),
    ambiguityAbstentionAccuracyDelta: optionalQualityDelta(input.beforeQuality.ambiguityAbstentionAccuracy, input.afterQuality.ambiguityAbstentionAccuracy),
...
/** Admitted attempts require verified token accounting; unavailable usage never masquerades as zero. */
