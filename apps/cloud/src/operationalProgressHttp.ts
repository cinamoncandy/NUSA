...
/** Read-only Supervisor progress projection. No mutation or authority-escalation path exists here. */
export function handleOperationalProgressHttp(
  request: DashboardHttpRequest,
  dependencies: OperationalProgressHttpDependencies
  ...
  try {
    ...
    if (snapshot.authority !== "READ_ONLY" || snapshot.scope !== "OPERATIONAL_EVIDENCE_ONLY") {
      return dashboardJsonResponse(503, { error: "OPERATIONAL_PROGRESS_AUTHORITY_VIOLATION" });
    }
