...
export function validateResearchRunManifest(manifest: ResearchRunManifest): void {
  const expected = createResearchRunManifest({ ...manifest, config: { configChecksum: manifest.configChecksum }, result: { resultChecksum: manifest.resultChecksum } });
  if (expected.parameterChecksum !== manifest.parameterChecksum || expected.configChecksum !== manifest.configChecksum || expected.resultChecksum !== manifest.resultChecksum || expected.manifestChecksum !== manifest.manifestChecksum) throw new Error("research manifest checksum mismatch");
}
