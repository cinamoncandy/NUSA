...
export interface AlphaEvidenceRegistrySnapshot {
  readonly alphas: readonly AlphaRecord[];
  readonly evidence: readonly EvidenceRecord[];
  readonly links: readonly AlphaEvidenceLink[];
...
const unique = (values: readonly string[], label: string): readonly string[] => {
  ...
  if (normalized.some((value) => value.length === 0)) throw new Error(`${label} cannot contain empty values`);
...
export const createAlphaRecord = (input: Omit<AlphaRecord, "contentHash">): AlphaRecord => {
  ...
  if (!ALPHA_STATUSES.has(input.status)) throw new Error("invalid alpha status");
  if (!EVIDENCE_GRADES.has(input.evidenceGrade)) throw new Error("invalid evidence grade");
  ...
  const payload = {
    ...input,
    alphaId: input.alphaId.trim(),
    name: input.name.trim(),
    category: input.category.trim(),
    owner: input.owner.trim(),
    horizon: input.horizon.trim(),
    riskClass: input.riskClass.trim(),
    markets: unique(input.markets, "markets"),
    dependencyAlphaIds: unique(input.dependencyAlphaIds, "dependencyAlphaIds")
  };
...
export const buildAlphaEvidenceRegistry = (input: Omit<AlphaEvidenceRegistrySnapshot, "snapshotHash">): AlphaEvidenceRegistrySnapshot => {
  ...
  const alphaVersions = new Map<string, number[]>();
  for (const alpha of input.alphas) {
    const key = `${alpha.alphaId}@${alpha.version}`;
    if (alphaKeys.has(key)) throw new Error(`duplicate alpha ${key}`);
    alphaKeys.add(key);
    alphaVersions.set(alpha.alphaId, [...(alphaVersions.get(alpha.alphaId) ?? []), alpha.version]);
    if (parseTime(alpha.createdAt, `${key}.createdAt`) > generatedAt) throw new Error(`${key} cannot be created in the future`);
    if (alpha.contentHash !== createAlphaRecord({ ...alpha, contentHash: undefined } as never).contentHash) throw new Error(`${key} content hash mismatch`);
  }
  for (const alpha of input.alphas) {
    ...
    for (const dependency of alpha.dependencyAlphaIds) if (!alphaVersions.has(dependency)) throw new Error(`missing dependency alpha ${dependency}`);
  }
  const evidenceIds = new Set<string>();
  for (const item of input.evidence) {
    if (evidenceIds.has(item.evidenceId)) throw new Error(`duplicate evidence ${item.evidenceId}`);
  ...
  for (const link of input.links) {
    if (linkIds.has(link.linkId)) throw new Error(`duplicate link ${link.linkId}`);
  ...
  const alphas = Object.freeze([...input.alphas]);
  const evidence = Object.freeze([...input.evidence]);
  const links = Object.freeze([...input.links]);
  const payload = { alphas, evidence, links, generatedAt: input.generatedAt };
