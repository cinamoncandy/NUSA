...
const canon=(e:StrategyGovernanceEvent)=>JSON.stringify({familyId:e.familyId,lifecycle:e.lifecycle,occurredAt:e.occurredAt,reason:e.reason,strategyId:e.strategyId,type:e.type,version:e.version,...(e.approval?{approval:{actorType:e.approval.actorType,approvalReference:e.approval.approvalReference,approvedAt:e.approval.approvedAt,decisionFingerprint:e.approval.decisionFingerprint}}:{})});
...
export function replayStrategyGovernanceLedger(records:readonly StrategyGovernanceLedgerRecord[]):StrategyGovernanceState{const life=new Map<string,StrategyLifecycle>(), champions=new Map<string,string>();let previous=genesis,last=-1;for(let i=0;i<records.length;i++){const r=records[i];validateApproval(r.event);if(r.sequence!==i+1||r.previousHash!==previous||r.hash!==hash(r.sequence,r.previousHash,r.event))throw new Error("governance ledger integrity violation");if(r.event.occurredAt<last)throw new Error("governance ledger timestamp regression");const k=key(r.event),old=life.get(k);validateLifecycleTransition(r.event,old);validatePrivilegedTransition(r.event,old);if(old&&terminal.has(old)&&r.event.lifecycle!==old)throw new Error("terminal lifecycle cannot resume");if(r.event.type==="CHAMPION_PROMOTED"){const incumbent=champions.get(r.event.familyId);if(incumbent&&incumbent!==k)life.set(incumbent,"CHALLENGER");champions.set(r.event.familyId,k);}if(r.event.type==="STRATEGY_ROLLED_BACK"||r.event.type==="STRATEGY_RETIRED")if(champions.get(r.event.familyId)===k)champions.delete(r.event.familyId);life.set(k,r.event.lifecycle);previous=r.hash;last=r.event.occurredAt;}return Object.freeze({lifecycles:life,champions,hash:previous});}
