(() => {
  ...
  const runtimeWindow = typeof window === "object" && window ? window : globalThis;
  ...
  for (const step of steps) {
    ...
    label.className = "decision-flow-rail__label";
    label.textContent = step.label;
    const value = document.createElement("span");
    value.className = "decision-flow-rail__value";
    value.dataset.decisionValue = step.key;
    value.textContent = "확인 중";
    const freshness = document.createElement("span");
    freshness.className = "decision-flow-rail__freshness";
    freshness.dataset.decisionFreshness = step.key;
    freshness.textContent = "상태 확인 중";
    button.append(label, value, freshness);
  ...
  const update = () => {
    ...
    setFreshness("risk", "실행 권한 없음", "restricted");
    ...
    let activeKey = null;
    if (activePage === "market") { activeKey = "observe"; requestedStepKey = null; }
    else if (activePage === "positions" || activePage === "dashboard") { activeKey = "assess"; requestedStepKey = null; }
    else if (activePage === "orders") activeKey = orderStepKeys.has(requestedStepKey) ? requestedStepKey : "action";
    else requestedStepKey = null;
  ...
  const queueUpdate = () => {
    if (queued) return;
    queued = true;
    Promise.resolve().then(() => { queued = false; update(); });
  };
  const MutationObserverCtor = runtimeWindow.MutationObserver;
  if (typeof MutationObserverCtor === "function") {
    const observer = new MutationObserverCtor((mutations) => {
      if (mutations.length > 0 && mutations.every((mutation) => mutation.target?.closest?.("[data-decision-flow-rail]"))) return;
      queueUpdate();
    });
