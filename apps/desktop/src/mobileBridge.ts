...
export interface MobileBridgeOptions { readonly port: number; readonly getStatus: () => MobileMonitorStatus; readonly getAccount: () => Readonly<Record<string, unknown>>; readonly getOpenOrderCount: () => number; readonly getEvents: () => readonly Readonly<Record<string, unknown>>[]; }
...
function route(req: IncomingMessage): string { try { return new URL(req.url ?? "/", "http://127.0.0.1").pathname; } catch { return ""; } }
...
export function startMobileBridge(options: MobileBridgeOptions): MobileBridgeHandle {
  ...
  const server: Server = http.createServer((req, res) => {
    try {
      ...
      const path = route(req); const status = options.getStatus();
