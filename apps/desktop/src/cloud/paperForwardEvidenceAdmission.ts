...
/**
 * Admits longitudinal PAPER/shadow outcomes as candidate-specific research evidence only when
 * identity, provenance, chronology and cost attribution are explicit. This module intentionally
 * does not infer candidate identity from PAPER account PnL or CioDecision text: current canonical
 * PAPER observability does not persist that binding, so guessing would create false evidence.
 */
