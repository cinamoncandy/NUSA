...
function entry(
  ...
  rank: number,
  leagueScore: number,
  overrides: Partial<LeagueRankedEntry> = {},
): LeagueRankedEntry {
  return {
    id,
    familyId: `family-${id}`,
    ...
    evidenceBreadth: 1,
    components: {
      outOfSamplePerformance: 0.05,
      benchmarkExcess: 0.02,
...
describe("adviseLeagueCapitalAllocation", () => {
  it("produces capped normalized research-only weights from League standings", () => {
    ...
    assert.deepEqual(advisory.reasons, ["NO_EXECUTION_AUTHORITY", "NORMALIZED_RESEARCH_WEIGHTS_ONLY"]);
  });
  ...
  it("keeps equal-score candidates equally weighted", () => {
    ...
    for (const candidate of advisory.entries) {
      assert.ok(Math.abs(candidate.researchWeight - 1 / 3) < 1e-9);
    }
  });
