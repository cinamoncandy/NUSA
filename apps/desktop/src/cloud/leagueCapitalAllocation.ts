...
/**
 * Converts League ranking evidence into normalized, capped research weights.
 *
 * Score magnitude is preserved through min-max normalization, while every selected candidate
 * receives a non-zero floor (25% of normalized utility) so tiny score noise cannot collapse a
 * diversified research basket to one candidate. The policy cap is then applied by deterministic
 * water-filling. These are advisory research weights only, never executable position sizes.
 */
