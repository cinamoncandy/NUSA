...
function AuthenticatedApp() {
  ...
  const nextAction: Readonly<{ title: string; detail: string; label: string; tab: Tab }> | null = snapshot == null ? null
    : snapshot.health !== "HEALTHY" || snapshot.dashboard.killSwitchActive
      ? { title: "안전 상태 우선", detail: "차단·저하 상태에서는 실행보다 PAPER 관찰 상태를 먼저 확인합니다.", label: "PAPER 상태 보기", tab: "Trade" }
      : ai?.status === "AVAILABLE"
