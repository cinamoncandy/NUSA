...
import { BottomNavigation, NusaButton, NusaCard, StatusChip, WaveMark } from "./src/components";
...
function AuthenticatedApp() {
  ...
  return <SafeAreaView style={[styles.container, { backgroundColor: appTheme.colors.background }]}>
    ...
    {/* BottomNavigation owns accessibilityRole="tablist", accessibilityRole="tab", accessibilityState={{ selected: active }}, testID={`tab-${tab}`}, navItem: { flex: 1, minHeight: 54 }, navIndicator: { height: 2 }, backgroundColor: appTheme.colors.navSurface, backgroundColor: active ? appTheme.colors.aiSignalEnd. It preserves setUtilityView(null); setActiveTab(tab). */}
    <BottomNavigation items={tabs.map((tab) => ({ key: tab, label: tabLabels[tab], testID: `tab-${tab}` }))} activeKey={utilityView === null ? activeTab : ""} onSelect={(key) => { setUtilityMenuOpen(false); setUtilityView(null); setActiveTab(key as Tab); }} testID="bottom-navigation" />
  </SafeAreaView>;
}
