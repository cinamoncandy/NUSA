...
import { isTradingSection, TradingWorkspace, type TradingSection } from "./src/tradingWorkspace";
...
const tabs = ["Home", "Trading", "LiveTrading", "Portfolio", "AiSignal"] as const;
type PrimaryTab = (typeof tabs)[number];
// Markets and Paper remain routes (deep links, openPaperTrade, back navigation) inside the single
// TRADING destination; only the primary navigation converged.
type Tab = PrimaryTab | TradingSection | "Order";
type UtilityView = "NOTIFICATIONS" | "SETTINGS" | null;
const tabLabels: Readonly<Record<PrimaryTab, string>> = { Home: "HOME", Trading: "TRADING", LiveTrading: "LIVE TRADING", Portfolio: "PORTFOLIO", AiSignal: "AI" };
const tabDisplayLabels: Readonly<Record<PrimaryTab, string>> = { Home: "NUSA", Trading: "TRADING", LiveTrading: "LIVE", Portfolio: "자산", AiSignal: "AI" };
const tabDescriptions: Readonly<Record<PrimaryTab, string>> = { Home: "현재 NUSA 상태", Trading: "시장과 PAPER 운용", LiveTrading: "실거래 (비활성)", Portfolio: "PAPER 자산과 결과", AiSignal: "AI 판단과 근거" };
...
function AuthenticatedApp() {
  ...
  return <SafeAreaView style={[styles.container, { backgroundColor: appTheme.colors.background }]}>
    ...
    {paperLearningOpen ? <PaperShadowMonitorView paper={paperLearningState} shadow={shadowOperations.status === "READY" ? shadowOperations.snapshot : null} shadowReason={shadowOperations.status === "READY" ? undefined : shadowOperations.reason} real={realReadOnlyOperations.status === "READY" ? realReadOnlyOperations.snapshot : null} realReason={realReadOnlyOperations.status === "READY" ? undefined : realReadOnlyOperations.reason} live={liveReadinessOperations.status === "READY" ? liveReadinessOperations.snapshot : null} liveReason={liveReadinessOperations.status === "READY" ? undefined : liveReadinessOperations.reason} refreshing={refreshing} onRefresh={onRefresh} onClose={() => setPaperLearningOpen(false)} />
      ...
      : activeTab === "Paper" ? <TradingWorkspace section="Paper" onSectionChange={setActiveTab}><TradingView error={readOnlyError} credentialSession={credentialSession} investmentPercent={investmentPercent} marketConnectionState={marketConnectionState} onOpenPaperLearning={openPaperLearning} onRefresh={onRefresh} paperLearning={paperLearningState} refreshing={refreshing} runtimeCanSubmit={runtimeCanSubmit} snapshot={snapshot?.portfolio ?? null} stale={stale} /></TradingWorkspace>
      : activeTab === "Markets" ? <TradingWorkspace section="Markets" onSectionChange={setActiveTab}><MarketsView chartError={publicMarkets.chartError} chartErrorDiagnostic={publicMarkets.chartErrorDiagnostic} error={publicMarkets.status === "ERROR" ? publicMarkets.error : null} currentPrice={publicMarkets.currentPrice} market={CHART_MARKET} marketConnectionState={publicMarketConnectionState} marketsStale={publicMarkets.status === "STALE"} onPaperTrade={openPaperTrade} onRefresh={refreshPublicMarkets} rawCandles={publicMarkets.candles === null ? null : [...publicMarkets.candles]} rawMarkets={publicMarkets.markets === null ? null : [...publicMarkets.markets]} refreshing={publicRefreshing} repository={watchlistRepository} stale={publicMarkets.status !== "READY"} /></TradingWorkspace>
