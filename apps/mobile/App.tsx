...
function AuthenticatedApp() {
  ...
  if (authStatus === "CHECKING") return <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}><View style={styles.authContent}><Text style={[styles.brand, { color: theme.colors.text }]}>NUSA</Text><Text style={[styles.heading, { color: theme.colors.text }]}>Loading</Text></View></SafeAreaView>;
  ...
  return <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
    <View style={styles.header}><Text style={[styles.brand, { color: theme.colors.text }]}>NUSA</Text><Text style={[styles.mode, { color: theme.colors.textMuted }]}>Personal PAPER · Read Only</Text></View>
    {activeTab === "Portfolio" ? <PortfolioView error={readOnlyError ?? notConfigured} onRefresh={onRefresh} refreshing={refreshing} snapshot={snapshot?.portfolio ?? null} />
      : activeTab === "Trade" ? <TradingView error={readOnlyError ?? notConfigured} marketConnectionState={marketConnectionState} onRefresh={onRefresh} refreshing={refreshing} snapshot={null} stale={stale} />
      ...
      : <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        <NusaCard testID="operations-card"><Text style={[styles.label, { color: theme.colors.textMuted }]}>Operations</Text><Text style={[styles.value, { color: theme.colors.primary }]}>{snapshot?.operations.runtimeState ?? "Not configured"}</Text><Text style={styles.meta}>Transport: {snapshot?.operations.transport ?? "-"}</Text><Text style={styles.meta}>Scheduler: {snapshot?.operations.schedulerMode ?? "-"}</Text><Text style={styles.meta}>Pending writes: {snapshot?.operations.pendingWrites ?? "-"}</Text></NusaCard>
        <NusaCard testID="research-card"><Text style={[styles.label, { color: theme.colors.textMuted }]}>Research</Text><Text style={[styles.value, { color: theme.colors.primary }]}>{snapshot?.research?.health ?? "Unavailable"}</Text><Text style={styles.meta}>Champion: {snapshot?.research?.champion.strategyId ?? "-"} · PAPER_ONLY</Text><Text style={styles.meta}>Challenger: {snapshot?.research?.challenger.strategyId ?? "-"} · ZERO_AUTHORITY</Text><Text style={styles.meta}>Candidates: {snapshot?.research?.candidateCount ?? "-"}</Text></NusaCard>
        <NusaCard testID="safety-card"><Text style={[styles.label, { color: theme.colors.textMuted }]}>Safety</Text><Text style={[styles.value, { color: snapshot?.health === "FAIL_CLOSED" ? theme.colors.danger : theme.colors.primary }]}>{snapshot?.health ?? "FAIL_CLOSED"}</Text><Text style={styles.meta}>PAPER ready: {snapshot?.readyForPaperOperations ? "Yes" : "No"}</Text><Text style={styles.meta}>Kill switch: {snapshot?.dashboard.killSwitchActive ? "ACTIVE" : "Not active"}</Text><Text style={styles.meta}>LIVE authority: {snapshot?.liveAuthority ?? "NONE"}</Text></NusaCard>
      </ScrollView>}
    <View style={styles.navigation}>{tabs.map((tab) => <Pressable key={tab} accessibilityRole="button" onPress={() => setActiveTab(tab)} style={styles.navItem}><Text style={[styles.navLabel, activeTab === tab && styles.navLabelActive]}>{tab}</Text></Pressable>)}</View>
  </SafeAreaView>;
}
