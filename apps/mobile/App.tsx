...
} from "./src/paperConnectionSession";
...
function DashboardConnectionRequired({ reason, onGoSettings }: Readonly<{ reason: string; onGoSettings: () => void }>) {
  ...
  return <View style={styles.connectionState} testID="dashboard-connection-required"><NusaCard raised>
    <View style={styles.cardHeader}><View><Text style={[styles.cardEyebrow, { color: appTheme.colors.warning }]}>PAPER CONNECTION</Text><Text style={[styles.cardTitle, { color: appTheme.colors.text }]}>PAPER 연결 필요</Text></View><StatusChip label="검증 필요" tone="warning" /></View>
    <Text style={[styles.notice, { color: appTheme.colors.textMuted }]}>{reason}</Text>
    <Text style={[styles.meta, { color: appTheme.colors.textMuted }]}>검증된 PAPER 스냅샷이나 가상 주문 기능이 필요한 화면입니다. 설정에서 Cloud endpoint와 메모리 세션 토큰을 검증해 주세요.</Text>
...
function AuthenticatedApp() {
  ...
  const [operations, setOperations] = useState<PersonalPaperOperationsLoadResult>({ status: "NOT_CONFIGURED", reason: "PAPER endpoint and credential are not verified." });
  ...
  const refresh = useCallback(async () => {
    ...
    if (connection.endpoint == null) {
      setOperations({ status: "NOT_CONFIGURED", reason: connection.reason });
      return;
    }
    if (!credentialSession.isConfigured()) {
      setOperations(connection.status === "UNAVAILABLE"
        ? { status: "UNAVAILABLE", reason: connection.reason }
        : { status: "NOT_CONFIGURED", reason: connection.reason });
      return;
    }
    const result = await loadPersonalPaperOperations({ baseUrl: connection.endpoint, credentialProvider: credentialSession.credentialProvider });
    if (result.status === "READY") markPaperConnectionReady(connection.endpoint);
    else if (result.status === "UNAVAILABLE") markPaperConnectionUnavailable(connection.endpoint, result.reason);
    else clearPaperConnectionSession(connection.endpoint);
  ...
  const handleSignOut = useCallback(() => {
    const endpoint = getPaperConnectionState().endpoint ?? "";
    credentialSession.clear();
    clearPaperConnectionSession(endpoint);
    setOperations({ status: "NOT_CONFIGURED", reason: endpoint ? "PAPER credential is not configured." : "PAPER endpoint and credential are not configured." });
  ...
  useEffect(() => {
    ...
    void settingsRepository.load().then((stored) => {
      if (!active) return;
      setConfiguredPaperEndpoint((stored ?? DEFAULT_SETTINGS).paperEndpoint);
      return refresh();
    }).catch(() => { if (active) setOperations({ status: "NOT_CONFIGURED", reason: "PAPER settings are unavailable." }); });
    const timer = setInterval(() => { if (active) void refresh(); }, 5000);
    return () => { active = false; clearInterval(timer); };
  }, [authStatus, refresh]);

  const onRefresh = useCallback(async () => { setRefreshing(true); try { await refresh(); } finally { setRefreshing(false); } }, [refresh]);

  if (authStatus === "CHECKING") return <SafeAreaView style={[styles.container, { backgroundColor: appTheme.colors.background }]}><View style={styles.authContent}><WaveMark /><Text style={[styles.brand, { color: appTheme.colors.text }]}>NUSA</Text><Text style={[styles.authHeading, { color: appTheme.colors.text }]}>로컬 화면 준비 중</Text></View></SafeAreaView>;

  if (authStatus !== "SIGNED_IN") {
    return <SafeAreaView style={[styles.container, { backgroundColor: appTheme.colors.background }]}><View style={styles.authContent} testID="local-onboarding">
      <View style={styles.authBrand}><WaveMark /><View><Text style={[styles.brand, { color: appTheme.colors.text }]}>NUSA</Text><Text style={[styles.eyebrow, { color: appTheme.colors.primary }]}>PERSONAL PAPER</Text></View></View>
      <Text style={[styles.authHeading, { color: appTheme.colors.text }]}>NUSA PAPER 시작</Text>
      <Text style={[styles.subtitle, { color: appTheme.colors.textMuted }]}>이 버튼은 이 기기의 로컬 화면 세션만 엽니다. 계정 인증을 가장하지 않으며, 서버 연결과 토큰 검증은 설정에서 별도로 수행합니다.</Text>
      <NusaCard raised><View style={styles.cardHeader}><Text style={[styles.cardTitle, { color: appTheme.colors.text }]}>권한 경계</Text><StatusChip label="PAPER ONLY" tone="primary" /></View><DataRow label="가상 PAPER 주문" value="지원" emphasis /><DataRow label="LIVE 권한" value="NONE" tone="success" /><DataRow label="Production mutation" value="금지" tone="success" /></NusaCard>
      <NusaButton accessibilityLabel="Open PAPER workspace" label="PAPER 작업공간 열기" onPress={signIn} testID="local-session-start" />
      <Text style={[styles.meta, { color: appTheme.colors.textMuted }]}>서버 자격 증명은 이 로컬 진입 동작에서 생성·추론·저장되지 않습니다.</Text>
    </View></SafeAreaView>;
  }
  ...
  const nextAction: Readonly<{ title: string; detail: string; label: string; tab: Tab }> | null = snapshot == null ? null
    : snapshot.health !== "HEALTHY" || snapshot.dashboard.killSwitchActive || !snapshot.readyForPaperOperations
      ? { title: "안전 상태 우선", detail: "차단·저하 또는 PAPER 운영 대기 상태에서는 실행보다 PAPER 상태를 먼저 확인합니다.", label: "PAPER 상태 보기", tab: "Trade" }
      : aiInsightAvailable
  ...
  return <SafeAreaView style={[styles.container, { backgroundColor: appTheme.colors.background }]}>
    ...
    <View style={[styles.authorityStrip, { borderBottomColor: appTheme.colors.border }]} testID="global-authority-strip"><StatusChip label="PAPER ONLY" tone="primary" /><StatusChip label="LIVE NONE" tone="info" /><Text style={[styles.authorityCopy, { color: appTheme.colors.textMuted }]}>가상 주문만 · production mutation 없음</Text></View>
    ...
    {utilityMenuOpen ? <View style={[styles.utilityMenu, { backgroundColor: appTheme.colors.surface, borderBottomColor: appTheme.colors.border }]} testID="header-tools-tray">
      ...
      <Pressable accessibilityLabel="설정" accessibilityRole="button" onPress={goSettings} style={[styles.utilityMenuButton, { borderColor: appTheme.colors.border, backgroundColor: appTheme.colors.surfaceSunken }]} testID="header-settings"><Text style={[styles.utilityText, { color: appTheme.colors.text }]}>설정</Text></Pressable>
    </View> : null}
    ...
    {requiresDashboardConnection ? <DashboardConnectionRequired reason={notConfigured ?? "PAPER 연결이 필요합니다."} onGoSettings={goSettings} />
      ...
      : utilityView === "SETTINGS" ? <SettingsView onConnectionChanged={() => { void refresh(); }} onSignOut={handleSignOut} repository={settingsRepository} />
      ...
      : <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={appTheme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="home-screen">
        <SectionHeading eyebrow="PERSONAL PAPER" title="오늘의 운영 상태" description="실제 PAPER 런타임과 검증된 스냅샷만 표시합니다." />
        {readOnlyError ? <View style={[styles.error, { backgroundColor: appTheme.colors.surfaceSunken, borderColor: appTheme.colors.danger }]} testID="home-connection-error"><Text style={[styles.errorTitle, { color: appTheme.colors.danger }]}>PAPER 연결 오류</Text><Text style={[styles.meta, { color: appTheme.colors.textMuted }]}>{readOnlyError}</Text><NusaButton label="설정에서 재검증" onPress={goSettings} tone="neutral" /></View> : null}
        {notConfigured ? <NusaCard testID="paper-connection-card" raised><View style={styles.cardHeader}><View><Text style={[styles.cardEyebrow, { color: appTheme.colors.primary }]}>PAPER CONNECTION</Text><Text style={[styles.cardTitle, { color: appTheme.colors.text }]}>PAPER 서버 연결</Text></View><StatusChip label="설정 필요" tone="warning" /></View><Text style={[styles.notice, { color: appTheme.colors.textMuted }]}>{notConfigured}</Text><NusaButton label="설정에서 연결" onPress={goSettings} testID="home-open-settings" /><Text style={[styles.meta, { color: appTheme.colors.textMuted }]}>Endpoint는 로컬 설정에 저장할 수 있지만 토큰은 메모리에만 유지합니다.</Text></NusaCard> : null}
        {snapshot ? <View style={[styles.homeColumns, isTablet && styles.homeColumnsWide]} testID={isTablet ? "home-tablet-grid" : "home-mobile-stack"}>
          ...
          <View style={styles.homeColumn}>
            <AuthorityBanner />
            <NusaCard testID="ai-card"><View style={styles.cardHeader}><View><Text style={[styles.cardEyebrow, { color: appTheme.colors.info }]}>AI READ-ONLY</Text><Text style={[styles.cardTitle, { color: appTheme.colors.text }]}>AI 인텔리전스</Text></View><StatusChip label={ai?.status ?? "UNAVAILABLE"} tone={ai?.status === "AVAILABLE" ? "success" : ai?.status === "INCOMPLETE" ? "warning" : "neutral"} /></View><Text style={[styles.aiThesis, { color: ai?.thesis ? appTheme.colors.text : appTheme.colors.textMuted }]}>{ai?.thesis ?? "현재 표시할 AI 분석이 없습니다."}</Text><DataRow label="원시 모델 확률 (미보정)" value={aiRawProbability} /><DataRow label="검증 신뢰도" value={aiTrustedConfidence} /><DataRow label="불확실성" value={ai?.uncertainty ?? "-"} /><DataRow label="보정 상태" value={ai?.calibrationStatus ?? "UNKNOWN"} /></NusaCard>
            <NusaCard testID="connection-management-card"><View style={styles.cardHeader}><Text style={[styles.cardTitle, { color: appTheme.colors.text }]}>연결 관리</Text><StatusChip label="검증됨" tone="success" /></View><DataRow label="PAPER endpoint" value={getPaperConnectionState().endpoint ?? "-"} /><Text style={[styles.meta, { color: appTheme.colors.textMuted }]}>연결 해제·토큰 재검증·endpoint 변경은 설정 한 곳에서 관리합니다.</Text><NusaButton label="연결 설정 열기" onPress={goSettings} tone="neutral" testID="home-manage-connection" /></NusaCard>
          </View>
        </View> : null}
      </ScrollView>}
...
const styles = StyleSheet.create({
  ...
  authContent: { flex: 1, justifyContent: "center", padding: 24, gap: 16, width: "100%", maxWidth: 560, alignSelf: "center" },
  ...
  connectionState: { flex: 1, justifyContent: "center", padding: 20, width: "100%", maxWidth: 760, alignSelf: "center" },
  authHeading: { fontSize: 29, fontWeight: "700", letterSpacing: -0.8 },
  subtitle: { fontSize: 14, lineHeight: 21 },
  meta: { fontSize: 12, lineHeight: 18 },
  notice: { fontSize: 13, lineHeight: 20 },
  error: { borderWidth: 1, padding: 14, borderRadius: 14, gap: 10 },
  ...
  homeColumns: { gap: 14 },
  homeColumnsWide: { flexDirection: "row", alignItems: "flex-start" },
  homeColumn: { flex: 1, minWidth: 0, gap: 14 },
