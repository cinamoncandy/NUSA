...
function isSecureEndpoint(baseUrl: string): boolean {
  try { const url = new URL(baseUrl); if (url.username || url.password) return false; if (url.protocol === "https:") return true; if (url.protocol !== "http:") return false; const host = url.hostname.toLowerCase(); return host === "localhost" || host === "127.0.0.1" || host === "::1" || host === "[::1]"; } catch { return false; }
}
...
export async function submitPersonalPaperOrder(options: PersonalPaperOrderClientOptions, command: PersonalPaperOrderCommand): Promise<PersonalPaperOrderSubmitResult> {
  ...
  try { validated = validatePersonalPaperOrderCommand(command); timeoutMs = readTimeoutMs(options.timeoutMs); }
  catch (error) { return Object.freeze({ status: "UNAVAILABLE", reason: error instanceof Error ? error.message : "PAPER order command is invalid." }); }
  ...
  try {
    ...
    const operation = (async () => {
      const response = await request(endpoint, { method: "POST", redirect: "error", signal: controller.signal, headers: { authorization: `Bearer ${token.trim()}`, accept: "application/json", "content-type": "application/json", "idempotency-key": validated.idempotencyKey }, body: JSON.stringify(validated) });
    ...
    const result = validatePersonalPaperOrderCommandResult(payload as PersonalPaperOrderCommandResult, Date.now());
