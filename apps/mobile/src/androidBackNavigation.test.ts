...
describe("Android back navigation containment", () => {
  it("closes PAPER learning before any parent surface", () => {
    assert.equal(resolveAndroidBackNavigation(state({ paperLearningOpen: true, utilityViewOpen: true, activeTab: "More" })), "CLOSE_PAPER_LEARNING");
  });
  ...
  it("closes the utility tray before changing tabs", () => {
    assert.equal(resolveAndroidBackNavigation(state({ utilityMenuOpen: true, activeTab: "Paper" })), "CLOSE_UTILITY_MENU");
  });

  it("returns any non-root tab or detail route to HOME", () => {
    for (const activeTab of ["Paper", "Live", "More"] as const) {
