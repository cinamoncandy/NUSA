...
export function WatchlistView({ repository, rawMarkets, error, stale, refreshing, onRefresh, selectedMarket, onSelectMarket }: WatchlistViewProps) {
  ...
  const header = <View style={styles.headerBlock}>
    <View style={styles.workspaceHeader}><View><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>MARKETS</Text><Text style={[styles.workspaceTitle, { color: theme.colors.text }]}>시장 목록</Text></View><View style={styles.statusChips}>{feedAge === null ? null : <Text style={[styles.freshness, { color: theme.colors.textMuted }]} testID="watchlist-freshness">{feedAge}</Text>}{stale ? <StatusChip label="STALE" tone="warning" /> : null}</View></View>
    {stale ? <Text style={[styles.contextNote, { color: theme.colors.textMuted }]}>마지막으로 검증된 시세입니다. 새 공개 시세를 받으면 자동 갱신됩니다.</Text> : null}
    <View style={styles.controls}><NusaTextField label="시장 검색" value={query} onChangeText={setQuery} placeholder="예: KRW-BTC" testID="watchlist-search" /><View style={styles.sortGroup}><Text style={[styles.controlLabel, { color: theme.colors.textMuted }]}>정렬</Text><SegmentedControl items={sortItems} selectedKey={sort} onChange={(key) => setSort(key as WatchlistSort)} testID="watchlist-sort" /></View></View>
    ...
    {model.activeMarkets.length === 0 ? <View style={[styles.quietEmpty, { borderColor: theme.colors.border }]} testID="watchlist-saved-empty"><Text style={[styles.quietEmptyText, { color: theme.colors.textMuted }]}>자주 보는 시장만 저장해 두세요.</Text></View> : <View style={styles.marketList}>{model.activeMarkets.map((item) => <MarketRow key={`saved-${item.market}`} active market={item} selected={selectedMarket === item.market} onSelect={onSelectMarket ? () => onSelectMarket(item.market) : undefined} onToggle={() => void toggle(item.market)} />)}</View>}
    <View style={styles.sectionHeader}><Text style={[styles.section, { color: theme.colors.text }]}>전체 시장</Text><Text style={[styles.sectionCount, { color: theme.colors.textMuted }]}>{model.searchResults.length}</Text></View>
  </View>;
...
const styles = StyleSheet.create({
  content: { paddingBottom: 28 }, headerBlock: { gap: 10, marginBottom: 8 }, marketListGap: { height: 0 }, state: { flex: 1, justifyContent: "center", padding: 20, gap: 14 }, stateTitle: { fontSize: 18, fontWeight: "700" }, message: { lineHeight: 21, fontSize: 14, marginTop: 8 }, workspaceHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end", gap: 12 }, statusChips: { flexDirection: "row", flexWrap: "wrap", gap: 6, justifyContent: "flex-end", alignItems: "center" }, freshness: { fontSize: 10, lineHeight: 14, fontWeight: "700", fontVariant: ["tabular-nums"] }, eyebrow: { fontSize: 9, fontWeight: "800", letterSpacing: 1.5, marginBottom: 2 }, workspaceTitle: { fontSize: 22, lineHeight: 27, fontWeight: "800", letterSpacing: -0.6 }, contextNote: { fontSize: 11, lineHeight: 16, maxWidth: 620 }, controls: { gap: 8 }, sortGroup: { gap: 5 }, controlLabel: { fontSize: 9, fontWeight: "800", letterSpacing: 1 }, sectionHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 5 }, section: { fontSize: 15, fontWeight: "800", letterSpacing: -0.2 }, sectionCount: { fontSize: 11, fontWeight: "700", fontVariant: ["tabular-nums"] }, quietEmpty: { minHeight: 44, justifyContent: "center", borderTopWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth, paddingVertical: 10 }, quietEmptyText: { fontSize: 12, lineHeight: 17 }, marketList: { backgroundColor: "transparent" }, marketRow: { borderBottomWidth: StyleSheet.hairlineWidth }, marketMain: { minHeight: 54, flexDirection: "row", alignItems: "center", gap: 8 }, marketSelect: { flex: 1, minWidth: 0, minHeight: 48, flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 6 }, marketIdentity: { flex: 1, minWidth: 84 }, marketNumbers: { minWidth: 116, alignItems: "flex-end", justifyContent: "center" }, market: { fontSize: 15, lineHeight: 19, fontWeight: "800", letterSpacing: -0.2 }, volumeInline: { marginTop: 2, fontSize: 9, lineHeight: 13, fontWeight: "600", fontVariant: ["tabular-nums"] }, price: { fontSize: 16, lineHeight: 20, fontWeight: "800", letterSpacing: -0.4, fontVariant: ["tabular-nums"] }, change: { fontSize: 11, lineHeight: 15, fontWeight: "700", marginTop: 1, fontVariant: ["tabular-nums"] }, favorite: { minWidth: 48, minHeight: 44, paddingHorizontal: 8, borderRadius: 12, borderWidth: 1, alignItems: "center", justifyContent: "center" }, favoriteLabel: { fontSize: 10, fontWeight: "800" },
});
