...
export function WatchlistView({ repository, rawMarkets, error, refreshing, onRefresh }: WatchlistViewProps) {
  ...
  if (model.state === "ERROR") return <View style={styles.state} testID="watchlist-error"><NusaCard><Text style={[styles.stateTitle, { color: theme.colors.danger }]}>시장 데이터가 유효하지 않습니다</Text><Text style={[styles.message, { color: theme.colors.textMuted }]}>{model.error}</Text><NusaButton label="다시 불러오기" onPress={onRefresh} /></NusaCard></View>;
