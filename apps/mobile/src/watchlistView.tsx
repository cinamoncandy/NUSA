...
interface WatchlistViewProps { readonly repository: WatchlistRepository; readonly rawMarkets: unknown[] | null; readonly error: string | null; readonly refreshing: boolean; readonly onRefresh: () => void; }
...
export function WatchlistView({ repository, rawMarkets, error, refreshing, onRefresh }: WatchlistViewProps) {
  ...
  const header = <View style={styles.headerBlock}>
    <View style={styles.workspaceHeader}><View><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>WATCHLIST</Text><Text style={[styles.workspaceTitle, { color: theme.colors.text }]}>관심시장</Text></View><StatusChip label="READ ONLY" tone="info" /></View>
    <Text style={[styles.readOnlyNote, { color: theme.colors.textMuted }]}>관심시장 저장은 이 기기의 로컬 보기 설정이며 계좌·주문 권한과 연결되지 않습니다.</Text>
