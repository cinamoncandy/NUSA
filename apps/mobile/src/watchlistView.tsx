...
export function WatchlistView({ repository, rawMarkets, error, stale, refreshing, onRefresh }: WatchlistViewProps) {
  ...
  const renderMarket = useCallback(({ item }: { item: WatchlistMarket }) => <MarketRow market={item} active={model.watchlist.includes(item.market)} onToggle={() => void toggle(item.market)} />, [model.watchlist]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // The saved-markets section is bounded by the user's own selections and always stays in the
  // header; "전체 결과" is every matching public market (up to Upbit's full catalog when the
  // search box is empty), so only that list is virtualized -- FlatList renders just the visible
  // rows instead of the whole catalog at once.
...
const styles = StyleSheet.create({
  ...
  content: { paddingBottom: 32 }, headerBlock: { gap: 14, marginBottom: 14 }, marketListGap: { height: 0 }, state: { flex: 1, justifyContent: "center", padding: 20, gap: 14 }, stateTitle: { fontSize: 18, fontWeight: "700" }, message: { lineHeight: 21, fontSize: 14, marginTop: 8 }, workspaceHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }, statusChips: { flexDirection: "row", flexWrap: "wrap", gap: 6, justifyContent: "flex-end" }, eyebrow: { fontSize: 10, fontWeight: "800", letterSpacing: 1.7, marginBottom: 4 }, workspaceTitle: { fontSize: 25, lineHeight: 30, fontWeight: "800", letterSpacing: -0.7 }, readOnlyNote: { fontSize: 12, lineHeight: 18, maxWidth: 620 }, sortGroup: { gap: 7 }, controlLabel: { fontSize: 10, fontWeight: "800", letterSpacing: 1.1 }, sectionHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 }, section: { fontSize: 17, fontWeight: "800", letterSpacing: -0.3 }, sectionCount: { fontSize: 12, fontWeight: "700", fontVariant: ["tabular-nums"] }, marketList: { backgroundColor: "transparent" }, marketRow: { borderBottomWidth: StyleSheet.hairlineWidth, paddingHorizontal: 2, paddingVertical: 12 }, marketMain: { minHeight: 52, flexDirection: "row", alignItems: "center", gap: 12 }, marketIdentity: { flex: 1, minWidth: 84 }, marketNumbers: { minWidth: 112, alignItems: "flex-end", justifyContent: "center" }, market: { fontSize: 16, fontWeight: "800", letterSpacing: -0.2 }, volumeInline: { marginTop: 5, fontSize: 10, lineHeight: 14, fontWeight: "600", fontVariant: ["tabular-nums"] }, price: { fontSize: 18, fontWeight: "800", letterSpacing: -0.5, fontVariant: ["tabular-nums"] }, change: { fontSize: 12, fontWeight: "700", marginTop: 3, fontVariant: ["tabular-nums"] }, favorite: { minWidth: 52, minHeight: 48, paddingHorizontal: 8, borderRadius: 999, borderWidth: 1, alignItems: "center", justifyContent: "center" }, favoriteLabel: { fontSize: 11, fontWeight: "800" },
});
