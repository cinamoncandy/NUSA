...
export function SettingsView({ repository, onSignOut }: SettingsViewProps) {
  ...
  return <ScrollView contentContainerStyle={styles.content} testID="settings-screen">
    <View style={styles.header}><SectionHeading eyebrow="APPLICATION" title="설정" description="PAPER 서버 연결, 화면, 알림, 로컬 세션을 관리합니다." /><StatusChip label="PAPER" tone="primary" /></View>
    ...
    <NusaCard testID="settings-paper-connection" raised>
      <View style={styles.modeHeader}><Text style={[styles.section, { color: theme.colors.text, marginBottom: 0 }]}>PAPER 서버 연결</Text><StatusChip label={connection.status === "READY" ? "연결됨" : "연결 필요"} tone={connectionTone} /></View>
      ...
      <NusaTextField label="세션 토큰" value={tokenDraft} onChangeText={setTokenDraft} placeholder="기기에 저장하지 않음" secureTextEntry testID="settings-paper-token" />
      ...
      <View style={styles.row}><NusaButton disabled={saving} label="저장 + 연결 테스트" onPress={() => void testConnection()} testID="settings-paper-connect" /><NusaButton label="연결 해제" onPress={disconnect} tone="neutral" testID="settings-paper-disconnect" /></View>
