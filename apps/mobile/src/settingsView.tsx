...
export function SettingsView({ repository, onSignOut, exchangeCash = 0, onCloudInvestmentPercentSave, onInvestmentPercentChanged }: SettingsViewProps) {
  ...
  const updateNotification = (field: "enabled" | "riskAlerts" | "orderUpdates") => { if (settings && !isBusyNow()) void persist({ ...settings, notifications: { ...settings.notifications, [field]: !settings.notifications[field] } }); };
  ...
  return <ScrollView contentContainerStyle={styles.content} testID="settings-screen">
    ...
    <View style={[styles.divider, { backgroundColor: theme.colors.border }]} />
    <View style={styles.sectionBlock} testID="settings-notifications"><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>05 · NOTIFICATIONS</Text><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>알림</Text><View style={styles.settingRow}><Text style={[styles.hint, { color: theme.colors.text }]}>전체 알림</Text><NusaButton disabled={busy} selected={settings.notifications.enabled} label={settings.notifications.enabled ? "켜짐" : "꺼짐"} onPress={() => updateNotification("enabled")} tone={settings.notifications.enabled ? "primary" : "neutral"} /></View><View style={styles.settingRow}><Text style={[styles.hint, { color: theme.colors.text }]}>리스크 알림</Text><NusaButton disabled={busy} selected={settings.notifications.riskAlerts} label={settings.notifications.riskAlerts ? "켜짐" : "꺼짐"} onPress={() => updateNotification("riskAlerts")} tone={settings.notifications.riskAlerts ? "primary" : "neutral"} /></View><View style={styles.settingRow}><Text style={[styles.hint, { color: theme.colors.text }]}>주문 상태 업데이트</Text><NusaButton disabled={busy} selected={settings.notifications.orderUpdates} label={settings.notifications.orderUpdates ? "켜짐" : "꺼짐"} onPress={() => updateNotification("orderUpdates")} tone={settings.notifications.orderUpdates ? "primary" : "neutral"} /></View></View>

    <View style={[styles.divider, { backgroundColor: theme.colors.border }]} />
    <View style={styles.sectionBlock} testID="settings-mode"><View style={styles.sectionHeader}><View><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>06 · SAFETY & LOCAL</Text><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>안전과 로컬 관리</Text></View><View style={styles.row}><StatusChip label="PAPER ONLY" tone="info" /><StatusChip label="READ ONLY" tone="info" /></View></View><NusaCard><DataRow label="운영 모드" value="PAPER" emphasis /><DataRow label="LIVE 주문" value="금지" tone="success" /><DataRow label="Production mutation" value="금지" tone="success" /><Text style={[styles.hint, { color: theme.colors.textMuted }]}>LIVE·출금·이체 권한은 이 화면에서 활성화할 수 없습니다.</Text></NusaCard><NusaCard testID="settings-about"><DataRow label="클라이언트" value="NUSA Mobile 0.1.0" /><DataRow label="용도" value="PAPER / Read Only" /></NusaCard><View style={styles.row} testID="settings-reset"><NusaButton label={busy ? "작업 중..." : "설정 초기화"} disabled={busy} onPress={resetSettings} tone="danger" /></View>{onSignOut ? <View testID="settings-session"><NusaButton disabled={busy} label="개인 모드 종료" onPress={signOutLocal} tone="neutral" testID="settings-sign-out" /></View> : null}</View>
  </ScrollView>;
}
