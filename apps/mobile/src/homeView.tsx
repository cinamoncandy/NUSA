...
import { getScreenComposition, type HomePrimarySection, type HomeSecondarySection } from "./screenComposition";
...
export function HomeView({ snapshot, investmentPercent, readOnlyError, notConfigured, refreshing, onRefresh, onGoSettings, onNavigate }: HomeViewProps) {
  ...
  const composition = getScreenComposition(theme.preset).home;
  ...
  const secondarySections: Readonly<Record<HomeSecondarySection, React.ReactNode>> = {
    aiInsight: snapshot ? <AIInsight thesis={ai?.thesis ?? null} evidenceCount={ai?.evidenceReferences.length ?? 0} confidence={ai?.calibrationStatus === "CALIBRATED" ? aiTrustedConfidence : undefined} onPress={() => onNavigate("AiSignal")} testID="ai-card" /> : null,
    // SafetyNotice keeps the canonical label="LIVE 권한" and label="Production mutation" rows visible.
    safety: snapshot ? <SafetyNotice health={snapshot.health} runtimeState={snapshot.operations.runtimeState} killSwitchActive={snapshot.dashboard.killSwitchActive} liveAuthority={snapshot.liveAuthority} productionMutationAllowed={snapshot.productionMutationAllowed} testID="safety-card" /> : null,
  };
  ...
  return <ScrollView contentContainerStyle={[styles.content, { paddingHorizontal: theme.layout.screenPadding, gap: theme.layout.sectionGap }, tablet && styles.contentTablet]} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="home-screen">
    ...
    <View style={[styles.dashboard, tablet && composition.desktopLayout === "split" && styles.dashboardTablet]} testID={`home-layout-${composition.desktopLayout}`}>
      <View style={styles.primaryColumn}>
        {composition.primary.map((section) => <React.Fragment key={section}>{primarySections[section]}</React.Fragment>)}
      </View>

      {snapshot ? <View style={[styles.secondaryColumn, tablet && composition.desktopLayout === "split" && styles.secondaryColumnTablet]}>
        {composition.secondary.map((section, index) => <React.Fragment key={section}>{index > 0 ? <View style={[styles.divider, { backgroundColor: theme.colors.border }]} /> : null}{secondarySections[section]}</React.Fragment>)}
      </View> : null}
    </View>
  </ScrollView>;
}
...
const styles = StyleSheet.create({
  ...
  hero: { minHeight: 300, paddingHorizontal: 20, paddingTop: 20, paddingBottom: 6, borderWidth: 1, borderRadius: 22, overflow: "hidden" }, heroDominant: { minHeight: 340 }, heroTopline: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }, balanceLabel: { fontSize: 10, lineHeight: 16, fontWeight: "800", letterSpacing: 1.7 }, heroState: { marginTop: 4, fontSize: 11, lineHeight: 16, fontWeight: "800", letterSpacing: 0.9 }, heroLiveDot: { width: 8, height: 8, borderRadius: 4, marginTop: 4 }, balance: { marginTop: 12, fontSize: 52, lineHeight: 59, fontWeight: "800", letterSpacing: -2.5, fontVariant: ["tabular-nums"] }, balanceTablet: { fontSize: 62, lineHeight: 69 }, balanceDominant: { fontSize: 58, lineHeight: 65 }, heroMeta: { marginTop: 10, flexDirection: "row", alignItems: "center", gap: 8, flexWrap: "wrap" }, pnl: { fontSize: 16, lineHeight: 22, fontWeight: "800", fontVariant: ["tabular-nums"] }, metaText: { fontSize: 12, lineHeight: 18, fontWeight: "600" }, metaDot: { width: 3, height: 3, borderRadius: 2 },
