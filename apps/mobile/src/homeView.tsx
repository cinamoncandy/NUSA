...
function exposureValue(account: Snapshot["portfolio"]["account"] | null): number | null {
  if (account == null) return null;
  const position = account.position;
  if (!position || !Number.isFinite(position.quantity) || !Number.isFinite(position.currentPrice)) return 0;
...
export function HomeView({
  ...
  const exposure = exposureValue(account);
  ...
  const openOrders = snapshot?.portfolio?.account?.openOrders?.length ?? snapshot?.orders?.filter((order) => order.status === "OPEN").length ?? null;

  return <View style={[styles.shell, { backgroundColor: theme.colors.background }]} testID="home-screen">
    <ScrollView
      contentContainerStyle={[styles.content, { maxWidth: tablet ? 980 : 680 }]}
      refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />}
      showsVerticalScrollIndicator={false}
    >
      <AuthorityRail
        detail="AUTONOMOUS INVESTMENT INTELLIGENCE · LIVE NONE · AI ZERO AUTHORITY"
        status={disconnected ? "NOT CONFIGURED" : readOnlyError ? "DEGRADED" : decisionSurface.statusLabel}
        tone={systemTone}
      ...
      <ScreenLead
        ...
        title={posture}
        detail={why}
        badge={disconnected ? "SETUP REQUIRED" : snapshot?.readyForPaperOperations ? "PAPER ACTIVE" : "OBSERVING"}
        badgeTone={disconnected ? "warning" : snapshot?.readyForPaperOperations ? "success" : "info"}
        testID="home-now"
      />

      {disconnected || readOnlyError ? <Pressable accessibilityRole="button" onPress={onGoSettings} testID="home-operational-notice">
        <StateNotice
          title={disconnected ? "PAPER 연결 필요" : "PAPER 연결 오류"}
          detail={`${disconnected ? "Cloud endpoint와 세션을 검증해야 합니다." : readOnlyError ?? "읽기 상태를 확인할 수 없습니다."} · 설정 열기`}
          tone="warning"
        />
      </Pressable> : null}
      ...
      <MetricStrip
        ...
        items={[
          { label: "EQUITY", value: krw(account?.equity) },
          { label: "TOTAL PNL", value: signedMoney(totalPnl), tone: totalPnl == null ? "neutral" : totalPnl >= 0 ? "success" : "danger" },
          { label: "CASH", value: krw(account?.cash) },
          { label: "EXPOSURE", value: krw(exposure) },
        ]}
      />
      ...
      <View style={[styles.twoColumn, tablet ? styles.twoColumnTablet : null]}>
        <IntelligenceSection title="판단 근거" kicker="WHY · AI INSIGHT" tone="primary" testID="ai-card" style={tablet ? styles.half : undefined} actionLabel={aiInsightAvailable ? "근거 보기" : undefined} onAction={aiInsightAvailable ? () => onNavigate("AiSignal") : undefined}>
          <Text style={[styles.primaryCopy, { color: theme.colors.text }]}>{why}</Text>
          <FactRow label="NOW" value={decisionSurface.now} />
          <FactRow label="RESULT" value={decisionSurface.result} />
          <FactRow label="AI AUTHORITY" value="ZERO AUTHORITY" tone="info" />
        </IntelligenceSection>
        ...
        <IntelligenceSection title="리스크 게이트" kicker="RISK STATUS" tone={riskTone} testID="home-risk-status" style={tablet ? styles.half : undefined}>
          <Text style={[styles.primaryCopy, { color: theme.colors.text }]}>{decisionSurface.risk}</Text>
          <FactRow label="RISK" value={rail.riskLabel} tone={riskTone} />
          <FactRow label="PAPER MODE" value={snapshot?.mode ?? (localPaperActive ? "LOCAL PAPER" : "UNAVAILABLE")} />
          <FactRow label="LIVE AUTHORITY" value="NONE" tone="success" />
        </IntelligenceSection>
      </View>
      ...
      <IntelligenceSection title="PAPER 운용 결과" kicker="PAPER PERFORMANCE" tone="success" testID="home-paper-performance" actionLabel="포트폴리오" onAction={() => onNavigate("Portfolio")}>
        ...
        <FactRow label="OPEN ORDERS" value={openOrders == null ? "—" : String(openOrders)} />
        <FactRow label="INVESTABLE CASH" value={krw(cashEnvelope?.investableCash)} />
        <FactRow label="RESERVED CASH" value={krw(cashEnvelope?.reservedCash)} tone="success" />
        <Text style={[styles.disclaimer, { color: theme.colors.textMuted }]}>{accountSource ? `${accountSource} PAPER source` : "PAPER source unavailable"} · REAL_READ_ONLY 자산과 합산하지 않습니다.</Text>
      </IntelligenceSection>
      ...
      <IntelligenceSection title="다음 학습 상태" kicker="LEARNING" tone="primary" testID="home-paper-learning" actionLabel="근거 보기" onAction={disconnected ? undefined : onOpenPaperLearning}>
        <Text style={[styles.primaryCopy, { color: theme.colors.text }]}>{decisionSurface.learning}</Text>
        <FactRow label="LEARNING" value={decisionSurface.learning} testID="home-supervisor-learning" />
        <FactRow label="RESULT BASIS" value={decisionSurface.result} />
      </IntelligenceSection>

      <View style={[styles.safetyFooter, { borderTopColor: theme.colors.border }]}>
...
const styles = StyleSheet.create({
  shell: { flex: 1 },
  content: { width: "100%", alignSelf: "center", paddingHorizontal: 18, paddingTop: 14, paddingBottom: 120, gap: 16 },
  statusRail: { minHeight: 32, flexDirection: "row", alignItems: "center", gap: 10, flexWrap: "wrap", paddingHorizontal: 2 },
  statusText: { fontSize: 10, lineHeight: 15, fontWeight: "700" },
  statusTextStrong: { fontSize: 10, lineHeight: 15, fontWeight: "900", letterSpacing: 0.5 },
  twoColumn: { gap: 16 },
  twoColumnTablet: { flexDirection: "row", alignItems: "stretch" },
  half: { flex: 1 },
  primaryCopy: { fontSize: 15, lineHeight: 23, fontWeight: "750" },
  disclaimer: { fontSize: 10, lineHeight: 16 },
