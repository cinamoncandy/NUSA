...
export function HomeView({ snapshot, investmentPercent, readOnlyError, notConfigured, refreshing, onRefresh, onGoSettings, onNavigate }: HomeViewProps) {
  ...
  const connectionLabel = snapshot?.health ?? (notConfigured ? "연결 필요" : "대기");
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="home-screen">
    <ScreenHeader eyebrow="PERSONAL PAPER" title="홈" description="계좌, PAPER 준비 상태와 읽기 전용 AI 신뢰도를 한눈에 확인합니다." statusLabel={connectionLabel} statusTone={snapshot ? healthTone(snapshot.health) : "warning"} />
    <View style={styles.authorityRow}><StatusChip label="PAPER ONLY" tone="primary" /><StatusChip label="LIVE NONE" tone="neutral" /></View>
    ...
    <View style={styles.metricGrid} testID="home-status-metrics">
      <MetricTile label="시장 연결" value={snapshot?.operations.transport ?? "연결 필요"} detail="PAPER transport" tone={snapshot?.operations.transport === "ONLINE" ? "success" : "warning"} />
      <MetricTile label="PAPER 준비" value={snapshot?.readyForPaperOperations ? "READY" : "대기/차단"} detail="실자금 실행 없음" tone={snapshot?.readyForPaperOperations ? "success" : "warning"} />
      <MetricTile label="AI 신뢰도" value={aiTrustedConfidence} detail="AI READ-ONLY" tone={ai?.calibrationStatus === "CALIBRATED" ? "info" : "default"} />
    </View>
    ...
    {snapshot ? <>
      ...
      <View style={styles.insight} testID="ai-card">
        <View style={styles.insightHeader}><View><Text style={[styles.kicker, { color: theme.colors.textMuted }]}>AI READ-ONLY</Text><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>오늘의 분석</Text></View><View style={styles.insightBadges}><StatusChip label="READ ONLY" tone="neutral" />{ai?.calibrationStatus === "CALIBRATED" ? <StatusChip label={aiTrustedConfidence} tone="info" /> : null}</View></View>
...
const styles = StyleSheet.create({
  ...
  authorityRow: { flexDirection: "row", gap: 7, flexWrap: "wrap" },
