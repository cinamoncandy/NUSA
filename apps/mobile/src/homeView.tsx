...
export function HomeView({ snapshot, investmentPercent, readOnlyError, notConfigured, refreshing, onRefresh, onGoSettings, onNavigate, onOpenPaperLearning }: HomeViewProps) {
  ...
  return <ScrollView contentContainerStyle={[styles.content, { maxWidth: tablet ? Math.max(profile.screen.maxWidth, 980) : profile.screen.maxWidth, paddingHorizontal: profile.screen.horizontalPadding, paddingTop: profile.screen.topPadding, paddingBottom: profile.screen.bottomPadding, gap: tablet ? 24 : 18 }]} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="home-screen">
