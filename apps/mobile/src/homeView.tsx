...
export function HomeView({ snapshot, readOnlyError, notConfigured, refreshing, onRefresh, onGoSettings, onNavigate }: HomeViewProps) {
  ...
  const nextAction = snapshot == null ? null
    : snapshot.health !== "HEALTHY" || snapshot.dashboard.killSwitchActive || !snapshot.readyForPaperOperations
      ? { title: "안전 상태를 먼저 확인하세요", detail: "차단·저하 또는 PAPER 운영 대기 상태에서는 주문보다 운영 상태 확인이 우선입니다.", label: "PAPER 상태 보기", destination: "Trade" as const, tone: "warning" as const }
      : aiInsightAvailable
        ? { title: "최신 AI 분석이 있습니다", detail: "검증된 분석과 근거를 읽기 전용으로 확인할 수 있습니다.", label: "AI 분석 보기", destination: "More" as const, tone: "info" as const }
        : { title: "시장 관찰을 이어가세요", detail: "AI 분석이 없을 때는 수신된 시장 데이터와 연결 상태부터 확인합니다.", label: "시장 보기", destination: "Markets" as const, tone: "primary" as const };
...
const styles = StyleSheet.create({
  ...
  hero: { paddingVertical: 8, gap: 6 }, heroTop: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }, heroLabel: { fontSize: 10, fontWeight: "800", letterSpacing: 1.2 }, heroValue: { marginTop: 5, fontSize: 42, lineHeight: 49, fontWeight: "800", letterSpacing: -1.8, fontVariant: ["tabular-nums"] }, heroPnl: { fontSize: 15, lineHeight: 21, fontWeight: "700", fontVariant: ["tabular-nums"] },
  metrics: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, grid: { flexDirection: "row", flexWrap: "wrap", gap: 14, alignItems: "flex-start" }, column: { flexGrow: 1, flexBasis: 440 },
