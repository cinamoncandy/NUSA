...
export function HomeView({ snapshot, investmentPercent, readOnlyError, notConfigured, refreshing, publicMarkets, onRefresh, onGoSettings, onNavigate, onOpenPaperLearning }: HomeViewProps) {
  ...
  const aiReady = decisionSurface.aiInsightAvailable;
  const marketRows = selectHomeMarketData(publicMarkets, snapshot?.markets ?? []).slice(0, 3);
  const stable = decisionSurface.attention === "QUIET";
  ...
  return <ScrollView testID="home-screen" contentContainerStyle={[styles.content, { maxWidth: tablet ? 920 : 620 }]} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={mint} />}>
    <View style={styles.masterRail} testID="home-master-rail">
      <View><Text style={[styles.brand, { color: theme.colors.text }]}>NUSA</Text><Text style={[styles.brandSub, { color: theme.colors.textMuted }]}>AI SUPERVISORY OS</Text></View>
    ...
    <View style={[styles.hero, { backgroundColor: theme.colors.surface, borderColor: stable ? theme.colors.borderStrong : theme.colors.warning }]} testID="home-supervisor-summary">
      <View style={styles.heroCopy}>
        ...
        <Text style={[styles.heroSubtitle, { color: stable ? mint : theme.colors.warning }]}>{stable ? "No Action Needed." : decisionSurface.now}</Text>
        <Text style={[styles.heroBody, { color: theme.colors.textMuted }]} testID="home-supervisor-now">{decisionSurface.now}</Text>
        <Pressable onPress={runPrimaryAction} style={[styles.detailButton, { borderColor: theme.colors.borderStrong }]} testID="home-supervisor-primary-action"><Text style={[styles.detailButtonText, { color: theme.colors.text }]}>{decisionSurface.primaryLabel} →</Text></Pressable>
      </View>
      <View style={styles.orbWrap} accessibilityLabel="NUSA white mint supervisory orb"><View style={[styles.orbOuter, { borderColor: mint, shadowColor: purple }]}><View style={[styles.orbInner, { borderColor: blue, backgroundColor: theme.colors.surfaceRaised }]} /></View></View>
      <View style={styles.stateRail}>{["STABLE", "WATCH", "CAUTION", "RISK", "ACTION"].map((label, index) => <View key={label} style={styles.stateRow}><View style={[styles.stateDot, { borderColor: index === 0 && stable ? mint : theme.colors.borderStrong, backgroundColor: index === 0 && stable ? mint : theme.colors.surface }]} /><Text style={[styles.stateLabel, { color: index === 0 && stable ? mint : theme.colors.textMuted }]}>{label}</Text></View>)}</View>
    </View>

    <View style={[styles.truthStrip, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
      <View style={styles.truthItem} testID="home-supervisor-why"><Text style={[styles.metricLabel, { color: theme.colors.textMuted }]}>WHY</Text><Text style={[styles.truthText, { color: theme.colors.text }]} numberOfLines={2}>{supervisorWhy}</Text></View>
      <View style={styles.truthItem} testID="home-supervisor-result"><Text style={[styles.metricLabel, { color: theme.colors.textMuted }]}>RESULT</Text><Text style={[styles.truthText, { color: theme.colors.text }]} numberOfLines={2}>{supervisorResult}</Text></View>
      <View style={styles.truthItem} testID="home-supervisor-risk"><Text style={[styles.metricLabel, { color: theme.colors.textMuted }]}>RISK</Text><Text style={[styles.truthText, { color: stable ? mint : theme.colors.warning }]} numberOfLines={2}>{supervisorRisk}</Text></View>
    </View>
    ...
    <View style={styles.marketGrid} testID="home-market-pulse">{[0,1,2].map((i) => { const m = marketRows[i]; return <Pressable key={m?.market ?? i} onPress={() => onNavigate("Markets")} style={[styles.marketCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}><Text style={[styles.marketSymbol, { color: theme.colors.text }]}>{m?.market ?? "—"}</Text><Text style={[styles.marketPrice, { color: theme.colors.text }]}>{m ? money(m.price) : "—"}</Text><Text style={[styles.marketChange, { color: m?.changeRate == null ? theme.colors.textMuted : m.changeRate >= 0 ? mint : theme.colors.danger }]}>{pct(m?.changeRate ?? null)}</Text></Pressable>; })}</View>
    ...
    <Pressable onPress={() => onNavigate("Portfolio")} style={[styles.capitalCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]} testID="account-hero-card">
      <View><Text style={[styles.metricLabel,{color:theme.colors.textMuted}]}>TOTAL ASSETS</Text><Text style={[styles.assetValue,{color:theme.colors.text}]}>{money(account?.equity)}</Text></View>
      ...
      <View testID="home-investable-cash"><Text style={[styles.metricLabel,{color:theme.colors.textMuted}]}>INVESTABLE</Text><Text style={[styles.metricValue,{color:theme.colors.text}]}>{money(cashEnvelope?.investableCash)}</Text></View>
      <View testID="home-reserved-cash"><Text style={[styles.metricLabel,{color:theme.colors.textMuted}]}>CASH SHIELD</Text><Text style={[styles.metricValue,{color:theme.colors.text}]}>{money(cashEnvelope?.reservedCash)}</Text></View>
    </Pressable>
    ...
    <View style={[styles.judgmentCard,{backgroundColor:theme.colors.surface,borderColor:theme.colors.border}]} testID="ai-card">
      ...
      <Text style={[styles.bias,{color:aiReady ? mint : theme.colors.textMuted}]}>{aiReady ? "VERIFIED" : "NEUTRAL"} ●</Text>
      <Text style={[styles.judgmentBody,{color:theme.colors.text}]}>{supervisorWhy}</Text>
      <View style={styles.judgmentMetrics}><View><Text style={[styles.metricLabel,{color:theme.colors.textMuted}]}>근거</Text><Text style={[styles.metricValue,{color:theme.colors.text}]}>{aiReady ? ai?.evidenceReferences.length : 0}</Text></View><View><Text style={[styles.metricLabel,{color:theme.colors.textMuted}]}>신뢰도</Text><Text style={[styles.metricValue,{color:theme.colors.text}]}>{decisionSurface.calibratedConfidence ?? "—"}</Text></View><View><Text style={[styles.metricLabel,{color:theme.colors.textMuted}]}>CALIBRATION</Text><Text style={[styles.metricValue,{color:ai?.calibrationStatus === "CALIBRATED" ? mint : theme.colors.textMuted}]}>{ai?.calibrationStatus ?? "PENDING"}</Text></View></View>
    </View>
    ...
    <View style={styles.evidenceRow} testID="home-supervisor-learning">{["거시 환경","시장 구조","온체인 데이터","심리 지표"].map((x) => <View key={x} style={[styles.evidenceCard,{backgroundColor:theme.colors.surface,borderColor:theme.colors.border}]}><Text style={[styles.evidenceTitle,{color:theme.colors.text}]}>{x}</Text><Text style={[styles.metricSub,{color:theme.colors.textMuted}]}>{decisionSurface.learning}</Text></View>)}</View>
    ...
    <View style={styles.commandRow}><Pressable onPress={() => onNavigate("Markets")} style={[styles.command,{backgroundColor:theme.colors.surface,borderColor:theme.colors.border}]}><Text style={[styles.commandText,{color:theme.colors.text}]}>모니터링 강화</Text></Pressable><Pressable onPress={() => onNavigate("AiSignal")} style={[styles.command,{backgroundColor:theme.colors.surface,borderColor:theme.colors.border}]}><Text style={[styles.commandText,{color:theme.colors.text}]}>시나리오 분석</Text></Pressable><Pressable onPress={() => onNavigate("Portfolio")} style={[styles.command,{backgroundColor:theme.colors.primarySoft,borderColor:theme.colors.borderStrong}]}><Text style={[styles.commandText,{color:theme.colors.text}]}>수동 개입</Text></Pressable></View>

    <View style={[styles.bottomNav,{backgroundColor:theme.colors.navSurface,borderColor:theme.colors.border}]} testID="home-reference-navigation"><View style={styles.navItem}><Text style={[styles.navActive,{color:theme.colors.text}]}>●</Text><Text style={[styles.navText,{color:theme.colors.text}]}>NOW</Text></View><Pressable style={styles.navItem} onPress={() => onNavigate("Markets")}><Text style={[styles.navText,{color:theme.colors.textMuted}]}>▥</Text><Text style={[styles.navText,{color:theme.colors.textMuted}]}>MARKET</Text></Pressable><Pressable style={[styles.nusaButton,{backgroundColor:theme.colors.primarySoft,borderColor:purple}]} onPress={() => onNavigate("AiSignal")}><Text style={[styles.nusaMark,{color:theme.colors.text}]}>✦</Text><Text style={[styles.navText,{color:theme.colors.text}]}>NUSA</Text></Pressable><Pressable style={styles.navItem} onPress={() => onNavigate("Portfolio")}><Text style={[styles.navText,{color:theme.colors.textMuted}]}>◔</Text><Text style={[styles.navText,{color:theme.colors.textMuted}]}>ASSETS</Text></Pressable><Pressable style={styles.navItem} onPress={onGoSettings}><Text style={[styles.navText,{color:theme.colors.textMuted}]}>☷</Text><Text style={[styles.navText,{color:theme.colors.textMuted}]}>CONTROL</Text></Pressable></View>
    <Text style={[styles.safety,{color:theme.colors.textMuted}]}>PAPER ONLY · LIVE NONE · AI ZERO AUTHORITY</Text>
    <Text style={[styles.safety,{color:theme.colors.textMuted}]}>AI ZERO AUTHORITY · productionMutationAllowed=false · liveAuthority=NONE</Text>
  </ScrollView>;
}

const styles = StyleSheet.create({
  content:{width:"100%",alignSelf:"center",paddingHorizontal:18,paddingTop:14,paddingBottom:40,gap:14},
  masterRail:{flexDirection:"row",alignItems:"center",justifyContent:"space-between"}, brand:{fontSize:34,fontWeight:"700",letterSpacing:7},brandSub:{fontSize:10,fontWeight:"700",letterSpacing:1.6,marginTop:2},avatar:{width:42,height:42,borderRadius:21,alignItems:"center",justifyContent:"center"},avatarText:{fontSize:18,fontWeight:"700"},
