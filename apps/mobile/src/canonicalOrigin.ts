...
function normalizeCanonicalOrigin(value: string): string {
  ...
  if (host.endsWith(".trycloudflare.com")) throw new Error("temporary Cloudflare Quick Tunnel origins are not canonical production origins");
  return url.origin;
}

export function readCanonicalNusaOrigin(): string {
  return normalizeCanonicalOrigin(CANONICAL_NUSA_ORIGIN);
}

export function tryReadCanonicalNusaOrigin(): string | null {
  try { return readCanonicalNusaOrigin(); } catch { return null; }
}
