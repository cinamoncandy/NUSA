...
export function PortfolioView({ snapshot, investmentPercent, error, refreshing, onRefresh, upbitSnapshot = null, upbitStatus = "DISCONNECTED", upbitError = null, onOpenPaperLearning }: PortfolioViewProps) {
  ...
  return <ScrollView style={{ backgroundColor: theme.colors.background }} contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} showsVerticalScrollIndicator={false} testID="portfolio-screen">
    ...
    <MetricStrip testID="portfolio-supervisor-summary" items={[{ label: "EQUITY", value: money(model?.totalEquity) }, { label: "TOTAL PNL", value: signedMoney(model?.totalPnl), tone: model == null ? "neutral" : model.totalPnl >= 0 ? "success" : "danger" }, { label: "CASH", value: money(model?.cash) }, { label: "EXPOSURE", value: money(model?.assetValue) }]} />
    {model && allocation ? <IntelligenceSection title="자본 배분" kicker="CAPITAL LIMITS" tone="primary" testID="portfolio-allocation-rail">
      <FactRow label="INVESTMENT LIMIT" value={`${allocation.investmentPercent}%`} /><FactRow label="INVESTABLE CASH" value={money(allocation.investableCash)} testID="portfolio-investable-cash" /><FactRow label="RESERVED CASH" value={money(allocation.reservedCash)} tone="success" testID="portfolio-reserved-cash" />
...
const styles = StyleSheet.create({ content: { paddingHorizontal: 20, paddingTop: 14, paddingBottom: 120, gap: 16, width: "100%", maxWidth: 720, alignSelf: "center" }, allocationRail: { height: 8, borderRadius: 999, overflow: "hidden" }, allocationFill: { height: "100%", borderRadius: 999 }, note: { fontSize: 10, lineHeight: 16 }, footer: { textAlign: "center", fontSize: 9, lineHeight: 14, fontWeight: "900", letterSpacing: 1.05, paddingTop: 4 } });
