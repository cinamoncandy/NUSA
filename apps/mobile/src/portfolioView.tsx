...
export function PortfolioView({ snapshot, investmentPercent, error, refreshing, onRefresh }: PortfolioViewProps) {
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="portfolio-screen">
    <ScreenHeader eyebrow="PAPER PORTFOLIO" title="자산" description="계정 평가액, 손익과 투자 가능한 현금·보호 현금을 구분해 확인합니다." statusLabel={`투자 ${allocation.investmentPercent}%`} statusTone="primary" />
    ...
    <View style={styles.metricGrid}>
      <MetricTile label="거래소 현금" value={money(model.cash)} detail="현재 PAPER 현금" tone="default" testID="portfolio-cash" />
      <MetricTile label="투자 가능 현금" value={money(allocation.investableCash)} detail={`${allocation.investmentPercent}% 신규 매수 한도`} tone="primary" testID="portfolio-investable-cash" />
      <MetricTile label="미투자 보호 현금" value={money(allocation.reservedCash)} detail={`${allocation.reservePercent}% 보호`} tone="info" testID="portfolio-reserved-cash" />
