...
export function PortfolioView({ snapshot, investmentPercent, error, refreshing, onRefresh, upbitSnapshot = null, upbitStatus = "DISCONNECTED", upbitError = null, onOpenPaperLearning }: PortfolioViewProps) {
  ...
  return <ScrollView style={{ backgroundColor: theme.colors.background }} contentContainerStyle={[styles.content, { maxWidth: tablet ? 1080 : 720 }]} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} showsVerticalScrollIndicator={false} testID="portfolio-screen">
    ...
    {model && allocation ? <IntelligenceSection style={tablet ? styles.column : undefined} title="자본 배분" kicker="CAPITAL LIMITS" tone="primary" testID="portfolio-allocation-rail">
    ...
    <IntelligenceSection style={tablet ? styles.column : undefined} title="현재 포지션" kicker="PAPER EXPOSURE" tone={position ? "info" : "neutral"} testID={position ? "portfolio-position" : "portfolio-empty"}>
    ...
    <View style={tablet ? styles.columns : styles.stack}>
    <IntelligenceSection style={tablet ? styles.column : undefined} title="실행 결과" kicker="PAPER ACCOUNTING" tone="success" testID="portfolio-account-breakdown">
    ...
    <IntelligenceSection style={tablet ? styles.column : undefined} title="실거래소 기준선" kicker="REAL_READ_ONLY · REFERENCE" tone="info" testID="portfolio-upbit-read-only">
