...
import { buildOrderHistoryViewModel, type OrderHistoryFilter, type OrderHistorySort } from "./orderHistory";
...
export function OrderHistoryView({ rawOrders, error, refreshing, onRefresh }: OrderHistoryViewProps) {
  ...
  const model = useMemo(() => buildOrderHistoryViewModel({ rawOrders, query, filter, sort, page }), [filter, page, query, rawOrders, sort]);
