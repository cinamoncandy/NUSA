...
function AiState({ title, detail, testID, retry, loading = false }: Readonly<{ title: string; detail: string; testID: string; retry?: () => void; loading?: boolean }>) {
  ...
  return <View style={styles.state} testID={testID}><NusaCard>{loading ? <ActivityIndicator color={theme.colors.primary} /> : null}<Text style={[styles.stateTitle, { color: theme.colors.text }]}>{title}</Text><Text style={[styles.body, { color: theme.colors.textMuted }]}>{detail}</Text>{retry ? <NusaButton label="다시 불러오기" onPress={retry} /> : null}</NusaCard></View>;
}
...
export function AiView({ ai, research, health, liveAuthority, productionMutationAllowed, killSwitchActive, error, refreshing, onRefresh }: AiViewProps) {
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="ai-screen">
    ...
    <View style={[styles.aiGrid, isTablet && styles.aiGridWide]} testID={isTablet ? "ai-tablet-grid" : "ai-mobile-stack"}>
      <View style={styles.aiColumn}>
        ...
        <View style={styles.authoritySummary} testID="ai-authority-summary"><View style={styles.statusRow}><StatusChip label="AI ZERO AUTHORITY" tone="info" /><StatusChip label="READ ONLY" tone="primary" /></View><Text style={[styles.authorityCopy, { color: theme.colors.textMuted }]}>AI에는 주문·이체·LIVE 실행 권한이 없습니다. PAPER 주문 경로와 분리됩니다.</Text></View>
      </View>
      ...
      <View style={styles.aiColumn}>
        <NusaCard testID="ai-evidence-card">
          ...
          {ai && ai.disagreements.length > 0 ? <><Text style={[styles.label, styles.sectionGap, { color: theme.colors.textMuted }]}>분석 간 불일치</Text>{ai.disagreements.slice(0, 5).map((item, index) => <Text key={`${index}-${item}`} style={[styles.evidence, { color: theme.colors.text }]} numberOfLines={3}>• {item}</Text>)}</> : null}
        </NusaCard>
        <NusaCard testID="ai-research-card"><View style={styles.cardHeader}><Text style={[styles.cardTitle, { color: theme.colors.text }]}>리서치 상태</Text><StatusChip label={research?.health ?? "UNAVAILABLE"} tone={research?.health === "HEALTHY" ? "success" : research?.health === "FAIL_CLOSED" ? "danger" : "warning"} /></View><DataRow label="현재 PAPER 전략" value={research?.champion.strategyId ?? "-"} /><DataRow label="현재 전략 권한" value={research?.champion.authority ?? "-"} emphasis /><DataRow label="검증 후보 전략" value={research?.challenger.strategyId ?? "-"} /><DataRow label="후보 전략 권한" value={research?.challenger.authority ?? "-"} emphasis /><DataRow label="연구 후보" value={research == null ? "-" : String(research.candidateCount)} /><DataRow label="실험" value={research == null ? "-" : String(research.experimentCount)} /></NusaCard>
        <NusaCard testID="ai-authority-card"><View style={styles.cardHeader}><Text style={[styles.cardTitle, { color: theme.colors.text }]}>운영 경계</Text><StatusChip label={health ?? "UNKNOWN"} tone={health === "HEALTHY" ? "success" : "warning"} /></View><DataRow label="AI LIVE 권한" value={liveAuthority ?? "-"} emphasis /><DataRow label="Production mutation" value={productionMutationAllowed == null ? "-" : "금지"} tone={productionMutationAllowed === false ? "success" : "default"} /><DataRow label="킬 스위치" value={killSwitchActive == null ? "-" : killSwitchActive ? "활성" : "비활성"} tone={killSwitchActive === true ? "danger" : killSwitchActive === false ? "success" : "default"} /><Text style={[styles.body, { color: theme.colors.textMuted }]}>서버 snapshot이 연결되면 실제 권한 불변식과 안전 상태를 이 진단 영역에 표시합니다.</Text></NusaCard>
      </View>
    </View>
  </ScrollView>;
}
...
const styles = StyleSheet.create({
  ...
  aiGrid: { gap: 14 },
  aiGridWide: { flexDirection: "row", alignItems: "flex-start" },
  aiColumn: { flex: 1, minWidth: 0, gap: 14 }
});
