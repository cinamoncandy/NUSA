...
export function AiView({ ai, research, health, liveAuthority, productionMutationAllowed, killSwitchActive, error, refreshing, onRefresh }: AiViewProps) {
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="ai-screen">
    ...
    <View style={styles.detailGrid}><View style={styles.detailCell}><NusaCard testID="ai-diagnostics-card"><View style={styles.cardHeader}><View><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>DETAILS</Text><Text style={[styles.cardTitle, { color: theme.colors.text }]}>신뢰도 진단</Text></View></View><Text style={[styles.body, { color: theme.colors.textMuted }]}>원시 모델 확률은 미보정 모델 출력이며, 검증된 성공 확률이나 성과 보장이 아닙니다. CALIBRATED일 때만 별도의 검증 신뢰도와 보정 확률을 표시합니다.</Text><DataRow label="보정 상태" value={ai?.calibrationStatus ?? "UNKNOWN"} /><DataRow label="보정 표본" value={ai?.calibrationSampleCount == null ? "-" : String(ai.calibrationSampleCount)} /><DataRow label="보정 확률" value={calibratedProbability} /><DataRow label="원시 모델 확률 (미보정)" value={rawProbability} /><DataRow label="ECE" value={metric(ai?.calibrationExpectedError)} /><DataRow label="Brier" value={metric(ai?.calibrationBrierScore)} /><DataRow label="불확실성" value={ai?.uncertainty ?? "-"} /><DataRow label="비판 위험도" value={ai?.criticSeverity ?? "-"} tone={severityTone(ai?.criticSeverity ?? null)} /><DataRow label="설명 신뢰성 검증" value={ai?.explanationVerdict ?? "NOT_EVALUATED"} tone={ai?.explanationVerdict === "PASS" ? "default" : ai?.explanationVerdict === "ABSTAIN" ? "warning" : "default"} /><DataRow label="참고한 과거 사례" value={ai?.recentLessonCount == null ? "-" : String(ai.recentLessonCount)} /><DataRow label="시나리오 강건성" value={ai?.scenarioRobustnessState ?? "NOT_EVALUATED"} tone={ai?.scenarioRobustnessState === "ROBUST" ? "default" : ai?.scenarioRobustnessState === "SENSITIVE" || ai?.scenarioRobustnessState === "CONTRADICTORY" ? "warning" : "default"} /></NusaCard></View>
    ...
    {/* v5 (docs/NUSA_MOBILE_UIUX_V5_OBSIDIAN_FINANCE.md §9): authority boundary is the last
        step, cleanly after detail/diagnostics -- not interrupting the run between evidence
        and diagnostics. */}
