...
export function AiView({ ai, research, health, liveAuthority, productionMutationAllowed, killSwitchActive, error, refreshing, onRefresh }: AiViewProps) {
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="ai-screen">
    ...
    <View style={styles.metricGrid}>
      <MetricTile label="신뢰 수준" value={trustedConfidence} detail={ai?.calibrationStatus === "CALIBRATED" ? "보정 완료" : "보정 필요"} tone={ai?.calibrationStatus === "CALIBRATED" ? "success" : "warning"} testID="ai-trusted-confidence" />
    ...
    <InlineNotice title="AI 분석은 판단 보조입니다" detail="성과를 보장하지 않으며, PAPER·LIVE 주문, 이체, 출금 또는 운영 변경 권한이 없습니다." tone="info" />

    <View style={styles.detailGrid}><View style={styles.detailCell}><NusaCard testID="ai-diagnostics-card"><View style={styles.cardHeader}><View><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>DETAILS</Text><Text style={[styles.cardTitle, { color: theme.colors.text }]}>신뢰도 진단</Text></View></View><DataRow label="보정 상태" value={ai?.calibrationStatus ?? "UNKNOWN"} /><DataRow label="보정 확률" value={calibratedProbability} /><DataRow label="원시 모델 확률" value={rawProbability} /><DataRow label="ECE" value={metric(ai?.calibrationExpectedError)} /><DataRow label="Brier" value={metric(ai?.calibrationBrierScore)} /><DataRow label="불확실성" value={ai?.uncertainty ?? "-"} /><DataRow label="비판 위험도" value={ai?.criticSeverity ?? "-"} tone={severityTone(ai?.criticSeverity ?? null)} /></NusaCard></View>

    <View style={styles.detailCell}><NusaCard testID="ai-research-card"><View style={styles.cardHeader}><View><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>RESEARCH</Text><Text style={[styles.cardTitle, { color: theme.colors.text }]}>리서치 상태</Text></View><StatusChip label={research?.health ?? "UNAVAILABLE"} tone={research?.health === "HEALTHY" ? "success" : research?.health === "FAIL_CLOSED" ? "danger" : "warning"} /></View><DataRow label="현재 PAPER 전략" value={research?.champion.strategyId ?? "-"} /><DataRow label="현재 전략 권한" value={research?.champion.authority ?? "-"} emphasis /><DataRow label="검증 후보 전략" value={research?.challenger.strategyId ?? "-"} /><DataRow label="연구 후보" value={research == null ? "-" : String(research.candidateCount)} /><DataRow label="실험" value={research == null ? "-" : String(research.experimentCount)} /></NusaCard></View></View>

    <NusaCard testID="ai-authority-card"><View style={styles.cardHeader}><View><Text style={[styles.eyebrow, { color: theme.colors.info }]}>SAFETY BOUNDARY</Text><Text style={[styles.cardTitle, { color: theme.colors.text }]}>AI 권한</Text></View><StatusChip label={health ?? "UNKNOWN"} tone={health === "HEALTHY" ? "success" : "warning"} /></View><DataRow label="LIVE 권한" value={liveAuthority ?? "-"} emphasis /><DataRow label="Production mutation" value={productionMutationAllowed == null ? "-" : "금지"} tone={productionMutationAllowed === false ? "success" : "default"} /><DataRow label="킬 스위치" value={killSwitchActive == null ? "-" : killSwitchActive ? "활성" : "비활성"} tone={killSwitchActive === true ? "danger" : killSwitchActive === false ? "success" : "default"} /></NusaCard>
  </ScrollView>;
}

const styles = StyleSheet.create({ content: { paddingHorizontal: 20, paddingTop: 20, gap: 20, paddingBottom: 44, width: "100%", maxWidth: 980, alignSelf: "center" }, state: { flex: 1, justifyContent: "center", padding: 20, alignItems: "center" }, stateInner: { width: "100%", maxWidth: 720, gap: 12 }, hero: { paddingVertical: 10, gap: 13 }, heroTop: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }, eyebrow: { fontSize: 10, lineHeight: 15, fontWeight: "800", letterSpacing: 1.1 }, heroTitle: { marginTop: 4, fontSize: 20, lineHeight: 26, fontWeight: "800", letterSpacing: -0.4 }, thesis: { fontSize: 25, lineHeight: 35, fontWeight: "700", letterSpacing: -0.75 }, heroMeta: { flexDirection: "row", flexWrap: "wrap", gap: 12 }, meta: { fontSize: 11, lineHeight: 17, fontWeight: "600" }, metricGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, evidenceSection: { gap: 14, paddingVertical: 4 }, sectionHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }, sectionTitle: { marginTop: 4, fontSize: 19, lineHeight: 25, fontWeight: "800", letterSpacing: -0.4 }, evidenceGrid: { flexDirection: "row", flexWrap: "wrap", gap: 20 }, evidenceColumn: { flexGrow: 1, flexBasis: 300 }, columnLabel: { fontSize: 11, lineHeight: 16, fontWeight: "800", letterSpacing: 0.7, marginBottom: 7 }, evidence: { fontSize: 13, lineHeight: 20, marginBottom: 6 }, disagreement: { paddingTop: 4 }, detailGrid: { flexDirection: "row", flexWrap: "wrap", gap: 14, alignItems: "flex-start" }, detailCell: { flexGrow: 1, flexBasis: 400, gap: 14 }, cardHeader: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 10 }, cardTitle: { marginTop: 4, fontSize: 18, fontWeight: "700", letterSpacing: -0.4 }, body: { fontSize: 13, lineHeight: 20, marginTop: 8 } });
