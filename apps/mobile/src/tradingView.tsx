...
interface TradingViewProps { readonly snapshot: PortfolioAccountResponse | null; readonly investmentPercent: number; readonly marketConnectionState: string; readonly stale: boolean; readonly error: string | null; readonly refreshing: boolean; readonly onRefresh: () => void; readonly onSubmit?: (draft: TradingDraft) => void; }
...
export function TradingView({ snapshot, investmentPercent, marketConnectionState, stale, error, refreshing, onRefresh, onSubmit }: TradingViewProps) {
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={onRefresh} />} testID="trading-screen">
    ...
    <View style={styles.ticket} testID="paper-order-ticket">
      ...
      <View style={styles.ticketSection}><Text style={[styles.stepLabel, { color: theme.colors.textMuted }]}>03 · 주문 입력</Text>{orderType === "LIMIT" ? <NusaTextField autoCorrect={false} editable={!submitting} keyboardType="decimal-pad" label="지정 가격" value={priceInput} onChangeText={(value) => { setPriceInput(value); setConfirming(false); }} placeholder="KRW 가격" returnKeyType="done" /> : null}<NusaTextField autoCorrect={false} editable={!submitting} keyboardType="decimal-pad" label={`수량 (${tradingAssetCode(model.market)})`} value={quantityInput} onChangeText={(value) => { setQuantityInput(value); setConfirming(false); }} placeholder="수량" returnKeyType="done" /></View>

      {side === "BUY" ? <View style={[styles.allocationPanel, { borderColor: theme.colors.border, backgroundColor: theme.colors.surfaceSunken }]} testID="paper-allocation-panel"><View style={styles.allocationTop}><View><Text style={[styles.eyebrow, { color: theme.colors.textMuted }]}>BUYING POWER</Text><Text style={[styles.allocationValue, { color: theme.colors.text }]}>{formatTradingAmount(cashEnvelope.investableCash, "KRW")}</Text></View><Text style={[styles.allocationPercent, { color: theme.colors.primary }]}>{cashEnvelope.investmentPercent}%</Text></View><View style={[styles.allocationTrack, { backgroundColor: theme.colors.border }]}><View style={[styles.allocationFill, { width: `${allocationRatio}%`, backgroundColor: theme.colors.primary }]} /></View><View style={styles.allocationAmounts}><Text style={[styles.smallCopy, { color: theme.colors.textMuted }]}>거래소 현금 {formatTradingAmount(snapshot.account.cash, "KRW")}</Text><Text style={[styles.smallCopy, { color: theme.colors.textMuted }]}>보호 {formatTradingAmount(cashEnvelope.reservedCash, "KRW")}</Text></View></View> : <InlineNotice title="매도는 투자비중에 의해 차단되지 않습니다" detail={`현재 보유 ${snapshot.account.position.quantity} ${tradingAssetCode(model.market)} 범위에서 PAPER 매도를 검증합니다.`} tone="info" />}

      <View style={styles.ticketSection}><Text style={[styles.stepLabel, { color: theme.colors.textMuted }]}>04 · 주문 미리보기</Text><View style={[styles.preview, { borderColor: theme.colors.border }]}><DataRow label="예상 주문 금액" value={formatTradingAmount(model.estimatedNotional, "KRW")} emphasis /><DataRow label={side === "BUY" ? "투자 한도 내 사용 가능" : "사용 가능"} value={formatTradingAmount(model.availableAmount, model.availableUnit)} />{side === "BUY" ? <DataRow label="주문과 무관하게 보호" value={formatTradingAmount(cashEnvelope.reservedCash, "KRW")} tone="success" /> : null}</View></View>

      {side === "BUY" && cashEnvelope.investmentPercent === 0 ? <InlineNotice title="신규 매수 비중이 0%입니다" detail="현금 전액을 보호하고 있습니다. 설정에서 투자 비중을 높이기 전까지 신규 매수는 차단되며 매도는 계속 가능합니다." tone="warning" /> : null}
      {!submitAvailable ? <InlineNotice title="PAPER 주문 연결 필요" detail="설정에서 Cloud endpoint와 메모리 세션 토큰을 검증하면 주문 컨트롤이 활성화됩니다." tone="warning" /> : null}
      {model.validationErrors.length > 0 || model.blockedReasons.length > 0 ? <InlineNotice title="현재 주문을 전송할 수 없습니다" detail={[...model.validationErrors, ...model.blockedReasons].join(" · ")} tone="warning" /> : null}

      <View style={styles.ticketSection}><Text style={[styles.stepLabel, { color: theme.colors.textMuted }]}>05 · 확인</Text>{!confirming ? <NusaButton label={submitEnabled ? `${side === "BUY" ? "매수" : "매도"} PAPER 주문 검토` : "PAPER 주문 사용 불가"} disabled={!submitEnabled} onPress={requestSubmit} /> : <View style={[styles.confirmPanel, { borderColor: theme.colors.primary, backgroundColor: theme.colors.surfaceSunken }]}><Text style={[styles.confirmTitle, { color: theme.colors.text }]}>이 PAPER 주문을 전송할까요?</Text><Text style={[styles.confirmCopy, { color: theme.colors.textMuted }]}>{model.market} · {side} · {orderType} · {quantityInput || "-"} {tradingAssetCode(model.market)}</Text>{side === "BUY" ? <Text style={[styles.confirmCopy, { color: theme.colors.textMuted }]}>투자 {cashEnvelope.investmentPercent}% · 보호 {cashEnvelope.reservePercent}%</Text> : null}<View style={styles.confirmActions}><NusaButton label={submitting ? "전송 중..." : "PAPER 주문 확정"} disabled={submitting} onPress={() => void submitBuiltIn()} /><NusaButton label="취소" disabled={submitting} onPress={() => setConfirming(false)} tone="neutral" /></View></View>}</View>
      {submitMessage ? <InlineNotice title="PAPER 주문 결과" detail={submitMessage} tone={submitMessage.includes("완료") ? "success" : "info"} /> : null}
    </View>
  </ScrollView>;
}

const styles = StyleSheet.create({ content: { paddingHorizontal: 20, paddingTop: 20, gap: 18, paddingBottom: 40, width: "100%", maxWidth: 820, alignSelf: "center" }, state: { flex: 1, justifyContent: "center", padding: 20, gap: 14, alignItems: "center" }, stateInner: { width: "100%", maxWidth: 720, gap: 12 }, stateTitle: { fontSize: 18, fontWeight: "700" }, quoteHero: { paddingVertical: 6, gap: 6 }, quoteTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }, statusRow: { flexDirection: "row", gap: 7, flexWrap: "wrap", justifyContent: "flex-end" }, eyebrow: { fontSize: 10, lineHeight: 15, fontWeight: "800", letterSpacing: 1.2 }, market: { marginTop: 4, fontSize: 18, lineHeight: 23, fontWeight: "800" }, price: { fontSize: 40, lineHeight: 47, fontWeight: "800", letterSpacing: -1.5, fontVariant: ["tabular-nums"] }, quoteMeta: { fontSize: 11, lineHeight: 17 }, ticket: { gap: 20 }, ticketSection: { gap: 9 }, stepLabel: { fontSize: 10, lineHeight: 15, fontWeight: "800", letterSpacing: 1.15 }, allocationPanel: { borderWidth: 1, borderRadius: 16, padding: 16, gap: 12 }, allocationTop: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }, allocationValue: { marginTop: 5, fontSize: 25, lineHeight: 31, fontWeight: "800", letterSpacing: -0.8, fontVariant: ["tabular-nums"] }, allocationPercent: { fontSize: 18, lineHeight: 24, fontWeight: "800", fontVariant: ["tabular-nums"] }, allocationTrack: { height: 6, borderRadius: 3, overflow: "hidden" }, allocationFill: { height: "100%", borderRadius: 3 }, allocationAmounts: { flexDirection: "row", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }, smallCopy: { fontSize: 11, lineHeight: 17, fontVariant: ["tabular-nums"] }, preview: { borderTopWidth: 1, borderBottomWidth: 1, paddingVertical: 4 }, confirmPanel: { borderWidth: 1, borderRadius: 16, padding: 16, gap: 8 }, confirmTitle: { fontSize: 18, lineHeight: 24, fontWeight: "800", letterSpacing: -0.4 }, confirmCopy: { fontSize: 13, lineHeight: 20 }, confirmActions: { marginTop: 6, flexDirection: "row", gap: 10, flexWrap: "wrap" } });