...
export function TradingView({ snapshot, investmentPercent, marketConnectionState, stale, error, refreshing, onRefresh, onSubmit, runtimeCanSubmit = true }: TradingViewProps) {
  ...
  useEffect(() => {
    ...
    let active = true;
    const refreshLocalPrice = async (): Promise<void> => {
      try {
        const markets = await loadUpbitPublicMarkets();
        const selected = markets.find((market) => market.market === LOCAL_PAPER_MARKET);
        if (!active) return;
        if (selected == null || !Number.isFinite(selected.price) || selected.price <= 0) { setLocalPriceError("KRW-BTC 공개 시세를 아직 받지 못했습니다."); setLocalMarkPrice(null); return; }
        setLocalMarkPrice(selected.price); setLocalPriceError(null);
      } catch (loadError) { if (!active) return; setLocalPriceError(loadError instanceof Error ? loadError.message : "Upbit 공개 시세를 불러올 수 없습니다."); }
    };
    void refreshLocalPrice();
    const timer = setInterval(() => { void refreshLocalPrice(); }, 10_000);
  ...
  const submitBuiltIn = async () => {
    ...
    try {
      ...
      if (usingLocalPaper) {
        ...
        if (!Number.isFinite(quantity) || quantity <= 0 || price == null || !Number.isFinite(price) || price <= 0) { setSubmitMessage("수량과 공개 시세를 확인하세요."); return; }
        ...
        setSubmitMessage(`LOCAL PAPER 체결 완료 · ${order.id}`);
      ...
      if (result.status === "READY") { setSubmitMessage(result.result.status === "FILLED" ? `PAPER 체결 완료 · ${result.result.order?.id ?? ""}` : `${result.result.status}${result.result.reason ? ` · ${result.result.reason}` : ""}`); if (result.result.status === "FILLED") { setQuantityInput(""); setPriceInput(""); } await Promise.resolve(onRefresh()); } else setSubmitMessage(result.reason);
    } catch (submitError) { setSubmitMessage(submitError instanceof Error ? submitError.message : "PAPER 주문을 처리할 수 없습니다."); }
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={usingLocalPaper ? () => undefined : onRefresh} />} testID="trading-screen">
    ...
    <View style={styles.ticket} testID="paper-order-ticket">
      ...
      {usingLocalPaper && localPriceError ? <InlineNotice title="공개 시세 확인 필요" detail={localPriceError} tone="warning" /> : null}
