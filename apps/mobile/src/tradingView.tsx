...
import {
  LOCAL_PAPER_INITIAL_CASH,
  LOCAL_PAPER_MARKET,
  getLocalPaperState,
  placeLocalPaperOrder,
  restoreLocalPaperState,
  setLocalPaperMarkPrice,
  subscribeLocalPaper,
  type LocalPaperState,
} from "./localPaperStore";
...
interface TradingViewProps {
  ...
  readonly runtimeCanSubmit?: boolean;
}
...
function ErrorState({ message, onRetry }: Readonly<{ message: string; onRetry: () => void }>) {
  return <View style={styles.state}><InlineNotice title="PAPER 화면을 표시할 수 없습니다" detail={message} tone="danger" /><NusaButton label="다시 불러오기" onPress={onRetry} /></View>;
}
...
export function TradingView({ snapshot, investmentPercent, marketConnectionState, stale, error, refreshing, onRefresh, onSubmit, runtimeCanSubmit = true }: TradingViewProps) {
    ...
    const refreshLocalMarket = async (): Promise<void> => {
      ...
      if (tickerResult.status === "fulfilled") {
        ...
        if (selected && Number.isFinite(selected.price) && selected.price > 0) {
          setLocalPaperMarkPrice(selected.price);
          setLocalPriceError(null);
        } else {
          setLocalPaperMarkPrice(null);
          setLocalPriceError("KRW-BTC 공개 시세를 아직 받지 못했습니다.");
        }
      } else {
        setLocalPaperMarkPrice(null);
  ...
  const model = buildTradingViewModel({
    market: { market: effectiveSnapshot.account.position.market, connectionState: effectiveConnectionState, stale: effectiveStale, price: effectiveSnapshot.account.markPrice },
    account: { mode: effectiveSnapshot.mode, liveMutationAllowed: false, cash: modelCash, assetQuantity: effectiveSnapshot.account.position.quantity, market: effectiveSnapshot.account.position.market },
    draft,
    submitAvailable,
  });
    ...
    try {
      ...
      if (usingLocalPaper) {
        ...
        if (!Number.isFinite(quantity) || quantity <= 0 || price == null || !Number.isFinite(price) || price <= 0) {
          setSubmitMessage("수량과 공개 시세를 확인하세요."); setOrderPhase("ERROR"); return;
        }
      ...
      if (!configuredEndpoint || !isPaperConnectionVerified(configuredEndpoint)) {
        setSubmitMessage("설정에서 PAPER endpoint와 세션을 먼저 검증하세요."); setOrderPhase("ERROR"); return;
      }
      ...
      if (result.status === "READY") {
        const filled = result.result.status === "FILLED";
        setSubmitMessage(filled ? `PAPER 체결 완료 · ${result.result.order?.id ?? ""}` : `${result.result.status}${result.result.reason ? ` · ${result.result.reason}` : ""}`);
        setOrderPhase(filled ? "FILLED" : "ERROR");
        if (filled) { setQuantityInput(""); setPriceInput(""); }
        await Promise.resolve(onRefresh());
      } else { setSubmitMessage(result.reason); setOrderPhase("ERROR"); }
    } catch (submitError) {
      setSubmitMessage(submitError instanceof Error ? submitError.message : "PAPER 주문을 처리할 수 없습니다."); setOrderPhase("ERROR");
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={theme.colors.primary} refreshing={refreshing} onRefresh={usingLocalPaper ? () => undefined : onRefresh} />} testID="trading-screen">
    ...
    <View style={styles.hero} testID="paper-quote-hero">
      ...
      <Text style={[styles.meta, { color: theme.colors.textMuted }]}>실제 주문 권한 없음 · Production mutation 금지</Text>
    </View>

    {usingLocalPaper ? <View style={[styles.panel, { borderColor: theme.colors.border, backgroundColor: theme.colors.surfaceSunken }]} testID="paper-upbit-market-panel">
      <View style={styles.row}><Text style={[styles.title, { color: theme.colors.text }]}>KRW-BTC 1분 차트</Text><StatusChip label={chartModel.state === "READY" ? "차트 LIVE" : "차트 대기"} tone={chartModel.state === "READY" ? "success" : "warning"} /></View>
      {chartModel.state === "READY" ? <View style={styles.miniChart} testID="paper-upbit-chart">{chartBars.map((bar) => <View key={bar.openTime} style={styles.chartColumn}><View style={[styles.chartWick, { backgroundColor: bar.up ? theme.colors.success : theme.colors.danger, top: `${bar.wickTop}%`, height: `${bar.wickHeight}%` }]} /><View style={[styles.chartBody, { backgroundColor: bar.up ? theme.colors.success : theme.colors.danger, top: `${bar.bodyTop}%`, height: `${bar.bodyHeight}%` }]} /></View>)}</View> : <InlineNotice title="차트를 불러오는 중" detail={localChartError ?? chartModel.error ?? "Upbit 1분 캔들을 기다리고 있습니다."} tone="warning" />}
      {localPriceError ? <InlineNotice title="Upbit 공개 시세 확인 필요" detail={localPriceError} tone="warning" /> : null}
    </View> : null}
    ...
    <View style={styles.ticket} testID="paper-order-ticket">
      ...
      <SegmentedControl disabled={submitting} items={SIDE_ITEMS} selectedKey={side} onChange={(key) => { setSide(key as TradingOrderSide); resetReview(); }} testID="paper-side-segmented-control" />
      <SegmentedControl disabled={submitting} items={ORDER_TYPE_ITEMS} selectedKey={orderType} onChange={(key) => { setOrderType(key as TradingOrderType); resetReview(); }} testID="paper-type-segmented-control" />
      {orderType === "LIMIT" ? <NusaTextField autoCorrect={false} editable={!submitting} keyboardType="decimal-pad" label="지정 가격" value={priceInput} onChangeText={(value) => { setPriceInput(value); resetReview(); }} placeholder="KRW 가격" returnKeyType="done" /> : null}
      <NusaTextField autoCorrect={false} editable={!submitting} keyboardType="decimal-pad" label={`수량 (${tradingAssetCode(model.market)})`} value={quantityInput} onChangeText={(value) => { setQuantityInput(value); resetReview(); }} placeholder="수량" returnKeyType="done" />
      <View style={[styles.panel, { borderColor: theme.colors.border, backgroundColor: theme.colors.surfaceSunken }]} testID={side === "BUY" ? "paper-allocation-panel" : "paper-holdings-panel"}><DataRow label={side === "BUY" ? "주문 가능 현금" : "매도 가능 수량"} value={side === "BUY" ? formatTradingAmount(cashEnvelope.investableCash, "KRW") : `${positionQuantity} ${tradingAssetCode(model.market)}`} emphasis /><DataRow label="예상 주문 금액" value={formatTradingAmount(model.estimatedNotional, "KRW")} /></View>
      ...
      {!confirming ? <NusaButton label={submitEnabled ? `${side === "BUY" ? "매수" : "매도"} 주문 검토` : "PAPER 주문 사용 불가"} disabled={!submitEnabled} onPress={requestSubmit} /> : <View style={[styles.panel, { borderColor: theme.colors.primary, backgroundColor: theme.colors.surfaceSunken }]}><Text style={[styles.title, { color: theme.colors.text }]}>이 PAPER 주문을 확정할까요?</Text><Text style={[styles.meta, { color: theme.colors.textMuted }]}>{model.market} · {side} · {orderType} · {quantityInput} {tradingAssetCode(model.market)}</Text><View style={styles.actions}><NusaButton label={submitting ? "전송 중..." : "PAPER 주문 확정"} disabled={submitting} onPress={() => void submitBuiltIn()} /><NusaButton label="돌아가기" disabled={submitting} onPress={resetReview} tone="neutral" /></View></View>}
      {submitMessage ? <InlineNotice title="PAPER 주문 결과" detail={submitMessage} tone={orderPhase === "FILLED" ? "success" : "warning"} /> : null}

      {usingLocalPaper ? <View style={[styles.panel, { borderColor: theme.colors.border, backgroundColor: theme.colors.surfaceSunken }]} testID="paper-local-ledger"><View style={styles.row}><Text style={[styles.title, { color: theme.colors.text }]}>LOCAL PAPER LEDGER</Text><StatusChip label={`${localState.trading.orders.length} 체결`} tone={localState.trading.orders.length > 0 ? "success" : "info"} /></View><DataRow label="KRW 현금" value={formatTradingAmount(effectiveSnapshot.account.cash, "KRW")} /><DataRow label="BTC 보유" value={`${effectiveSnapshot.account.position.quantity} BTC`} /><DataRow label="평균 매수가" value={formatTradingAmount(effectiveSnapshot.account.position.averagePrice, "KRW")} /><DataRow label="평가 손익" value={formatTradingAmount(effectiveSnapshot.account.unrealizedPnl, "KRW")} tone={effectiveSnapshot.account.unrealizedPnl >= 0 ? "success" : undefined} />{recentOrders.length === 0 ? <Text style={[styles.meta, { color: theme.colors.textMuted }]}>아직 체결된 PAPER 주문이 없습니다.</Text> : recentOrders.map((order) => <View key={order.id} style={styles.orderRow}><Text style={[styles.meta, { color: theme.colors.text }]}>{order.side} · {order.quantity} BTC · {formatTradingAmount(order.price, "KRW")}</Text><StatusChip label={order.status} tone={order.status === "FILLED" ? "success" : "warning"} /></View>)}</View> : null}
    </View>
  </ScrollView>;
}

const styles = StyleSheet.create({
  content: { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 44, gap: 20, width: "100%", maxWidth: 820, alignSelf: "center" },
  state: { flex: 1, justifyContent: "center", padding: 20, gap: 12 },
  hero: { gap: 8 }, ticket: { gap: 14 }, panel: { borderWidth: StyleSheet.hairlineWidth, borderRadius: 18, padding: 14, gap: 10 },
  row: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }, chips: { flexDirection: "row", flexWrap: "wrap", gap: 7 }, actions: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  eyebrow: { fontSize: 10, lineHeight: 15, fontWeight: "800", letterSpacing: 1.1 }, market: { marginTop: 4, fontSize: 18, lineHeight: 23, fontWeight: "800" }, title: { fontSize: 18, lineHeight: 24, fontWeight: "800" }, meta: { fontSize: 12, lineHeight: 18 },
  price: { fontSize: 38, lineHeight: 45, fontWeight: "800", letterSpacing: -1.4, fontVariant: ["tabular-nums"] },
  miniChart: { height: 180, flexDirection: "row", alignItems: "stretch", gap: 1, overflow: "hidden", position: "relative" }, chartColumn: { flex: 1, minWidth: 2, position: "relative" }, chartWick: { position: "absolute", left: "50%", width: 1 }, chartBody: { position: "absolute", left: "15%", right: "15%", minHeight: 2 },
  orderRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 10, paddingTop: 6 },
});
