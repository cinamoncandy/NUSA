...
import { submitPersonalPaperOrder } from "./personalPaperOrderClient";
...
export function TradingView({ snapshot, marketConnectionState, stale, error, refreshing, onRefresh, onSubmit }: TradingViewProps) {
  ...
  const submitBuiltIn = async () => {
    ...
    const result = await submitPersonalPaperOrder({ baseUrl: configuredEndpoint, credentialProvider: credentialSession.credentialProvider }, { schemaVersion: 1, authority: "PAPER_ONLY", productionMutationAllowed: false, idempotencyKey: idempotencyKey(), market: model.market, side, orderType, quantity, ...(limitPrice === undefined ? {} : { limitPrice }) });
