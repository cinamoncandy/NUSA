...
import { effectivePaperEndpoint } from "./canonicalOrigin";
...
export class VersionedSettingsRepository implements SettingsRepository {
  ...
  public async load(): Promise<AppSettings | null> { const value = await this.store.load(); setConfiguredPaperEndpoint(effectivePaperEndpoint((value ?? normalizeSettings({})).paperEndpoint)); return value; }
  public async save(value: AppSettings): Promise<void> { const normalized = normalizeSettings(value); await this.store.save(normalized); setConfiguredPaperEndpoint(effectivePaperEndpoint(normalized.paperEndpoint)); }
}
