...
import { normalizeSettings, type AppSettings, type SettingsNormalizationOptions, type SettingsRepository } from "./settings";
...
export class VersionedSettingsRepository implements SettingsRepository {
  ...
  public constructor(storage: VersionedStorage, key = "nusa:app-settings:v1", private readonly options: SettingsNormalizationOptions = {}) { this.store = new VersionedJsonStore(storage, key, 1, (value) => normalizeSettings(value as Partial<AppSettings>, this.options), (value, version) => { if (version !== 0 || value === null || typeof value !== "object") throw new Error("unsupported settings migration"); return normalizeSettings(value as Partial<AppSettings>, this.options); }); }
  public async load(): Promise<AppSettings | null> { const value = await this.store.load(); setConfiguredPaperEndpoint((value ?? normalizeSettings({}, this.options)).paperEndpoint); return value; }
  public async save(value: AppSettings): Promise<void> { const normalized = normalizeSettings(value, this.options); await this.store.save(normalized); setConfiguredPaperEndpoint(normalized.paperEndpoint); }
}
