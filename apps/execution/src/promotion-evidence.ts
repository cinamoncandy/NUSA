...
import { IndependentApprovalDecision } from "./independent-approval";
import type { SqliteIndependentApprovalEvidenceRepository } from "./independent-approval-evidence";
...
export class SqlitePromotionEvidenceRepository {
 public constructor(private readonly db:DatabaseSync,private readonly approvals:SqliteIndependentApprovalEvidenceRepository){this.db.exec(`CREATE TABLE IF NOT EXISTS promotion_evidence (promotion_id TEXT PRIMARY KEY,candidate_id TEXT NOT NULL,approval_id TEXT NOT NULL,decision TEXT NOT NULL,blockers_json TEXT NOT NULL,evaluated_at_ms INTEGER NOT NULL,production_mutation_allowed INTEGER NOT NULL CHECK (production_mutation_allowed = 0));`);}
 public append(input:{readonly promotionId:string;readonly promotion:ReleaseCandidatePromotionResult;readonly approvalId:string;}):PromotionEvidence{
  ...
  if(!approval)throw new Error("referenced independent approval evidence does not exist");
  if(approval.candidateId!==input.promotion.candidateId)throw new Error("approval candidate mismatch");
  if(approval.decision!==IndependentApprovalDecision.VALID)throw new Error("valid independent approval is required");
  if(this.get(input.promotionId))throw new Error("duplicate promotion evidence id");
  this.db.prepare("INSERT INTO promotion_evidence VALUES (?,?,?,?,?,?,0)").run(input.promotionId,input.promotion.candidateId,approval.approvalId,input.promotion.decision,JSON.stringify(input.promotion.blockers),input.promotion.evaluatedAtMs);
