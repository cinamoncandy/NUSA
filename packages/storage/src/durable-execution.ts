...
export class SqliteDurableExecutionRepository implements ExecutionRepository {
  ...
  appendFill(fill: ExecutionFill): void {
    // First-write-wins per exchange trade (reconcile replays the same fills
    // routinely), but a reused fill/trade id with divergent economics must
    // fail closed instead of vanishing into OR IGNORE silence.
    this.db.transaction(() => {
      const existing = this.db.connection.prepare("SELECT execution_id, exchange_trade_id, fill_json FROM execution_fills WHERE fill_id=? OR exchange_trade_id=?").get(fill.fillId, fill.exchangeTradeId) as { execution_id: string; exchange_trade_id: string; fill_json: string } | undefined;
      if (existing == null) {
        this.db.connection.prepare("INSERT INTO execution_fills(fill_id,execution_id,exchange_trade_id,fill_json) VALUES(?,?,?,?)").run(fill.fillId, fill.executionId, fill.exchangeTradeId, JSON.stringify(fill));
        return;
      }
      const stored = parseStored<ExecutionFill>(existing.fill_json, "execution-fill", fill.fillId);
      // Identity is the exchange trade within its execution; fillId is a
      // label and may be regenerated by a re-fetch with identical economics.
      const same = existing.execution_id === fill.executionId && existing.exchange_trade_id === fill.exchangeTradeId &&
        stored.quantity === fill.quantity && stored.price === fill.price &&
        stored.fee === fill.fee && stored.feeCurrency === fill.feeCurrency && stored.executedAt === fill.executedAt;
      if (!same) throw new Error("EXECUTION_FILL_CONFLICT");
    });
  }
