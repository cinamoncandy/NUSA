...
export class SqliteDurableExecutionRepository implements ExecutionRepository {
  ...
  get(id: string): ExecutionRecord | null { const r = this.db.connection.prepare("SELECT record_json FROM execution_records WHERE execution_id=?").get(id) as { record_json: string } | undefined; return r ? JSON.parse(r.record_json) as ExecutionRecord : null; }
  getByClientOrderId(id: string): ExecutionRecord | null { const r = this.db.connection.prepare("SELECT record_json FROM execution_records WHERE client_order_id=?").get(id) as { record_json: string } | undefined; return r ? JSON.parse(r.record_json) as ExecutionRecord : null; }
  ...
  transitions(id: string): readonly ExecutionTransition[] { const rows = this.db.connection.prepare("SELECT transition_json FROM execution_transitions WHERE execution_id=? ORDER BY sequence").all(id) as Array<{ transition_json: string }>; return Object.freeze(rows.map(x => JSON.parse(x.transition_json) as ExecutionTransition)); }
  fills(id: string): readonly ExecutionFill[] { const rows = this.db.connection.prepare("SELECT fill_json FROM execution_fills WHERE execution_id=? ORDER BY rowid").all(id) as Array<{ fill_json: string }>; return Object.freeze(rows.map(x => JSON.parse(x.fill_json) as ExecutionFill)); }
