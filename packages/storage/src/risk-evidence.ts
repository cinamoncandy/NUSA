...
type Row = { decision_id: string; result: string; policy_version: string; reason_codes: string; observed_at: string; execution_id: string };
...
export class SqliteRiskEvidenceRepository implements RiskEvidenceRepository {
  public constructor(private readonly db: RiskEvidenceDatabase) {
    this.db.connection.exec("CREATE TABLE IF NOT EXISTS risk_decision_evidence (decision_id TEXT PRIMARY KEY, result TEXT NOT NULL, policy_version TEXT NOT NULL, reason_codes TEXT NOT NULL, observed_at TEXT NOT NULL, execution_id TEXT NOT NULL); CREATE INDEX IF NOT EXISTS idx_risk_evidence_execution ON risk_decision_evidence(execution_id, observed_at, decision_id);");
  }
  ...
  public append(record: RiskDecision): void {
    ...
    this.db.connection.prepare("INSERT INTO risk_decision_evidence(decision_id,result,policy_version,reason_codes,observed_at,execution_id) VALUES(?,?,?,?,?,?)").run(record.decisionId, record.result, record.policyVersion, JSON.stringify([...record.reasonCodes]), record.observedAt, record.executionId);
  }
...
function decode(row: Row): RiskDecision {
  return Object.freeze({ decisionId: row.decision_id, result: row.result as RiskResult, policyVersion: row.policy_version, reasonCodes: Object.freeze(JSON.parse(row.reason_codes) as string[]), observedAt: row.observed_at, executionId: row.execution_id });
}
