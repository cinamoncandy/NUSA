...
function decode(row: SqlRow): FundingReconciliationResult {
  return Object.freeze({
    ...
    status: String(row.status) as FundingReconciliationResult["status"],
    localCount: Number(String(row.local_count)),
    providerCount: Number(String(row.provider_count)),
    mismatchFundingIds: Object.freeze(JSON.parse(String(row.mismatch_funding_ids_json)) as string[]),
...
export class SqliteFundingReconciliationEvidenceRepository implements FundingReconciliationEvidenceRepository {
  ...
  public append(result: FundingReconciliationResult): void {
    this.db.connection.prepare(`
      INSERT INTO funding_reconciliation_results (
    ...
    `).run(
      ...
      JSON.stringify([...result.mismatchFundingIds].sort()),
  ...
  public getById(reconciliationId: string): FundingReconciliationResult | undefined {
    const row = this.db.connection.prepare(
      "SELECT * FROM funding_reconciliation_results WHERE reconciliation_id = ?"
    ).get(reconciliationId) as SqlRow | undefined;
