...
function validate(record: ResearchSessionRecord): void {
  ...
  if (metrics.experimentCount !== record.experimentCount || !Number.isSafeInteger(metrics.experimentCount) || metrics.experimentCount < 0 || !Number.isSafeInteger(metrics.positiveEvaluationCount) || !Number.isSafeInteger(metrics.negativeEvaluationCount) || !Number.isSafeInteger(metrics.championBetterCount) || !Number.isSafeInteger(metrics.challengerBetterCount) || !Number.isSafeInteger(metrics.equivalentCount) || !Number.isSafeInteger(metrics.inconclusiveCount) || !finite(metrics.cumulativeNetReturn) || !finite(metrics.averageNetReturn) || !finite(metrics.costAdjustedPerformance)) throw new Error("research session metrics are invalid");
