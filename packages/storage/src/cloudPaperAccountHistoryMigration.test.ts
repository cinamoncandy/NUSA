...
describe("canonical PAPER account history migration", () => {
  ...
  it("bootstraps an existing VALID canonical account when upgrading from migration 017", () => {
    ...
    try {
      const migration017Index = migrations.findIndex((migration) => migration.id === "017_paper_realized_periods");
      assert.notEqual(migration017Index, -1);
      runMigrations(db, migrations.slice(0, migration017Index + 1), () => new Date("2026-08-29T00:00:00.000Z"));
      ...
      assert.deepEqual(result.applied, ["018_cloud_paper_account_history", "019_paper_public_market_observations", "020_evolution_learning_ledger", "021_research_factory_decision_history", "022_research_memory_semantic_overlay", "023_cloud_paper_fill_ledger"]);
