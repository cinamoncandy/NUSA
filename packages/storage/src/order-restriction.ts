...
export class SqliteOrderOperationalRestrictionRepository implements OrderOperationalRestrictionRepository {
  public constructor(private readonly db: SqliteDatabase) {
    this.db.connection.exec(`
      CREATE TABLE IF NOT EXISTS order_operational_restrictions (
        ...
        reason TEXT NOT NULL CHECK (reason IN ('CRITICAL_UNKNOWN_SUBMISSION', 'POSITION_MISMATCH', 'POSITION_STATE_UNCERTAIN', 'POSITION_RECONCILIATION_STALE', 'BALANCE_MISMATCH', 'BALANCE_STATE_UNCERTAIN', 'BALANCE_RECONCILIATION_STALE')),
  ...
  public getById(restrictionId: string): OrderOperationalRestriction | undefined {
    const row = this.db.connection.prepare("SELECT * FROM order_operational_restrictions WHERE restriction_id = ?").get(restrictionId) as SqlRow | undefined;
    return row == null ? undefined : decode(row);
  }

  public getActiveForAccount(accountId: string): OrderOperationalRestriction | undefined {
    const row = this.db.connection.prepare("SELECT * FROM order_operational_restrictions WHERE account_id = ? AND status = 'ACTIVE'").get(accountId) as SqlRow | undefined;
    return row == null ? undefined : decode(row);
  }

  public save(restriction: OrderOperationalRestriction): OrderOperationalRestriction {
    const existing = this.db.connection.prepare("SELECT * FROM order_operational_restrictions WHERE restriction_id = ?").get(restriction.restrictionId) as SqlRow | undefined;
    if (existing == null) {
      ...
      `).run(restriction.restrictionId, restriction.accountId, restriction.reason, restriction.sourceRunId, JSON.stringify([...restriction.sourceIntentIds].sort()), restriction.status, restriction.createdAtMs.toString(), restriction.releasedAtMs?.toString() ?? null);
    } else {
      ...
      this.db.connection.prepare("UPDATE order_operational_restrictions SET status = ?, released_at_ms = ? WHERE restriction_id = ?").run(restriction.status, restriction.releasedAtMs?.toString() ?? null, restriction.restrictionId);
    }
