...
export function validatePersonalPaperOrderCommandResult(
  ...
  if (result.order != null) {
    ...
    if (corresponding.market !== command.market || corresponding.side !== command.side || corresponding.quantity !== command.quantity) {
      throw new Error("PAPER order result does not match submitted command");
    }
    order = corresponding;
  }

  if ((result.status === "FILLED" || result.status === "DUPLICATE") && (order == null || snapshot == null)) {
    throw new Error(`${result.status} PAPER order result requires order and snapshot`);
  }
