...
const safeIdempotencyKey = (value: string): string => {
  if (typeof value !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._:-]{15,159}$/.test(value)) {
...
function validateOrderProjection(order: PersonalPaperOrderProjection): PersonalPaperOrderProjection {
  if (
    ...
    order.status !== "FILLED" ||
    !Number.isFinite(Date.parse(order.filledAt))
  ) {
    throw new Error("invalid PAPER order projection");
  }
  if (
    ...
    throw new Error("invalid PAPER order value");
  }
  ...
  const fills = order.fills.map((fill) => {
    if (
      ...
      throw new Error("invalid PAPER fill projection");
    }
...
const sameCommandBinding = (
  ...
  result.orderType === command.orderType &&
  result.quantity === command.quantity &&
  (command.orderType === "LIMIT"
    ? result.limitPrice === command.limitPrice
    : result.limitPrice === undefined);

export function validatePersonalPaperOrderCommand(
  command: PersonalPaperOrderCommand
): PersonalPaperOrderCommand {
  if (command.schemaVersion !== 1) {
    throw new Error("unsupported PAPER order command schemaVersion");
  }
  if (command.authority !== "PAPER_ONLY" || command.productionMutationAllowed !== false) {
    throw new Error("PAPER order authority invariant violated");
  }
  ...
  if (command.orderType !== "MARKET" && command.orderType !== "LIMIT") {
    throw new Error("orderType is invalid");
  }

  const quantity = finitePositive(command.quantity, "quantity");
  const limitPrice = command.orderType === "LIMIT"
    ? finitePositive(command.limitPrice ?? Number.NaN, "limitPrice")
    : undefined;
...
export function validatePersonalPaperOrderCommandResult(
  ...
  if (result.liveAuthority !== "NONE" || result.productionMutationAllowed !== false) {
    throw new Error("PAPER order result authority invariant violated");
  }
  if (!["FILLED", "REJECTED", "BLOCKED", "DUPLICATE"].includes(result.status)) {
    throw new Error("PAPER order result status is invalid");
  }
  if (result.idempotencyKey == null) {
    throw new Error("PAPER order result command binding is missing");
  }
  safeIdempotencyKey(result.idempotencyKey);
  if (!sameCommandBinding(result, command)) {
    throw new Error("PAPER order result command binding mismatch");
  }

  const reason = result.reason;
  if (reason != null && (typeof reason !== "string" || !reason.trim())) {
    throw new Error("PAPER order result reason is invalid");
  }
  ...
  if (result.order != null) {
    ...
    if (snapshot != null) {
      ...
      if (corresponding == null || !sameOrder(validatedOrder, corresponding)) {
    ...
    if (
      ...
      throw new Error("PAPER order result does not match submitted command");
    }
  }
  ...
  if (result.status === "DUPLICATE" && (order == null || snapshot == null)) {
    throw new Error("DUPLICATE PAPER order result requires order and snapshot");
  }
