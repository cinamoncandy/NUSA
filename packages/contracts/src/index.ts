...
export * from "./aiEvaluationMarketSeriesIdentity";
...
export function assertBigInt(value: unknown, name: string): bigint {
  if (typeof value !== "bigint") throw new Error(`${name} must be bigint`);
  return value;
}

export function compareLedgerOrder(a: PositionLedgerEntry, b: PositionLedgerEntry): number {
...
export function makePositionLedgerEntry(input: PositionLedgerEntryInput): PositionLedgerEntry {
  ...
  return Object.freeze({
    id: assertNonEmpty(input.id, "id"), walletId: assertNonEmpty(input.walletId, "walletId"),
    strategyId: input.strategyId == null ? undefined : assertNonEmpty(input.strategyId, "strategyId"),
    symbol: assertNonEmpty(input.symbol, "symbol"), side: input.side, baseQtyRaw, quoteQtyRaw,
    ts: assertNonEmpty(input.ts, "ts"), createdAt: assertNonEmpty(input.createdAt, "createdAt"), sourceTradeId: input.sourceTradeId
  });
}

export function emptyWalletPositionSnapshot(walletId: string, symbol: string): WalletPositionSnapshot {
  return Object.freeze({ scopeType: PositionScopeType.WALLET, walletId: assertNonEmpty(walletId, "walletId"), symbol: assertNonEmpty(symbol, "symbol"), baseQtyRaw: 0n, quoteCostRaw: 0n, avgEntryPriceRaw: undefined, realizedPnlRaw: 0n, status: PositionStatus.CLOSED, lastLedgerEntryId: undefined, lastLedgerOrderKey: undefined, version: 0 });
}

export function emptyStrategyPositionSnapshot(walletId: string, strategyId: string, symbol: string): StrategyPositionSnapshot {
  return Object.freeze({ scopeType: PositionScopeType.STRATEGY, walletId: assertNonEmpty(walletId, "walletId"), strategyId: assertNonEmpty(strategyId, "strategyId"), symbol: assertNonEmpty(symbol, "symbol"), baseQtyRaw: 0n, quoteCostRaw: 0n, avgEntryPriceRaw: undefined, realizedPnlRaw: 0n, status: PositionStatus.CLOSED, lastLedgerEntryId: undefined, lastLedgerOrderKey: undefined, version: 0 });
}
