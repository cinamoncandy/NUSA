...
export interface TailLossObservation {
  ...
  /** Dependence-group id (e.g. from aiEvaluationDependenceGroups.ts) -- observations sharing a
   * groupId are treated as one clustered stress episode, not independent samples. */
  readonly groupId: string;
  /** Realized loss magnitude; higher means worse (a positive number, not signed PnL). */
...
/** Collapses observations to one representative (worst/highest-loss) entry per groupId --
 * the dependence adjustment that keeps a clustered episode from counting as many samples. */
...
/**
 * Mean of the `k` worst dependence-adjusted (one-per-group) losses. Fails closed if the
 * dependence-adjusted effective sample size (distinct groupId count) is below
 * `minEffectiveSampleSize`, or if `k` exceeds the effective sample size, or `k` is not a positive
 * integer -- never silently averaging over fewer independent stress episodes than required, and
 * never silently clamping k down.
 */
...
/**
 * Tail conditional loss (CVaR-style): the mean of the worst `ceil(alpha * effectiveSampleSize)`
 * dependence-adjusted losses, for `alpha` in (0, 1] (e.g. alpha=0.05 is the worst 5%). Delegates
 * to computeWorstKMean's same fail-closed conditions, plus rejects an out-of-(0,1] alpha.
 */
