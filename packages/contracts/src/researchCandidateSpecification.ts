...
export function validateResearchCandidateSpecification(value: unknown): readonly string[] {
  ...
  if (typeof v.parametersCanonicalJson !== "string" || v.parametersCanonicalJson.trim() === "") errors.push("PARAMETERS_CANONICAL_JSON_INVALID");
    ...
    try {
      const parsed = JSON.parse(v.parametersCanonicalJson) as JsonValue;
      if (canonicalizeJson(parsed) !== v.parametersCanonicalJson) errors.push("PARAMETERS_CANONICAL_JSON_INVALID");
    } catch {
      errors.push("PARAMETERS_CANONICAL_JSON_INVALID");
    }
  }
