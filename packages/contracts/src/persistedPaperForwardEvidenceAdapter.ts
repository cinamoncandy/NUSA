import type { PersistedPaperPeriodEnvelope } from "./persistedPaperPeriod";
import { adaptPersistedPaperPeriods } from "./persistedPaperPeriodAdapter";
import { admitPaperForwardEvidence, type PaperForwardEvidenceAdmission, type PaperForwardEvidenceAdmissionPolicy, type PaperForwardEvidenceSource } from "./paperForwardEvidenceAdmission";
...
function candidateIdentities(envelope: PersistedPaperPeriodEnvelope): ReadonlyMap<string, { readonly datasetId: string; readonly datasetContentSha256: string }> {
  ...
  for (const provenance of envelope.candidateProvenance) {
    ...
    if (!candidateId || !datasetId || !SHA256.test(provenance.datasetContentSha256)) throw new PersistedPaperForwardEvidenceAdapterError("INVALID_DATASET_PROVENANCE", "candidate dataset provenance is incomplete", record.recordId, candidateId || undefined);
    if (!advisoryIds.has(candidateId) || !realizedIds.has(candidateId)) throw new PersistedPaperForwardEvidenceAdapterError("CANDIDATE_PROVENANCE_LINK_MISSING", `candidate ${candidateId} is not linked to the advisory and realized return`, record.recordId, candidateId);
    const entry = record.advisory.entries.find((item) => item.id === candidateId);
    if (entry == null || !entry.sourceDatasetIds.includes(datasetId) || !record.advisory.provenance.sourceDatasetIds.includes(datasetId)) throw new PersistedPaperForwardEvidenceAdapterError("DATASET_PROVENANCE_LINK_MISSING", `candidate ${candidateId} dataset provenance is not present in the advisory`, record.recordId, candidateId);
  ...
  if (identities.size !== advisoryIds.size || identities.size !== realizedIds.size) throw new PersistedPaperForwardEvidenceAdapterError("CANDIDATE_PROVENANCE_SET_MISMATCH", `record ${record.recordId} has an incomplete candidate provenance set`, record.recordId);
  return identities;
}
...
function turnoverByCandidate(periods: readonly ShadowAllocationPeriodInput[], evaluated: ReturnType<typeof evaluateShadowAllocation>): readonly Readonly<Record<string, number>>[] {
  ...
  return freeze(periods.map((period, index) => {
    ...
    if (expected == null || Math.abs(total - expected) > COST_TOLERANCE) throw new PersistedPaperForwardEvidenceAdapterError("TURNOVER_RECONCILIATION_MISMATCH", `period ${period.periodIndex} turnover could not be reconciled`);
    previous.clear();
...
export function adaptPersistedPaperForwardEvidence(
  ...
  ordered.forEach((envelope, index) => {
    ...
    if (ids.length !== candidateIds.length || ids.some((id, candidateIndex) => id !== candidateIds[candidateIndex])) throw new PersistedPaperForwardEvidenceAdapterError("CANDIDATE_SET_DRIFT", `candidate set changed in period ${envelope.record.recordId}`, envelope.record.recordId);
    for (const candidateId of candidateIds) {
      const expected = firstIdentities.get(candidateId)!;
      const current = identities.get(candidateId)!;
      if (current.datasetId !== expected.datasetId || current.datasetContentSha256 !== expected.datasetContentSha256) throw new PersistedPaperForwardEvidenceAdapterError("DATASET_PROVENANCE_MISMATCH", `candidate ${candidateId} dataset provenance changed across PAPER periods`, envelope.record.recordId, candidateId);
      const record = envelope.record;
      ...
      if (Math.abs(record.turnoverCostRate - costRate) > COST_TOLERANCE) throw new PersistedPaperForwardEvidenceAdapterError("COST_RATE_RECONCILIATION_MISMATCH", `period ${record.recordId} turnover cost rate does not match execution cost evidence`, record.recordId, candidateId);
      ...
      periodsByCandidate.get(candidateId)!.push(freeze({ periodId: record.recordId, candidateId, datasetId: expected.datasetId, datasetContentSha256: expected.datasetContentSha256, advisoryGeneratedAt, periodStartAt: record.periodStartAt, periodEndAt: record.periodEndAt, grossReturn, turnover: turnover[index]![candidateId] ?? 0, feeRate: costEvidence.feeRate, spreadRate: costEvidence.spreadRate, slippageRate: costEvidence.slippageRate, status: record.status }));
    }
  });
  ...
  const candidates = candidateIds.map((candidateId) => {
    ...
    return freeze({ candidateId, datasetId: identity.datasetId, datasetContentSha256: identity.datasetContentSha256, periods, admission, listPaperRealizedPeriods: () => periods });
  });
