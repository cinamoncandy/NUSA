...
const valid = () => ({ schemaVersion: 1 as const, scope: "OPERATIONAL_EVIDENCE_ONLY" as const, authority: "READ_ONLY" as const, headSha: "a".repeat(40), asOf: NOW - 1_000, level: 1, overallProgressRatio: 0.5, domains: [{ domain: "INFRASTRUCTURE_MODULE_HEALTH", completionRatio: 1 }], achievedCriteria: ["exact-head-repository-ci"], blockedCriteria: ["actual-paper-runtime"], reasons: ["runtime evidence required"], blockers: ["actual-paper-runtime:runtime evidence required"] });
