...
export function validateOperationalProgressSnapshot(value: unknown, now = Date.now(), maxAgeMs = 15 * 60_000): OperationalProgressSnapshot {
  ...
  return freeze({ schemaVersion: 1, scope: "OPERATIONAL_EVIDENCE_ONLY", authority: "READ_ONLY", headSha: input.headSha, asOf: input.asOf, level: input.level as number, overallProgressRatio: ratio(input.overallProgressRatio, "overallProgressRatio"), domains, achievedCriteria: strings(input.achievedCriteria, "achievedCriteria"), blockedCriteria: strings(input.blockedCriteria, "blockedCriteria"), reasons: strings(input.reasons, "reasons"), blockers: strings(input.blockers, "blockers") });
}
