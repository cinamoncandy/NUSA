...
test("rejects malformed or non-canonical parameters and non-forward evaluation windows", () => {
  const malformed = validateResearchCandidateSpecification({
    schemaVersion: 1,
    ...base,
  ...
  assert.ok(malformed.includes("PARAMETERS_CANONICAL_JSON_INVALID"));
  assert.ok(malformed.includes("EVALUATION_WINDOW_INVALID"));

  const nonCanonical = validateResearchCandidateSpecification({
    schemaVersion: 1,
    ...base,
    parametersCanonicalJson: '{"z":1,"a":{"y":2,"x":3}}',
    authority: "PAPER_ONLY",
  });
  assert.ok(nonCanonical.includes("PARAMETERS_CANONICAL_JSON_INVALID"));

  const canonical = validateResearchCandidateSpecification({
    schemaVersion: 1,
    ...base,
    parametersCanonicalJson: '{"a":{"x":3,"y":2},"z":1}',
    authority: "PAPER_ONLY",
  });
