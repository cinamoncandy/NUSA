...
export interface ResearchFactoryDecision {
  ...
  /** Qualification is research/PAPER eligibility only. */
...
function assertInput(input: ResearchFactoryDecisionInput): void {
  ...
  for (const key of EVIDENCE_KEYS) {
    if (!STATE.has(input.evidence[key])) throw new Error(`RESEARCH_FACTORY_EVIDENCE_STATE_INVALID:${key}`);
  }
}

/**
 * Deterministic disposition:
 * - any explicit FAIL => REJECTED;
 * - otherwise any UNKNOWN => INSUFFICIENT;
 * - only all PASS => QUALIFIED_FOR_LEAGUE.
 *
 * FAIL dominates UNKNOWN so known counter-evidence cannot be softened into an abstention merely by
 * leaving another gate unresolved. Missing/unknown evidence can never silently pass.
 */
export function decideResearchFactoryOutcome(input: ResearchFactoryDecisionInput): ResearchFactoryDecision {
  ...
  const outcome: ResearchFactoryOutcome = failed.length > 0
    ? "REJECTED"
    : unknown.length > 0
      ? "INSUFFICIENT"
