 ...
 * before any outcome in that family was observed. A selection frozen after outcomes were already
 * known could have been cherry-picked to flatter the result -- that is exactly the leakage this
 * module rejects, regardless of how plausible the selection looks in isolation.
 */
...
/**
 * Validates that `selection` was frozen at or before `earliestOutcomeObservedAt` -- the earliest
 * moment any outcome in its evaluationFamilyId became known. Fails closed: a selection frozen
 * strictly after that moment is rejected as post-hoc (potential cherry-picking), and malformed
 * input (missing ids, invalid timestamps) is rejected rather than assumed valid.
 */
