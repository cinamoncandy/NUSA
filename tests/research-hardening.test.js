...
const evidence = (p, overrides = {}) => ({ schemaVersion: 1, researchRunId: p.researchRunId, evaluationId: p.evaluationId, strategyId: p.strategyId, strategyVersion: p.strategyVersion, marketDataTimestamp: 2, evaluationTimestamp: 3, canonicalInputHash: p.canonicalInputHash, modelVersion: p.modelVersion, fillModelVersion: p.fillModelVersion, feeModelVersion: p.feeModelVersion, slippageModelVersion: p.slippageModelVersion, provenance: p, champion: { strategyId: "champion-1", strategyVersion: "1.0.0", authority: "PAPER_ONLY", evaluatorVersion: "champion-eval", canonicalInputHash: p.canonicalInputHash, metrics: metrics({ netReturn: 0.1, costAdjustedReturn: 0.09, sharpeRatio: 1.1 }), signal: "HOLD" }, challenger: { strategyId: p.strategyId, strategyVersion: p.strategyVersion, authority: "ZERO_AUTHORITY", evaluatorVersion: "eval-1", canonicalInputHash: p.canonicalInputHash, metrics: metrics(), signal: "HOLD" }, result: "CHALLENGER_BETTER", reason: "MULTI_METRIC_COMPARISON", productionMutationAllowed: false, promotionAllowed: false, ...overrides });

test("a single winning net-return evaluation never creates eligibility", () => {
  const p = provenance("window-1");
  const result = new ResearchCandidateGate({ nowMs: 10 }).evaluate({ candidateId: "candidate-1", evidence: [evidence(p)] });
...
test("multiple independent windows with untouched holdout and multi-metric evidence are eligible only for PAPER", () => {
  ...
  const result = new ResearchCandidateGate({ nowMs: 10 }).evaluate({ candidateId: "candidate-1", evidence: [evidence(first), evidence(second)], provenance: [first, second] });
...
test("missing provenance, faults, poor drawdown, and reused holdout fail closed", () => {
  const first = provenance("window-1");
  const second = provenance("window-2", "HOLDOUT", "holdout");
  const result = new ResearchCandidateGate({ nowMs: 10 }).evaluate({ candidateId: "candidate-1", evidence: [evidence(first, { challenger: { ...evidence(first).challenger, metrics: metrics({ unresolvedFaultCount: 1, maximumDrawdown: 0.3 }) } }), evidence(second)], provenance: [first, { ...second, splitHash: first.splitHash }] });
  assert.equal(result.status, "NOT_ELIGIBLE");
