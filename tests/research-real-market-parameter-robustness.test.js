...
const manifest = { market: "KRW-BTC", datasetId: "upbit-days-example", contentSha256: "a".repeat(64) };
const candles = [{ market: "KRW-BTC", interval: "1d", openTime: 1, closeTime: 86_401_000, open: 100, high: 110, low: 90, close: 105, volume: 1 }];
...
test("canonical real-market run builds fixed parameter robustness evidence from its manifest and candles", () => {
  ...
  assert.deepEqual(first.neighborhood, { shortOffsets: [-2, -1, 0, 1, 2], longOffsets: [-5, -2, 0, 2, 5] });
  ...
  assert.deepEqual(first.evaluation, { mode: "BOTH", oosWindows: { trainingCandles: 120, testCandles: 20, stepCandles: 20 } });
...
test("parameter robustness request refuses missing canonical source inputs", () => {
  assert.throws(() => buildParameterRobustnessRequest({ candles: [], manifest }), /canonical candles/);
  assert.throws(() => buildParameterRobustnessRequest({ candles, manifest: { market: "KRW-BTC" } }), /canonical dataset manifest/);
});
...
test("canonical Donchian robustness request is the precommitted five-period line with explicit adjacency", () => {
  ...
  assert.deepEqual(request.candidateGrid.map((entry) => entry.key), ["donchian-10", "donchian-20", "donchian-30", "donchian-40", "donchian-55"]);
  assert.deepEqual(request.referenceParameters, [
