...
const filledResult = (overrides = {}, submitted = command()) => {
  const order = filledOrder({ market: submitted.market, side: submitted.side, quantity: submitted.quantity });
  return {
    schemaVersion: 1,
    status: "FILLED",
    ...binding(submitted),
...
const blockedWireResult = (submitted = command(), overrides = {}) => ({
  schemaVersion: 1,
  status: "BLOCKED",
  ...binding(submitted),
  reason: "TEST_BLOCK",
...
test("FILLED result requires truthful command binding, order, and canonical snapshot", () => {
  ...
  assert.equal(result.idempotencyKey, submitted.idempotencyKey);
});

test("result rejects foreign idempotency, market, side, and quantity bindings", () => {
  const submitted = command();
  for (const overrides of [
    { idempotencyKey: "paper-mobile-00000002" },
    { market: "KRW-ETH" },
    { side: "SELL" },
    { quantity: 0.002 }
  ]) assert.throws(() => validatePersonalPaperOrderCommandResult(filledResult(overrides, submitted), submitted, NOW), /binding mismatch|submitted command/);
});

test("FILLED result rejects stale, malformed, or authority-invalid nested PAPER snapshots", () => {
  const submitted = command();
  const valid = filledResult({}, submitted);
  ...
  assert.throws(() => validatePersonalPaperOrderCommandResult(malformed, submitted, NOW), /invalid PAPER order value/);

  const authorityInvalid = structuredClone(valid);
  authorityInvalid.snapshot.liveAuthority = "LIVE";
  assert.throws(() => validatePersonalPaperOrderCommandResult(authorityInvalid, submitted, NOW), /authority/);
});

test("FILLED result rejects malformed fill and standalone order mismatch", () => {
  const submitted = command();
  const malformedFill = filledResult({}, submitted);
  malformedFill.order = { ...malformedFill.order, fills: [{ ...malformedFill.order.fills[0], quantity: -1 }] };
  assert.throws(() => validatePersonalPaperOrderCommandResult(malformedFill, submitted, NOW), /fill projection/);

  const valid = filledResult({}, submitted);
  const mismatched = { ...valid, order: { ...valid.order, price: valid.order.price + 1 } };
  assert.throws(() => validatePersonalPaperOrderCommandResult(mismatched, submitted, NOW), /does not match snapshot order/);
});

test("DUPLICATE result must carry the prior truthful order bound to the submitted command", () => {
  const submitted = command();
  const prior = filledOrder();
  const duplicate = validatePersonalPaperOrderCommandResult({
    schemaVersion: 1,
    status: "DUPLICATE",
    ...binding(submitted),
  ...
  assert.equal(duplicate.order.id, prior.id);

  assert.throws(() => validatePersonalPaperOrderCommandResult({ ...duplicate, order: { ...prior, market: "KRW-ETH" } }, submitted, NOW), /submitted command/);
});
  ...
  const result = await submitPersonalPaperOrder({
    ...
    request: async (url, init) => {
      ...
      return {
        ...
        status: 200,
        redirected: false,
        url: "https://paper.example.test/api/paper-orders",
...
test("mobile client rejects redirected or changed final PAPER endpoint", async () => {
  ...
  const redirected = await submitPersonalPaperOrder({
    baseUrl: "https://paper.example.test",
    credentialProvider: async () => "1234567890123456",
    request: async () => ({ ok: true, status: 200, redirected: true, url: "https://other.example.test/api/paper-orders", json: async () => blockedWireResult(submitted) })
  }, submitted);
  assert.equal(redirected.status, "UNAVAILABLE");
  assert.match(redirected.reason, /redirect/i);

  const changed = await submitPersonalPaperOrder({
    baseUrl: "https://paper.example.test",
    credentialProvider: async () => "1234567890123456",
    request: async () => ({ ok: true, status: 200, redirected: false, url: "https://paper.example.test/other", json: async () => blockedWireResult(submitted) })
  }, submitted);
  assert.equal(changed.status, "UNAVAILABLE");
  assert.match(changed.reason, /endpoint changed/i);
});
...
test("exact duplicate manual PAPER order produces one side effect only", () => {
  const submitted = validatePersonalPaperOrderCommand(command({ quantity: 0.001 }));
  ...
  const first = loop.submitManualOrder(submitted, context);
  const second = loop.submitManualOrder(submitted, context);
  ...
  assert.equal(loop.snapshot().processedIdempotencyKeys.filter((key) => key === submitted.idempotencyKey).length, 1);
});
