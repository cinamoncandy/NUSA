...
test("Home exposes one real safety-first next action from verified runtime state", () => {
  ...
  assert.match(home, /if \(notConfigured \|\| readOnlyError\) return onGoSettings\(\)/);
  ...
  assert.match(home, /aiInsightAvailable\s*\? "OPEN SIGNAL"/s);
  assert.doesNotMatch(home, /ORDER_CREATE|LIVE_EXECUTION|onSubmit/);
});
