...
test("SQLite restart preserves strategy depth residual and canonical fill-ledger continuation", () => {
  ...
  try {
    ...
    const restored = restoredLoop.snapshot();
    assert.equal(restored.workingOrders.length, 1);
    assert.equal(restored.workingOrders[0].lifecycle.remainingQuantity, 0.015);
    assert.equal(restored.workingOrders[0].remainingAllocationCapital, 750_000);
    assert.equal(restored.workingOrders[0].lastOrderBookObservedAt, 1_900);

    let restoredRiskCalls = 0;
    ...
    assert.equal(terminal.reason, "PAPER_STRATEGY_FILLED");
    assert.equal(restoredRiskCalls, 1);
