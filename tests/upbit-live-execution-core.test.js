...
} = require("../dist/apps/desktop/src/upbitRestAdapter.js");
const {
...
test("Upbit JWT uses HS512 and hashes the raw unencoded body parameter string", async () => {
  const calls = [];
  ...
  assert.notEqual(payload.query_hash, createHash("sha512").update(new URLSearchParams({
    market: "KRW-BTC",
    side: "bid",
    ord_type: "limit",
    volume: "0.0001",
...
test("test, real submit, and cancel use the correct Upbit endpoints exactly once", async () => {
  const calls = [];
  const fetchImpl = async (url, init) => {
    calls.push({ url, init });
    return jsonResponse(orderFixture());
  };
  const client = new UpbitRestClient({ credentials, fetchImpl, nonce: () => "nonce" });
...
test("mutating Upbit requests are never automatically retried", async () => {
  ...
  const fetchImpl = async () => {
    calls += 1;
    return jsonResponse({ error: { name: "server_error", message: "temporary" } }, 500);
  };
  const client = new UpbitRestClient({
    credentials,
    fetchImpl,
  ...
  await assert.rejects(
    client.submitOrder({ market: "KRW-BTC", side: "bid", ord_type: "price", price: "10000" }),
  );
...
test("invalid order combinations fail before any network request", async () => {
  ...
  await assert.rejects(
    client.submitOrder({ market: "KRW-BTC", side: "bid", ord_type: "market", volume: "0.001" }),
    /market order is a market sell/,
  );
  assert.equal(calls, 0);
});

test("default LIVE adapter stays read-only and live execution requires all environment gates", () => {
  const readOnly = createTradingAdapter({
    NUSA_TRADING_ADAPTER_MODE: "LIVE",
    NUSA_ENABLE_LIVE_ADAPTER: "true",
    UPBIT_ACCESS_KEY: "a",
    UPBIT_SECRET_KEY: "b",
  });
  ...
  assert.throws(() => createLiveOrderExecutionAdapter({
    NUSA_TRADING_ADAPTER_MODE: "LIVE",
    NUSA_ENABLE_LIVE_ADAPTER: "true",
    UPBIT_ACCESS_KEY: "a",
    UPBIT_SECRET_KEY: "b",
...
test("per-order live authority is mandatory and withdraw remains disabled", async () => {
  ...
  const adapter = createLiveOrderExecutionAdapter({
    NUSA_TRADING_ADAPTER_MODE: "LIVE",
    NUSA_ENABLE_LIVE_ADAPTER: "true",
    NUSA_ENABLE_LIVE_ORDER_MUTATION: "true",
  }, { liveOrderAdapter: orderAdapter });
