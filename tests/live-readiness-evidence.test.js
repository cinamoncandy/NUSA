...
test('a previously green prerequisite automatically revokes readiness on recomputation', () => {
  ...
  const regressed = evaluateLiveReadiness(
    collectLiveReadinessEvidence(greenSnapshot({ realAccountMonitor: 'STALE' })),
    healthyRuntime,
  );
...
test('required workflows only pass when every required workflow is green on the exact current head', () => {
  const wrongHead = collectLiveReadinessEvidence(greenSnapshot({
    workflows: { ...greenSnapshot().workflows, headSha: 'older-head' },
  }));
  assert.equal(wrongHead.requiredCiPasses, false);

  const failedMobile = collectLiveReadinessEvidence(greenSnapshot({
    workflows: { ...greenSnapshot().workflows, mobileNative: 'FAIL' },
  }));
...
test('unknown or failing machine evidence never auto-passes', () => {
  const evidence = collectLiveReadinessEvidence(greenSnapshot({
    governance: 'UNKNOWN',
    prohibitedFinancialMutationScan: 'UNKNOWN',
  }));
