...
const { SqliteDatabase, SqliteCandidatePromotionRepository } = require("../dist/packages/storage/src/index.js");
const { InMemoryResearchEvaluationLedger } = require("../dist/apps/cloud/src/researchRuntimeCoordinator.js");
...
test("owner promotion rejects missing or invalid explicit cost evidence", () => {
  for (const costEvidence of [undefined, { ...evidence().costEvidence, observedAt: now }]) {
    ...
    try {
      ...
      ledger.append(evidence({ costEvidence }));
      const result = runtime.evaluatePromotionEligibility("candidate-1", "evaluation-1");
      assert.equal(result.eligible, false);
      assert.match(result.reason, /COST_EVIDENCE/);
    } finally {
      db.close();
    }
  }
...
test("inconclusive, stale, mismatched, or tampered evidence cannot promote", () => { for (const override of [{ result: "INCONCLUSIVE" }, { canonicalInputHash: "d".repeat(64) }, { evaluationTimestamp: now - 90_000_000 }]) { const { db, ledger, runtime } = setup(); try { runtime.registerCandidate(identity()); ledger.append(evidence(override)); assert.equal(runtime.evaluatePromotionEligibility("candidate-1", "evaluation-1").eligible, false); } finally { db.close(); } } });
test("same command id with a different payload is rejected and CAS conflicts retain champion", () => { const { db, ledger, runtime } = setup(); try { runtime.registerCandidate(identity()); ledger.append(evidence()); const base = { promotionCommandId: "promotion-1", expectedCurrentChampionCandidateId: null, candidateId: "candidate-1", evidenceEvaluationId: "evaluation-1", evidenceHash: digest(evidence()), ownerActorRef: "owner-1", reason: "owner review", requestedAt: now }; assert.equal(runtime.promote(base).status, "PROMOTED"); assert.equal(runtime.promote({ ...base, reason: "different" }).status, "CONFLICT"); assert.equal(runtime.promote({ ...base, promotionCommandId: "promotion-2", expectedCurrentChampionCandidateId: null }).status, "CONFLICT"); } finally { db.close(); } });
test("audit chain uses verified actor identity and detects tampering", () => { const { db, ledger, runtime } = setup(); try { runtime.registerCandidate(identity()); ledger.append(evidence()); const command = { promotionCommandId: "promotion-1", expectedCurrentChampionCandidateId: null, candidateId: "candidate-1", evidenceEvaluationId: "evaluation-1", evidenceHash: digest(evidence()), ownerActorRef: "bad", reason: "review", requestedAt: now }; runtime.promote(command); assert.equal(runtime.listAudit().length, 1); assert.equal(runtime.listAudit()[0].actorRef, "UNAUTHENTICATED"); runtime.verifyAudit(); db.connection.prepare("UPDATE research_promotion_audit SET record_json = ? WHERE sequence = 1").run("{}"); assert.throws(() => runtime.verifyAudit(), /integrity/); } finally { db.close(); } });

test("new candidates require explicit canonical family binding", () => {
  const { db, runtime } = setup();
  try {
    assert.throws(() => runtime.registerCandidate(identity({ familyId: undefined })), /FAMILY_BINDING_REQUIRED/);
    assert.throws(() => runtime.registerCandidate(identity({ familyId: "unknown.family" })), /UNKNOWN_FAMILY/);
  } finally { db.close(); }
});
test("promotion fails closed when persisted candidate family binding is absent", () => {
