...
test("FIXED_FRACTIONAL mode sizes from equity * riskFraction / price", () => {
  const planner = new OrderPlanner("FIXED_FRACTIONAL", 0.01);
...
test("FIXED_FRACTIONAL mode floors to quantityStep when provided", () => {
  const planner = new OrderPlanner("FIXED_FRACTIONAL", 0.0123);
...
test("constructor requires a valid riskFraction when sizingMode is FIXED_FRACTIONAL", () => {
  assert.throws(() => new OrderPlanner("FIXED_FRACTIONAL"), /riskFraction/);
  assert.throws(() => new OrderPlanner("FIXED_FRACTIONAL", 0), /riskFraction/);
  assert.throws(() => new OrderPlanner("FIXED_FRACTIONAL", 1.5), /riskFraction/);
});
