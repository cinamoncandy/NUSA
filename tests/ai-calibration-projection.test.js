...
const baseResult = () => ({
  ...
  agents: [{ modelVersionId: "model-v1", definitionVersion: "1.0.0" }],
  contexts: [],
...
const cohort = { providerId: "openai", modelVersionId: "model-v1", promptArtifactDigest: digest, outcomeDefinitionId: "next-period-positive", outcomeDefinitionVersion: "1" };
...
test("insufficient and degraded calibration can never create trusted confidence", () => {
  for (const status of ["INSUFFICIENT_DATA", "DEGRADED"]) {
    ...
    assert.equal(projection.calibrationStatus, status);
    assert.equal(projection.rawProbability, 0.8);
    assert.equal(projection.confidence, 0);
    assert.equal(projection.effectiveConfidence, 0);
    assert.equal(projection.calibratedProbability, null);
...
test("calibrated confidence is exposed conservatively and never exceeds raw probability", () => {
  ...
  assert.equal(verified.calibrationStatus, "CALIBRATED");
  assert.equal(verified.rawProbability, 0.8);
  assert.equal(verified.calibratedProbability, 0.6);
  assert.equal(verified.confidence, 0.6);
  assert.equal(verified.effectiveConfidence, 0.6);
  assert.equal(verified.calibrationSampleCount, 25);
  assert.equal(verified.calibrationExpectedError, 0.05);
  assert.equal(verified.calibrationBrierScore, 0.1);

  const defensive = projectAiReadOnly(baseResult(), profile({ calibratedProbability: 0.95, effectiveConfidence: 0.95 }));
  assert.equal(defensive.confidence, 0.8);
  assert.equal(defensive.effectiveConfidence, 0.8);
});
...
test("calibration identity mismatch fails closed instead of borrowing another cohort", () => {
  for (const mismatched of [
    ...
    assert.equal(projection.confidence, 0);
    assert.equal(projection.calibratedProbability, null);
    assert.equal(projection.calibrationCohort, null);
  }
});

test("incomplete governance suppresses trusted confidence even with a valid calibrated profile", () => {
  const result = baseResult();
  result.governanceDecision = { result: "incomplete", unresolvedDisagreements: ["critic"], vetoReasons: [] };
