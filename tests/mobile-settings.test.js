...
test("mock settings repository persists an immutable normalized model including non-secret PAPER endpoint", async () => {
  ...
  assert.deepEqual(await repository.load(), { theme: "DARK", locale: "en-US", notifications: { enabled: true, riskAlerts: false, orderUpdates: true }, paperEndpoint: "https://paper.test" });
});
...
test("secure settings repository persists encoded settings and endpoint but no dashboard credential", async () => {
  ...
  assert.deepEqual(await repository.load(), { theme: "LIGHT", locale: "ko-KR", notifications: { enabled: false, riskAlerts: true, orderUpdates: false }, paperEndpoint: "https://paper.test" });
});