...
test("settings UI exposes PAPER connection, cash allocation, appearance, safety and local management", () => {
  ...
  assert.doesNotMatch(source, /settings-paper-endpoint|endpointDraft|Canonical Cloud endpoint/);
  assert.match(source, /연결 주소는 앱이 안전하게 관리합니다\. bootstrap token은 저장하지 않고 한 번만 세션으로 교환합니다/);
  ...
  assert.match(source, /paperConnected \? "PAPER 연결됨"/);
