...
test("autopilot execution workflow keeps ownership metadata optional as an exact pair", () => {
  ...
  assert.throws(() => validate({ canonical_owner: "autopilot.control-plane" }), /must be supplied together/, "owner-only payload fails closed");
  assert.throws(() => validate({ conflict_keys: ["module:apps/autopilot/src"] }), /must be supplied together/, "conflict-only payload fails closed");
  assert.doesNotThrow(() => validate({ canonical_owner: "autopilot.control-plane", conflict_keys: ["module:apps/autopilot/src"] }), "valid owned dispatch is accepted");
