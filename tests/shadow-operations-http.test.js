...
const token = "shadow-test-token";
const principal = Object.freeze({ userId: "shadow-user", email: "shadow@nusa.local", scopes: Object.freeze(["dashboard:read"]) });
const verifier = { ownerPrincipal: principal, verify(value) { return value === token ? principal : undefined; } };
...
test("Shadow operations is authenticated GET-only and preserves safety invariants", async () => {
  ...
  try {
    ...
    const response = await request(handle.port, "GET", "/api/shadow-operations", { authorization: `Bearer ${token}` });
    ...
    assert.equal((await request(handle.port, "POST", "/api/shadow-operations", { authorization: `Bearer ${token}` })).status, 405);
...
test("Shadow operations fails closed when provider is absent", async () => {
  ...
  try { assert.equal((await request(handle.port, "GET", "/api/shadow-operations", { authorization: `Bearer ${token}` })).status, 503); }
  finally { await handle.stop(); }
});
