...
test("mobile PAPER falls back to a shared 10M KRW local simulator without Cloud authority", () => {
  const app = read("apps/mobile/App.tsx");
  const view = read("apps/mobile/src/tradingView.tsx");
  const store = read("apps/mobile/src/localPaperStore.ts");
  assert.match(app, /activeTab !== "Portfolio" && activeTab !== "Paper"/);
  assert.match(store, /LOCAL_PAPER_INITIAL_CASH = 10_000_000/);
  assert.match(store, /new MockTradingService\(\[\{ currency: "KRW", available: LOCAL_PAPER_INITIAL_CASH \}\]\)/);
  assert.match(view, /const usingLocalPaper = !builtInSubmitAvailable/);
  assert.match(view, /loadUpbitPublicMarkets\(\)/);
  assert.match(view, /placeLocalPaperOrder/);
