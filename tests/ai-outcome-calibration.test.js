...
const cohort = Object.freeze({ providerId: "openai", modelVersionId: "model-v1", promptArtifactDigest: digest, outcomeDefinitionId: "next-period-positive", outcomeDefinitionVersion: "1" });
...
test("reliability buckets include probability zero and one and compute weighted ECE and Brier", () => {
  ...
  assert.equal(perfect.sampleCount, 2);
  assert.equal(perfect.expectedCalibrationError, 0);
  assert.equal(perfect.brierScore, 0);
  assert.equal(perfect.reliabilityBuckets[0].count, 1);
  assert.equal(perfect.reliabilityBuckets[9].count, 1);

  const known = computeCalibrationMetrics([{ probability: 0.25, outcome: true }, { probability: 0.25, outcome: false }], 2);
  assert.equal(known.expectedCalibrationError, 0.25);
  assert.equal(known.brierScore, 0.3125);
});

test("profiles are exact-cohort, insufficient by default, and synthetic observations cannot satisfy runtime evidence", () => {
  const ledger = new OutcomeCalibrationLedger();
  const synthetic = prediction("synthetic", 0.9, "SYNTHETIC_TEST");
  ledger.appendPrediction(synthetic);
  ledger.appendOutcome(outcome("synthetic", true, "SYNTHETIC_TEST"));
  ...
  const verified = prediction("verified", 0.8);
  ledger.appendPrediction(verified);
  ledger.appendOutcome(outcome("verified", true));
  profile = ledger.profile({ ...cohort, modelVersionId: "other-model" }, 0.8, { minimumSamples: 1 });
  assert.equal(profile.sampleCount, 0);
  assert.equal(profile.status, "INSUFFICIENT_DATA");
});
...
test("calibrated confidence is conservative and degraded calibration cannot increase authority or confidence", () => {
  ...
  for (const id of ["a", "b"]) {
    good.appendPrediction(prediction(id, 0.8));
    good.appendOutcome(outcome(id, true));
  }
  ...
  assert.equal(calibrated.calibratedProbability, 1);
  assert.equal(calibrated.effectiveConfidence, 0.8);
  assert.equal(calibrated.liveAuthority, "NONE");
  assert.equal(calibrated.productionMutationAllowed, false);

  const bad = new OutcomeCalibrationLedger();
  for (const id of ["c", "d"]) {
    bad.appendPrediction(prediction(id, 0.9));
    bad.appendOutcome(outcome(id, false));
  }
  const degraded = bad.profile(cohort, 0.9, { minimumSamples: 2, maximumExpectedCalibrationError: 0.1, maximumBrierScore: 0.1 });
  assert.equal(degraded.status, "DEGRADED");
  assert.equal(degraded.calibratedProbability, null);
  assert.equal(degraded.effectiveConfidence, 0);
  assert.equal(degraded.liveAuthority, "NONE");
  assert.equal(degraded.productionMutationAllowed, false);
});

test("outcome definition and provenance mismatches fail closed", () => {
  const ledger = new OutcomeCalibrationLedger();
  ledger.appendPrediction(prediction("p1", 0.5));
