...
test("the header presents the NUSA product identity", () => {
  assert.match(indexHtml, /<h1 class="cr-wordmark">NUSA<\/h1>/);
  assert.ok(fs.existsSync(path.join(root, "apps/desktop/renderer/assets/dokkaebi-mark.svg")));
  assert.match(indexHtml, /<title>NUSA Paper Console<\/title>/);
});
