...
const result = (overrides = {}) => ({
  ...
  agents: [{ agentId: "ai-strategy-proposer", modelVersionId: "model-v1", definitionVersion: "1.0.0", promptArtifactId, promptArtifactDigest: digest }], contexts: [],
  ...
  failureCodes: [], outputHashes: [], structuredOutputs: [{ schemaVersion: 1, role: "STRATEGY_PROPOSER", evidenceReferences: ["e-1"], payload: { rawProbability: 0.73 } }],
  liveAuthority: "NONE", realOrderAuthority: false, realTransferAuthority: false, productionMutationAllowed: false, ...overrides
});
const options = { providerId: "openai", outcomeDefinitionId: "next-period-positive", outcomeDefinitionVersion: "1", horizonMs: 60_000 };
...
test("verified proposer output becomes an immutable calibration prediction identity", () => {
  ...
  assert.equal(prediction.rawProbability, 0.73); assert.equal(prediction.predictedAt, 120); assert.equal(prediction.horizonMs, 60_000); assert.equal(prediction.provenance, "VERIFIED_RUNTIME"); assert.equal(verifyCalibrationPrediction(prediction), true);
});

test("bridge refuses incomplete, failed, missing, mismatched, or invalid proposer output", () => {
  ...
  assert.throws(() => createVerifiedRuntimeCalibrationPrediction(result({ structuredOutputs: [{ schemaVersion: 1, role: "STRATEGY_PROPOSER", evidenceReferences: [], payload: { rawProbability: 2 } }] }), options), /within \[0,1\]/);
