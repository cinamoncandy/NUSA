...
test("HOME MASTER AI judgment drills into verified evidence without a dead control", () => {
  ...
  assert.match(home, /value=\{decision\.why\}/);
});

test("approved HOME scan order is market pulse → AI terrain/evidence → PAPER → risk authority", () => {
  const home = read("apps/mobile/src/homeView.tsx");
  ...
  assert.ok(market < ai && ai < why && why < paper && paper < risk);
