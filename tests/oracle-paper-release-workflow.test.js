...
test("failed rollback readiness explicitly stops runtimes fail-closed", () => {
  ...
  assert.ok(activateCase.indexOf("stop_units_fail_closed") < activateCase.indexOf("rollback PAPER readiness failed; services stopped"), "services must be stopped before the rollback PAPER failure exits");
});
