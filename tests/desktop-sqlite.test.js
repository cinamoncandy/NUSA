...
test("paper account, orders, control events, and signal keys survive restart", () => {
  ...
  const second = new DesktopPersistenceStore(filename); assert.deepEqual(second.load(), expected); second.close();
});
...
test("legacy JSON state imports only once", () => {
  ...
  assert.deepEqual(store.load(), expected); store.close();
});
...
test("failed write rolls back every runtime table", () => {
  ...
  store.db.exec("CREATE TRIGGER reject_control BEFORE UPDATE ON desktop_control_state BEGIN SELECT RAISE(ABORT, 'forced rollback'); END");
