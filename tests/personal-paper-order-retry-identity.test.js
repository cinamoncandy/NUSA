...
const responseFor = (submitted) => ({ ok: true, status: 200, redirected: false, url: "https://paper.example.test/api/paper-orders", json: async () => definitiveBlocked(submitted) });
...
test("ambiguous network retry sends the same idempotency key until a definitive PAPER result arrives", async () => {
  ...
  const first = await submitPersonalPaperOrderWithRetryIdentity({
    ...
    request: async (_url, init) => { observedKeys.push(init.headers["idempotency-key"]); throw new Error("connection reset after request write"); }
  ...
  const second = await submitPersonalPaperOrderWithRetryIdentity({
    baseUrl: "https://paper.example.test",
    credentialProvider: async () => "1234567890123456",
    request: async (_url, init) => { observedKeys.push(init.headers["idempotency-key"]); const submitted = JSON.parse(init.body); return responseFor(submitted); }
  }, identity, fingerprint, createKey, draft);
  assert.equal(second.status, "READY");
  assert.equal(second.result.status, "BLOCKED");
  assert.equal(observedKeys[1], observedKeys[0], "ambiguous retry must hit the server with the original idempotency key");

  const third = await submitPersonalPaperOrderWithRetryIdentity({
    baseUrl: "https://paper.example.test",
    credentialProvider: async () => "1234567890123456",
    request: async (_url, init) => { observedKeys.push(init.headers["idempotency-key"]); throw new Error("stop after observing new submission identity"); }
  }, identity, fingerprint, createKey, draft);
  assert.equal(third.status, "UNAVAILABLE");
  assert.notEqual(observedKeys[2], observedKeys[1], "a new user submission after a definitive result must get a fresh key");
});
