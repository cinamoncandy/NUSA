...
test("Settings fields use mobile-appropriate endpoint and secret input semantics", () => {
  ...
  assert.match(source, /disabled=\{busy\} label=\{connecting \? "연결 확인 중\.\.\." : "저장 \+ 연결 테스트"\}/);
  assert.match(source, /disabled=\{busy\} label="연결 해제"/);
  assert.match(source, /value=\{connecting \? "CHECKING" : connection\.status\}/);
});
...
test("PAPER order inputs are numeric-first and locked during an in-flight submit", () => {
  ...
  assert.match(source, /keyboardType="decimal-pad" label="가격"/);
  assert.match(source, /keyboardType="decimal-pad" label=\{`수량/);
  assert.equal((source.match(/editable=\{!submitting\}/g) ?? []).length, 2);
  assert.match(source, /<NusaButton disabled=\{submitting\} label="매수"/);
  assert.match(source, /<NusaButton disabled=\{submitting\} label="시장가"/);
