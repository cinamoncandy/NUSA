...
const { setConfiguredPaperEndpoint, clearConfiguredPaperEndpoint, markPaperConnectionVerified } = require("../dist/apps/mobile/src/paperConnectionSession.js");
...
test("dashboard bearer credential is sent only to a verified secure or loopback endpoint", async () => {
  ...
  markPaperConnectionVerified("http://192.168.1.50:41731");
  ...
  setConfiguredPaperEndpoint("https://nusa.invalid");
  markPaperConnectionVerified("https://nusa.invalid");
  ...
  markPaperConnectionVerified("http://127.0.0.1:41731");
  ...
  assert.equal(requestCount, 2, "explicitly saved and verified loopback endpoint may receive the memory-only credential");
});
...
test("cloud runtime exposes one authenticated read-only snapshot with real PAPER account projection", async () => {
  ...
  try {
    ...
    assert.equal(body.liveAuthority, "NONE"); assert.equal(body.productionMutationAllowed, false); assert.equal(body.portfolio.mode, "PAPER"); assert.equal(body.portfolio.account.cash, 1000000); assert.equal(body.portfolio.account.equity, 1000000); assert.equal(body.portfolio.account.assetValue, 0); assert.equal(body.portfolio.account.realizedPnl, 0); assert.deepEqual(body.orders, []); assert.deepEqual(body.markets, []);
...
test("multi-position PAPER portfolio projects aggregate totals without corrupting representative position", async () => {
  ...
  const restoredState = Object.freeze({ version: 1, initialCapital: 1000000, cash: 603000, equity: 1043000, realizedPnL: 3000, unrealizedPnL: 40000, positions: Object.freeze([Object.freeze({ market: "KRW-BTC", quantity: 1, averageEntryPrice: 200000, realizedPnL: 1000, unrealizedPnL: 20000, markPrice: 220000 }), Object.freeze({ market: "KRW-ETH", quantity: 2, averageEntryPrice: 100000, realizedPnL: 2000, unrealizedPnL: 20000, markPrice: 110000 })]), orders: Object.freeze([]), fills: Object.freeze([]), processedIdempotencyKeys: Object.freeze([]), updatedAt: 1000 });
  ...
  try {
    const body = await loadOperations(handle, token);
    assert.equal(body.portfolio.account.assetValue, 440000); assert.equal(body.portfolio.account.realizedPnl, 3000); assert.equal(body.portfolio.account.unrealizedPnl, 40000); assert.equal(body.portfolio.account.position.market, "KRW-BTC"); assert.equal(body.portfolio.account.position.realizedPnl, 1000); assert.equal(body.portfolio.account.position.unrealizedPnl, 20000);
    const view = buildPortfolioViewModel(body.portfolio);
    assert.equal(view.assetValue, 440000); assert.equal(view.realizedPnl, 3000); assert.equal(view.unrealizedPnl, 40000); assert.equal(view.totalPnl, 43000); assert.equal(view.position.market, "KRW-BTC"); assert.equal(view.position.realizedPnl, 1000); assert.equal(view.position.unrealizedPnl, 20000);
  } finally { await handle.stop(); }
});
...
test("closed lexicographic PAPER position cannot hide another open position", async () => {
  ...
  const restoredState = Object.freeze({ version: 1, initialCapital: 1000000, cash: 800000, equity: 1020000, realizedPnL: 0, unrealizedPnL: 20000, positions: Object.freeze([Object.freeze({ market: "KRW-BTC", quantity: 0, averageEntryPrice: 0, realizedPnL: 0, unrealizedPnL: 0, markPrice: 220000 }), Object.freeze({ market: "KRW-ETH", quantity: 2, averageEntryPrice: 100000, realizedPnL: 0, unrealizedPnL: 20000, markPrice: 110000 })]), orders: Object.freeze([]), fills: Object.freeze([]), processedIdempotencyKeys: Object.freeze([]), updatedAt: 1000 });
  const paperLoop = new PaperTradingExecutionLoop({ initialCapital: 1000000, restoredState });
  const handle = startCloudRuntime(testEnv(token, 41945), undefined, undefined, undefined, undefined, undefined, paperLoop);
  try {
    const body = await loadOperations(handle, token);
    assert.equal(body.portfolio.account.assetValue, 220000); assert.equal(body.portfolio.account.position.market, "KRW-ETH"); assert.equal(body.portfolio.account.position.quantity, 2);
    const view = buildPortfolioViewModel(body.portfolio);
    assert.equal(view.assetValue, 220000); assert.equal(view.position.market, "KRW-ETH"); assert.equal(view.position.quantity, 2);
  } finally { await handle.stop(); }
});
...
test("open durable P0 projects Personal PAPER Operations as HALTED", async () => {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nusa-p8-open-p0-")); const dbPath = path.join(dir, "state.sqlite"); seedP0(dbPath);
  const handle = startCloudRuntime(testEnv(p0OpenCredential, 41942, dbPath));
  try { const body = await loadOperations(handle, p0OpenCredential); assert.equal(body.operations.runtimeState, "HALTED"); assert.equal(body.operations.accountHalted, true); assert.equal(body.health, "FAIL_CLOSED"); assert.equal(body.readyForPaperOperations, false); assert.equal(body.liveAuthority, "NONE"); assert.equal(body.productionMutationAllowed, false); }
...
test("unverifiable durable P0 projects Personal PAPER Operations as HALTED", async () => {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nusa-p8-corrupt-p0-")); const dbPath = path.join(dir, "state.sqlite"); seedP0(dbPath, true);
  const handle = startCloudRuntime(testEnv(p0CorruptCredential, 41943, dbPath));
  try { const body = await loadOperations(handle, p0CorruptCredential); assert.equal(body.operations.runtimeState, "HALTED"); assert.equal(body.operations.accountHalted, true); assert.equal(body.health, "FAIL_CLOSED"); assert.equal(body.readyForPaperOperations, false); assert.equal(body.liveAuthority, "NONE"); assert.equal(body.productionMutationAllowed, false); }
  finally { await handle.stop(); fs.rmSync(dir, { recursive: true, force: true }); }
});
...
test("mobile source keeps read projections separate while exposing only explicit PAPER mutation", () => {
  ...
  assert.match(app, /mobileSession/); assert.match(app, /snapshot\?\.portfolio/); assert.match(app, /snapshot\?\.orders/); assert.match(app, /publicMarkets\.markets/); assert.doesNotMatch(app, /snapshot\.markets/);
  ...
  assert.match(runtime, /portfolio:\s*buildReadOnlyPortfolio/); assert.match(runtime, /orders:\s*buildReadOnlyOrders/); assert.match(runtime, /markets:\s*\[\.\.\.latestTickers\.values\(\)\]/); assert.match(runtime, /latestTickers\.clear\(\)/); assert.match(runtime, /volume:\s*ticker\.acc_trade_volume\s*\?\?\s*null/); assert.doesNotMatch(runtime, /volume:\s*ticker\.acc_trade_price_24h/); assert.match(runtime, /SqliteP0AlertRepository/); assert.match(runtime, /readP0State:/); assert.match(runtime, /effectiveP0Repository\.readState\(\)\.openP0/);
});
