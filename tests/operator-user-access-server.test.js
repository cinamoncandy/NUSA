...
const verifier = {
  ownerPrincipal: { userId: "operator", email: "owner@nusa.local", scopes: ["dashboard:read", "users:manage"] },
...
test("first-seen authenticated user is PENDING, approval enables access, suspension invalidates the same bearer", async () => {
  ...
  const ownerPrincipal = { userId: "owner-real", email: "owner@example.com", scopes: ["dashboard:read", "users:manage"] };
  const identityVerifier = {
    ownerPrincipal,
    verify(value) {
      if (value === ownerToken) return ownerPrincipal;
      if (value === userToken) return { userId: "user-real", email: "user@example.com", displayName: "Real User", scopes: ["dashboard:read"] };
      return undefined;
    }
  };
  const repository = new InMemoryNusaUserAccessRepository();
