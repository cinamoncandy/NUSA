...
The repository now has grounded evidence, prompt/replay binding, a bounded asynchronous read-only runtime, and an explicitly opt-in real provider challenger. The weakest safe dimension is calibration/outcome linkage:

- `AiReadOnlyProjection.confidence` is still projected as zero.
- `calibrationStatus` is currently `UNKNOWN` only.
- the proposer contract does not yet carry a bounded numeric probability.
- existing `evaluateAgentCalibration()` accepts only a bare predicted confidence and boolean correctness and is not linked to provider/model/prompt/proposal/outcome identity.
...
Therefore calibration/outcome linkage is selected before explanation-confidence increases, model weighting, or any further provider-derived influence.
