...
### OCI Monitoring 측정 (2026-09-19, 소유자 콘솔)

인스턴스 `nusa-server`, 상태 **Running**. 위 HTTP 탐침과 독립적인 두 번째 측정입니다.

| 지표 | 2026-09-17 (#1949) | 2026-09-19 | 출처 |
|---|---|---|---|
| Load average | **21.75 / 19.93 / 19.45** | **약 0.5** (축 상한 1 미만) | OCI 그래프 |
| 가용 RAM | 954Mi 중 **147Mi** | 사용률 **41~51%** → 약 460~560Mi 여유 | 소유자가 그래프에서 읽음 |
| CPU | (포화) | 10~40% 변동, 천장 미접촉 | OCI 그래프 |
| swap | 6.0Gi 중 4.6Gi | (메트릭 없음) | — |
| Memory Allocation Stalls | — | 거의 0, 스파이크 1회 | OCI 그래프 |
| Disk Read/Write I/O | — | 정상 활동 | OCI 그래프 |
| **디스크 사용률 (%)** | **96%** | **측정 불가** | 아래 참조 |

**Load가 21.75에서 0.5로 떨어졌습니다.** 40배 이상입니다. 9/17에 호스트를 마비시킨 스래싱은
현재 존재하지 않습니다. 가용 RAM도 147Mi에서 약 500Mi로 회복됐습니다.

> **디스크는 여전히 미확인입니다.** OCI 기본 메트릭에는 Disk Read/Write **I/O**만 있고
> 파일시스템 점유율이 없습니다 — 그건 모니터링 플러그인이 켜져 있어야 나오며, 이 인스턴스는
> 꺼져 있습니다. 다만 certbot이 2026-09-19T10:01:36Z에 인증서를 **성공적으로 썼고** Disk Write
> I/O도 정상이므로, 쓰기가 막힐 만큼 가득 찬 상태는 아닙니다. 정확한 값은 셸에서 `df -h /`로
> 확인하며, 재배포에 어차피 셸이 필요하므로 별도 작업이 아닙니다.

### 재배포 시 반드시 지킬 것 — 사고 재현 방지

#1949의 근본 원인은 자원 고갈 자체가 아니라 **PAPER 서버를 빌드 머신으로 쓴 것**입니다. 9/17
기록에 `tsc` 프로세스가 1시간 43분~2시간 23분 실행 중이었습니다. 954Mi 호스트에서 전체
TypeScript 빌드를 돌리면 스왑에 빠지고, 여유가 500Mi인 지금도 `pnpm install` + `tsc`는 그것을
넘길 수 있습니다.

빌드에 메모리 상한을 걸어 호스트 전체를 보호하십시오:

```bash
systemd-run --scope -p MemoryMax=400M -p CPUQuota=50% <빌드 명령>
```

상한에 걸리면 그 프로세스만 죽고 서버는 삽니다. 상한이 없으면 커널이 무엇을 죽일지 고르며,
9/17에는 그것이 호스트 전체를 끌고 내려갔습니다.
