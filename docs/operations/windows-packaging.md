...
The installer is written to `release/Dokkaebi-<version>-Windows-Setup.exe`. The packaged app is
...
Run the NSIS installer, accept the per-user installation location, and launch Dokkaebi from the
...
normally `%APPDATA%\\dokkaebi`:

- `dokkaebi.db`: durable Paper/control/recovery state
- `paper-session.json`, `control-session.json`: compatibility session stores
- `shadow-evidence\\`: immutable Shadow Evidence archives
- recovery state and audit records: `dokkaebi.db` and the existing recovery/audit stores
