...
| Strategy | EXISTS (strongest) | `packages/core/src/strategyEngine.ts`: `SmaCrossoverStrategy`, `RsiMeanReversionStrategy`, `BollingerBreakoutStrategy`, `MacdMomentumStrategy` (all trailing-only, lookahead-guard tested), `strategyRegistry.ts`, `backtestEngine.ts`, `walkForwardEngine.ts` |
